...
import { configs, log as internalLogger, notify } from "./common.js";
...
function destroyRequest(request) {
  ...
  if (requests) requests.delete(request);
...
browser.runtime.onMessage.addListener((message, _sender) => {
  ...
  for (const request of requests) {
    ...
    const checked = onChanged ? onChanged(true) : undefined;
...
export function bindToCheckbox(permissions, checkbox, options = {}) {
  ...
  isGranted(permissions)
    ...
    .catch((_error) => {
      ...
      if (label) label.setAttribute("disabled", true);
    });
  ...
  checkbox.requestPermissions = async () => {
    ...
    try {
      if (!checkbox.checked) {
        ...
        if (options.onChanged) options.onChanged(false);
        return;
      }
      ...
      if (configs.requestingPermissionsNatively) return;

      configs.requestingPermissionsNatively = permissions;
      let granted = await browser.permissions
        .request(permissions)
        .catch(ApiTabs.createErrorHandler());
      configs.requestingPermissionsNatively = null;

      if (granted === undefined) granted = await isGranted(permissions);
      else if (!granted) return;

      if (granted) {
        const checked = options.onChanged ? options.onChanged(true) : undefined;
        for (const checkbox of checkboxes) {
          checkbox.checked = checked !== undefined ? !!checked : true;
        }
        browser.runtime
...
export function bindToClickable(permissions, node, { onChanged } = {}) {
  ...
  if (node.requestPermissions) return;

  node.requestPermissions = async () => {
    try {
      const checkboxes = checkboxesForPermission.get(permissions);
      if (
        configs.requestingPermissionsNatively ||
...
export function requestPostProcess() {
  if (!configs.requestingPermissions) return false;
