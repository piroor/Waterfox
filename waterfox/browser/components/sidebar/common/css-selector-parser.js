...
export function splitSelectorParts(selector) {
  if (isAtRule(selector)) return [selector];
...
function split(input, { splitter, splitters, appendSplitter }) {
  ...
  for (const char of input) {
    ...
    if (char === "'") {
      ...
      if (doubleQuoted) doubleQuoted = false;
      ...
      if (!parens && !angulars && !singleQuoted && !doubleQuoted) {
        splitted.push(soFar.trim());
        soFar = "";
        if (appendSplitter) soFar += char;
        continue;
      }
...
`
  ...
  .split("\n")
  .map((item) => item.trim());
const PSEUDO_ELEMENTS_MATCHER = new RegExp(
