...
export async function getIndexes(...queriedTabIds) {
  ...
  if (queriedTabIds.length === 0) return [];

  const indexes = await Promise.all(
    queriedTabIds.map((tabId) => {
      return browser.tabs.get(tabId).catch((e) => {
...
export function isMissingTabError(error) {
  return error?.message?.includes("Invalid tab ID:");
}
...
export function isUnloadedError(error) {
  return error?.message?.includes("can't access dead object");
}
export function handleUnloadedError(error) {
  if (!isUnloadedError(error)) throw error;
}

export function isMissingHostPermissionError(error) {
  return error?.message?.includes("Missing host permission for the tab");
}
export function handleMissingHostPermissionError(error) {
  if (!isMissingHostPermissionError(error)) throw error;
}
  ...
  return (error) => {
    try {
      if (handlers.length > 0) {
        ...
        if (unhandledCount === handlers.length)
          // not handled
          throw error;
      } else {
        throw error;
      ...
      if (!configs.debug) throw newError;
      if (error === newError) console.log("Unhandled Error: ", error, stack);
      else console.log("Unhandled Error: ", error, newError, stack);
    }
  };
}
