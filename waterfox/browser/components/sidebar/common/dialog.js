...
export async function show(ownerWindow, dialogParams) {
  ...
  try {
    if (
    ...
    } else if (isMacOS() && ownerWindow.state === "fullscreen") {
      ...
      UserOperationBlocker.blockIn(ownerWindow.id, {
        throbber: false,
        shade: true,
      });
...
export function tabsToHTMLList(tabs, { maxHeight, maxWidth }) {
  const rootLevelOffset = tabs
    .map((tab) => parseInt(tab.$TST.getAttribute(Constants.kLEVEL) || 0))
    .sort()[0];
  return (
    ...
    tabs
      .map(
        (tab) => `<li style="align-items: center;
                                  display: flex;
                                  flex-direction: row;
                          ...
                          ><img style="display: flex;
                               ...
                               ><span style="margin-left: 0.25em;
                                             ...
                                             white-space: nowrap;"
                                     >${sanitizeForHTMLText(tab.title)}</span></li>`
      )
      .join("") +
    `</ul>`
  );
