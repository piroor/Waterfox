...
function flushBufferedUpdates() {
  ...
  (Constants.IS_BACKGROUND
    ...
    if (flushBufferedUpdates.triedAt !== triedAt) return;
    for (const update of mBufferedUpdates.values()) {
      ...
      if (
        Object.keys(update.attributes.updated).length > 0 ||
      ...
      if (update.pinned !== undefined)
        SidebarConnection.sendMessage({
          type: update.pinned
            ? Constants.kCOMMAND_NOTIFY_TAB_PINNED
            : Constants.kCOMMAND_NOTIFY_TAB_UNPINNED,
          windowId: update.windowId,
          tabId: update.tabId,
        });
      if (update.hidden !== undefined)
        SidebarConnection.sendMessage({
          type: update.hidden
            ? Constants.kCOMMAND_NOTIFY_TAB_HIDDEN
            : Constants.kCOMMAND_NOTIFY_TAB_SHOWN,
          windowId: update.windowId,
          tabId: update.tabId,
        });
    }
...
export function updateTab(tab, newState = {}, options = {}) {
  ...
  if ("url" in newState) {
    tab.$TST.setAttribute(
      ...
      (update.attributes.added[Constants.kCURRENT_URI] = newState.url)
    );
  ...
  if (
    ...
    tab.$TST.states.has(Constants.kTAB_STATE_GROUP_TAB) &&
  ...
  if (
    options.forceApply ||
    ("title" in newState && newState.title !== oldState.title)
  ) {
    if (options.forceApply) {
      tab.$TST.getPermanentStates().then((states) => {
        tab.$TST.toggleState(
          Constants.kTAB_STATE_UNREAD,
          states.includes(Constants.kTAB_STATE_UNREAD) && !tab.$TST.isGroupTab,
          { permanently: true }
        );
      });
  ...
  const openerOfGroupTab =
    tab.$TST.isGroupTab && Tab.getOpenerFromGroupTab(tab);
  if (openerOfGroupTab?.favIconUrl) {
    update.favIconUrl = openerOfGroupTab.favIconUrl;
  } else if (options.forceApply || "favIconUrl" in newState) {
    tab.$TST.setAttribute(
      Constants.kCURRENT_FAVICON_URI,
      (update.attributes.added[Constants.kCURRENT_FAVICON_URI] = tab.favIconUrl)
    );
  ...
  if (
    (options.forceApply || "pinned" in newState) &&
  ...
  if (options.forceApply || soundStateChanged || "audible" in newState) {
    soundStateChanged = true;
    tab.$TST.toggleState(
  ...
  if (options.forceApply || "sharingState" in newState) {
    ...
    const sharingScreen =
      newState.sharingState && !!newState.sharingState.screen;
  ...
  if (options.forceApply || "cookieStoreId" in newState) {
  ...
  if (options.forceApply || "incognito" in newState)
    tab.$TST.toggleState(
      Constants.kTAB_STATE_PRIVATE_BROWSING,
      newState.incognito
    );

  if (options.forceApply || "hidden" in newState) {
    if (newState.hidden) {
  ...
  if (options.forceApply || "highlighted" in newState)
    tab.$TST.toggleState(
      Constants.kTAB_STATE_HIGHLIGHTED,
      newState.highlighted
    );

  if (options.forceApply || "attention" in newState)
    tab.$TST.toggleState(Constants.kTAB_STATE_ATTENTION, newState.attention);

  if (options.forceApply || "discarded" in newState) {
    wait(0).then(() => {
  ...
  update.sharingStateChanged =
    update.sharingStateChanged || sharingStateChanged;
...
export async function updateTabsHighlighted(highlightInfo) {
  ...
  const alreadyHighlightedTabs = TabsStore.highlightedTabsInWindow.get(
  ...
  const inheritToCollapsedDescendants =
    !!highlightInfo.inheritToCollapsedDescendants;
...
async function updateTabHighlighted(
  tab,
  highlighted,
  { inheritToCollapsedDescendants } = {}
) {
  log(`highlighted status of ${dumpTab(tab)}: `, {
    old: tab.highlighted,
    new: highlighted,
  });
  ...
  if (!inheritHighlighted) win.tabsToBeHighlightedAlone.delete(tab.id);
  updateTab(tab, { highlighted });
  Tab.onUpdated.dispatch(
    tab,
    { highlighted },
    {
...
export async function completeLoadingTabs(windowId) {
  const completedTabs = new Set(
    (await browser.tabs.query({ windowId, status: "complete" })).map(
      (tab) => tab.id
    )
  );
  for (const tab of Tab.getLoadingTabs(windowId, {
    ordered: false,
    iterator: true,
  })) {
    if (completedTabs.has(tab.id)) updateTab(tab, { status: "complete" });
  }
}
