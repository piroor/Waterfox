...
export function getParentTabOperationBehavior(
  tab,
  {
    context,
    byInternalOperation,
  ...
  const sidebarVisible = SidebarConnection.isInitialized()
    ? (windowId || tab) && SidebarConnection.isOpen(windowId || tab.windowId)
    : true;
  log("getParentTabOperationBehavior ", tab, {
    byInternalOperation,
    preventEntireTreeBehavior,
    parent,
    sidebarVisible /*, stack: configs.debug && new Error().stack */,
  });
  ...
  if (
    ...
    replacedParentCount &&
    replacedParentCount >=
      configs.closeParentBehavior_replaceWithGroup_thresholdToPrevent
  ) {
    behavior = Constants.kPARENT_TAB_OPERATION_BEHAVIOR_PROMOTE_INTELLIGENTLY;
    log(
      " => the group tab is already replaced parent, fallback to kPARENT_TAB_OPERATION_BEHAVIOR_PROMOTE_INTELLIGENTLY"
    );
  }
  ...
  if (
    ...
    behavior = Constants.kPARENT_TAB_OPERATION_BEHAVIOR_PROMOTE_INTELLIGENTLY;
    log(
      " => preventEntireTreeBehavior behavior, fallback to kPARENT_TAB_OPERATION_BEHAVIOR_PROMOTE_INTELLIGENTLY"
    );
  }
...
export function calculateReferenceTabsFromInsertionPosition(
  ...
  if (insertBefore) {
    ...
    */
    ...
    if (!prevTab) {
      log(
        "calculateReferenceTabsFromInsertionPosition: from insertBefore, CASE 1/5"
      );
      // allow to move pinned tab to beside of another pinned tab
      if (!firstTab || firstTab.pinned === insertBefore?.pinned) {
        return {
          insertBefore,
        };
      } else {
      ...
      const prevLevel = Number(
        prevTab?.$TST?.getAttribute(Constants.kLEVEL) || 0
      );
      const targetLevel = Number(
        insertBefore?.$TST?.getAttribute(Constants.kLEVEL) || 0
      );
      let parent = null;
      if (!firstTab || !firstTab.pinned) {
        if (prevLevel < targetLevel) {
          if (context === Constants.kINSERTION_CONTEXT_MOVED) {
            log(
              "calculateReferenceTabsFromInsertionPosition: from insertBefore, CASE 4, prevTab = ",
              prevTab
            );
            parent = prevTab;
          } else {
            log(
              "calculateReferenceTabsFromInsertionPosition: from insertBefore, CASE 8, prevTab = ",
              prevTab
            );
          ...
          if (
            ...
            log(
              "calculateReferenceTabsFromInsertionPosition: from insertBefore, CASE 2/3/7"
            );
            parent = possibleParent;
          } else {
            log(
              "calculateReferenceTabsFromInsertionPosition: from insertBefore, CASE 6"
            );
            parent =
              (firstTab?.$TST?.parent !== possibleParent && possibleParent) ||
              firstTab?.$TST?.parent;
          }
        }
      }
  ...
  if (insertAfter) {
    ...
    */
    ...
    if (firstTab && unsafeNextTab === firstTab)
      // failsafe
    ...
    if (!nextTab) {
      ...
      if (context === Constants.kINSERTION_CONTEXT_MOVED) {
        log(
          "calculateReferenceTabsFromInsertionPosition: from insertAfter, CASE 1"
        );
        result = {
        ...
        log(
          "calculateReferenceTabsFromInsertionPosition: from insertAfter, CASE 5"
        );
        result = {
      ...
      const targetLevel = Number(
        insertAfter?.$TST?.getAttribute(Constants.kLEVEL) || 0
      );
      const nextLevel = Number(
        nextTab?.$TST?.getAttribute(Constants.kLEVEL) || 0
      );
      let parent = null;
      if (!firstTab || !firstTab.pinned) {
        if (targetLevel < nextLevel) {
          log(
            "calculateReferenceTabsFromInsertionPosition: from insertAfter, CASE 4/8"
          );
          parent = insertAfter;
        } else {
          ...
          if (
            ...
            log(
              "calculateReferenceTabsFromInsertionPosition: from insertAfter, CASE 2/3/7"
            );
            parent = possibleParent;
          } else {
            log(
              "calculateReferenceTabsFromInsertionPosition: from insertAfter, CASE 6"
            );
            parent =
              (firstTab?.$TST?.parent !== possibleParent && possibleParent) ||
              firstTab?.$TST.parent;
  ...
  throw new Error(
    "calculateReferenceTabsFromInsertionPosition requires one of insertBefore or insertAfter parameter!"
  );
}
...
export function getTreeStructureFromTabs(
  tabs,
  { full, keepParentOfRootTabs } = {}
) {
  ...
  */
  ...
  return cleanUpTreeStructureArray(
    tabs.map((tab, index) => {
      ...
      const indexInGivenTabs = parent
        ? tabIds.indexOf(parentId)
        : STRUCTURE_NO_PARENT;
      return indexInGivenTabs >= index ? STRUCTURE_NO_PARENT : indexInGivenTabs;
    }),
    STRUCTURE_NO_PARENT
  ).map((parentIndex, index) => {
    if (parentIndex === STRUCTURE_NO_PARENT && keepParentOfRootTabs)
