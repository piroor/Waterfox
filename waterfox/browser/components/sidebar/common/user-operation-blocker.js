...
export function block({ throbber, shade } = {}) {
  ...
  if (throbber) {
    ...
    document.documentElement.classList.add(
      Constants.kTABBAR_STATE_BLOCKING_WITH_SHADE
    );
  }
}
...
export function setProgress(percentage, windowId = null) {
  ...
  if (mProgressbar) mProgressbar.value = percentage;
...
export function blockIn(windowId, { throbber, shade } = {}) {
  ...
  if (targetWindow && targetWindow !== windowId) return;
...
export function unblock() {
  mBlockingThrobberCount--;
  if (mBlockingThrobberCount < 0) mBlockingThrobberCount = 0;
  if (mBlockingThrobberCount === 0) {
    setProgress(0);
    mProgressbar.classList.remove("shown");
    if (mProgressbar.delayedShow) clearTimeout(mProgressbar.delayedShow);
  ...
  mBlockingCount--;
  if (mBlockingCount < 0) mBlockingCount = 0;
  if (mBlockingCount === 0)
