...
export async function activateTab(
  tab,
  { byMouseOperation, keepMultiselection, silently } = {}
) {
  if (!Constants.IS_BACKGROUND)
    throw new Error(
      "Error: TabsInternalOperation.activateTab is available only on the background page, use a `kCOMMAND_ACTIVATE_TAB` message instead."
    );
  ...
  if (byMouseOperation) win.internallyFocusingByMouseTabs.add(tab.id);
  if (silently) win.internallyFocusingSilentlyTabs.add(tab.id);
  const onError = (e) => {
  ...
  if (
    configs.supportTabsMultiselect &&
    typeof browser.tabs.highlight === "function"
  ) {
    let tabs = [tab];
    if (tab.$TST.hasOtherHighlighted && keepMultiselection) {
      ...
      if (
        ...
        tabs = tabs.concat(
          mapAndFilter(
            highlightedTabs,
            (highlightedTab) =>
              (highlightedTab.id !== tab.id && highlightedTab) || undefined
          )
        );
      }
    }
    if (tabs.length === 1) win.tabsToBeHighlightedAlone.add(tab.id);
    highlightTabs(tabs);
  } else {
...
export async function blurTab(bluredTabs, { windowId, silently } = {}) {
  if (bluredTabs && !Array.isArray(bluredTabs)) bluredTabs = [bluredTabs];

  const bluredTabIds = new Set(
    Array.from(bluredTabs || [], (tab) => tab.id || tab)
  );

  // First, try to find successor based on successorTabId from left tabs.
  let successorTab = Tab.get(
    bluredTabs.find((tab) => tab.active)?.successorTabId
  );
  const scannedTabIds = new Set();
  while (successorTab && bluredTabIds.has(successorTab.id)) {
    if (scannedTabIds.has(successorTab.id)) break; // prevent infinite loop!
    scannedTabIds.add(successorTab.id);
    const nextSuccessorTab =
      successorTab.successorTabId > 0 &&
      successorTab.successorTabId !== successorTab.id
        ? Tab.get(successorTab.successorTabId)
        : null;
    if (!nextSuccessorTab) break;
  ...
  if (!successorTab || bluredTabIds.has(successorTab.id)) {
    if (successorTab) log(" => it cannot become the successor, find again");
    ...
    for (const tab of Tab.getVisibleTabs(windowId || bluredTabs[0].windowId)) {
      ...
      if (blured) bluredTabsFound = true;
      if (!bluredTabsFound) successorTab = tab;
      if (bluredTabsFound && !blured) {
        successorTab = tab;
        break;
      }
    }
  ...
  if (successorTab) await activateTab(successorTab, { silently });
  return successorTab;
}
...
export async function removeTabs(
  tabs,
  { keepDescendants, byMouseOperation, originalStructure, triggerTab } = {}
) {
  if (!Constants.IS_BACKGROUND)
    throw new Error(
      "TabsInternalOperation.removeTabs is available only on the background page, use a `kCOMMAND_REMOVE_TABS_INTERNALLY` message instead."
    );
  ...
  tabs = tabs.filter((tab) => {
    if (
      (!win || !win.internalClosingTabs.has(tab.id)) &&
      TabsStore.ensureLivingTab(tab)
    ) {
      tabIds.push(tab.id);
      if (tab.active) willChangeFocus = true;
      return true;
    }
    return false;
  });
  ...
  if (win) {
    ...
    for (const tab of tabs) {
      ...
      clearCache(tab);
      if (keepDescendants) win.keepDescendantsTabs.add(tab.id);
  ...
  const promisedRemoved = browser.tabs
    .remove(tabIds)
  ...
  if (win) {
    promisedRemoved.then(() => {
      ...
      const canceledTabs = new Set(
        tabs.filter((tab) => tab.$TST && !tab.$TST.destroyed)
      );
      ...
      for (const tab of canceledTabs) {
        ...
        if (keepDescendants) win.keepDescendantsTabs.delete(tab.id);
      }
      Tab.onMultipleTabsRemoved.dispatch(
        sortedTabs.filter((tab) => !canceledTabs.has(tab)),
        { triggerTab, originalStructure }
      );
    });
  }
...
export function clearCache(tab) {
  if (!tab) return;
  const errorHandler = ApiTabs.createErrorSuppressor(
...
export async function highlightTabs(
  tabs,
  { inheritToCollapsedDescendants } = {}
) {
  if (!Constants.IS_BACKGROUND)
    throw new Error(
      "TabsInternalOperation.highlightTabs is available only on the background page, use a `kCOMMAND_HIGHLIGHT_TABS` message instead."
    );

  if (!tabs || tabs.length === 0)
    throw new Error(
      "TabsInternalOperation.highlightTabs requires one or more tabs."
    );
  ...
  const indices = mapAndFilter(highlightedTabIds, (id) =>
    id === activeTab.id ? undefined : Tab.get(id).index
  );
  if (highlightedTabIds.has(activeTab.id)) indices.unshift(activeTab.index);
  ...
  while (highlightTabs.lastStartedAt === startAt) {
    count +=
      configs.provressiveHighlightingStep <= 0
        ? Number.MAX_SAFE_INTEGER
        : configs.provressiveHighlightingStep;
    await browser.tabs
      .highlight({
        windowId,
        populate: false,
    ...
    const progress = Math.ceil(
      (Math.min(indices.length, count) / indices.length) * 100
    );
    ...
    if (win.highlightingTabs.size < toBeHighlightedTabIds.size) {
      log(
        "highlightTabs: someone cleared multiselection while in-progress ",
        toBeHighlightedTabIds.size,
        win.highlightingTabs.size
      );
      break;
    }

    const unifiedHighlightTabIds = new Set([
      ...toBeHighlightedTabIds,
      ...win.highlightingTabs,
    ]);
    if (unifiedHighlightTabIds.size !== toBeHighlightedTabIds.size) {
      log(
        "highlightTabs: someone tried multiselection again while in-progress ",
        toBeHighlightedTabIds.size,
        win.highlightingTabs.size
      );
      break;
    ...
    case Constants.kCOMMAND_HIGHLIGHT_TABS:
      {
        ...
        highlightTabs(
          ...
          {
            inheritToCollapsedDescendants:
              message.inheritToCollapsedDescendants,
          }
        );
      }
      break;
