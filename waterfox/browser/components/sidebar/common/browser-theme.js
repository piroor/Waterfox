      ...
      switch (typeof theme[key]) {
        ...
        case "string":
          ...
          for (let alpha = 10; alpha < 100; alpha += 10) {
            rules.push(
              `--theme-${propertyKey}-${alpha}: ${Color.overrideCSSAlpha(value, alpha / 100)};`
            );
          }
          break;
      }
...
export async function generateThemeDeclarations(theme) {
  ...
  const extraColors = [];
  const themeFrameColor =
    theme.colors.frame || theme.colors.accentcolor /* old name */;
  const inactiveTextColor =
    theme.colors.tab_background_text || theme.colors.textcolor /* old name */;
  const activeTextColor =
    theme.colors.tab_text ||
    theme.colors.bookmark_text ||
    theme.colors.toolbar_text /* old name */ ||
    inactiveTextColor;
  let bgAlpha = 1;
  let hasImage = false;
  if (theme.images) {
    ...
    if (frameImage) {
      ...
      if (Color.isBrightColor(inactiveTextColor)) {
        // for bright text
        extraColors.push(
          "--browser-textshadow-for-header-image: 1px 1px 1.5px black"
        );
      } else {
        // for dark text
        extraColors.push(
          "--browser-textshadow-for-header-image: 0 0 1.5px white"
        );
      }
      images.push({
        url: frameImage,
    ...
    if (
      Array.isArray(theme.images.additional_backgrounds) &&
      theme.images.additional_backgrounds.length > 0
    ) {
      const leftImageCount = positions.filter((position) =>
        position.includes("left")
      ).length;
      const rightImageCount = positions.filter((position) =>
        position.includes("right")
      ).length;
      const repeatableImageCount = repeats.filter(
        (repeat) => repeat !== "no-repeat"
      ).length;
      for (
        let i = 0, maxi = theme.images.additional_backgrounds.length;
        i < maxi;
        i++
      ) {
        ...
        const position =
          (positions.length > 0 &&
            positions[Math.min(i, positions.length - 1)]) ||
          "default";
        const repeat =
          (repeats.length > 0 && repeats[Math.min(i, repeats.length - 1)]) ||
          "default";
        if (repeatableImageCount > 0 && repeat.includes("no-repeat")) continue;
        if (
          position &&
      ...
      images.map(async (image) => {
        ...
        try {
          const shouldRepeat =
            ...
            theme.properties.additional_backgrounds_tiling.some(
              (value) => value === "repeat" || value === "repeat-y"
            );
          ...
          if (!shouldRepeat && !shouldNoRepeat) {
            ...
            maybeRepeatable =
              loader.width / Math.max(1, loader.height) <=
              configs.unrepeatableBGImageAspectRatio;
          }
          if (shouldNoRepeat) image.size = "cover";
          else if (shouldRepeat || maybeRepeatable) image.size = "auto";
        } catch (error) {
          console.error(error);
        }
      })
    ...
    if (hasImage) {
      extraColors.push(
        "--browser-bg-images: " +
          images.map((image) => `url(${JSON.stringify(image.url)})`).join(",")
      );
      extraColors.push(
        "--browser-bg-position: " +
          images.map((image) => image.position).join(",")
      );
      extraColors.push(
        `--browser-bg-repeat: ${images.map((image) => image.repeat).join(",")}`
      );
      extraColors.push(
        `--browser-bg-size: ${images.map((image) => image.size).join(",")}`
      );
    }
  }
  const themeBaseColor = Color.overrideCSSAlpha(themeFrameColor, bgAlpha);
  let toolbarColor = Color.mixCSSColors(
    themeBaseColor,
  ...
  if (theme.colors.toolbar) {
    if (hasImage) {
      extraColors.push(
        `--browser-bg-for-header-image: ${theme.colors.toolbar};`
      );
    ...
    if (
      Color.isParsable(theme.colors.toolbar) &&
      Color.isParsable(theme.colors.toolbar_text)
    ) {
      const halfTransparentTextColor = Color.mixCSSColors(
        theme.colors.toolbar_text,
        theme.colors.toolbar_text,
        0.5
      );
      extraColors.push(
        `--browser-toolbar_text-darker: ${Color.mixCSSColors(theme.colors.toolbar, halfTransparentTextColor)}`
      );
    }
  } else if (hasImage) {
    extraColors.push(
      "--browser-bg-for-header-image: rgba(255, 255, 255, 0.25);"
    );
  }
  ...
  if (theme.colors.tab_loading)
    extraColors.push(
      `--browser-loading-indicator: ${theme.colors.tab_loading}`
    );
