...
const HEX_RRGGBBAA_MATCHER = new RegExp(
  `^#?(${DOUBLE_HEX})(${DOUBLE_HEX})(${DOUBLE_HEX})(${DOUBLE_HEX})?$`,
  "i"
);
const HEX_RGBA_MATCHER = new RegExp(
  `^#?(${HEX})(${HEX})(${HEX})(${HEX})?$`,
  "i"
);
const RGBA_MATCHER = new RegExp(
  `^rgba?\\(\\s*(${FLOAT_OR_PERCENTAGE})\\s*,?\\s*(${FLOAT_OR_PERCENTAGE})\\s*,?\\s*(${FLOAT_OR_PERCENTAGE})(?:\\s*[,/]?\\s*(${FLOAT_OR_PERCENTAGE})\\s*)?\\)$`,
  "i"
);
const HSLA_MATCHER = new RegExp(
  `^hsla?\\(\\s*(${FLOAT})(deg|rad|grad|turn)?\\s*,?\\s*(${FLOAT_OR_PERCENTAGE})\\s*,?\\s*(${FLOAT_OR_PERCENTAGE})(?:\\s*[,/]?\\s*(${FLOAT_OR_PERCENTAGE})\\s*)?\\)$`,
  "i"
);
...
export function isParsable(color) {
  ...
  return (
    HEX_RRGGBBAA_MATCHER.test(stringifiedColor) ||
...
function mixColors(base, over) {
  ...
  const red = Math.min(
    255,
    Math.round(base.red * (1 - alpha) + over.red * alpha)
  );
  const green = Math.min(
    255,
    Math.round(base.green * (1 - alpha) + over.green * alpha)
  );
  const blue = Math.min(
    255,
    Math.round(base.blue * (1 - alpha) + over.blue * alpha)
  );
...
export function isBrightColor(color) {
  ...
  const luminance =
    color.red * 0.2125 + color.green * 0.7154 + color.blue * 0.0721;
