...
import { configs, dumpTab, log as internalLogger } from "./common.js";
...
export default class Window {
  constructor(windowId) {
    ...
    this.preventToDetectTabBunchesUntil =
      Date.now() + configs.tabBunchesDetectionDelayOnNewWindow;
  ...
  getReversedOrderedTabs(startId, endId, tabs) {
    const orderedIds = this.sliceOrder(
      startId,
      endId,
      this.order.slice(0).reverse()
    );
    tabs = tabs || this.tabs;
    return function* () {
      for (const id of orderedIds) {
  ...
  sliceOrder(startId, endId, orderedIds) {
    if (!orderedIds) orderedIds = this.order;
    if (startId) {
      if (!this.tabs.has(startId)) return [];
      orderedIds = orderedIds.slice(orderedIds.indexOf(startId));
    }
    if (endId) {
      if (!this.tabs.has(endId)) return [];
  ...
  trackTab(tab) {
    ...
    if (alreadyTracked) tab = alreadyTracked;
    ...
    if (this.tabs.has(tab.id)) {
      ...
      for (
        let i = Math.min(index, tab.index),
          maxi = Math.min(Math.max(index, tab.index) + 1, order.length);
        i < maxi;
        i++
      ) {
        ...
        if (!tab)
          throw new Error(
            `Unknown tab: ${i}/${order[i]} (${order.join(", ")})`
          );
        tab.index = i;
      ...
      log(
        `tab ${dumpTab(tab)} is re-tracked under the window ${this.id}: `,
        prevState,
        index,
        "=>",
        tab.reindexedBy,
      ...
      log(
        `tab ${dumpTab(tab)} is newly tracked under the window ${this.id}: `,
        order
      );
    }
  ...
  detachTab(tabId) {
    ...
    const index = order.indexOf(tab.id);
    if (index < 0)
      // the tab is not tracked yet!
      return;
    order.splice(index, 1);
    if (this.tabs.size === 0) {
      if (!TabsStore.getCurrentWindowId()) {
        // only in the background page - the sidebar has no need to destroy itself manually.
        ...
        if (this.delayedDestroy) clearTimeout(this.delayedDestroy);
        this.delayedDestroy = setTimeout(
          () => {
            if (this.tabs && this.tabs.size === 0) this.destroy();
          },
