(() => {
  if (window.handleAccelKeyLoaded) return;

  function stringifySoloModifier(event) {
    switch (event.key) {
      case "Alt":
        return !event.ctrlKey && !event.metaKey ? "alt" : null;

      case "Control":
        return !event.altKey && !event.metaKey ? "control" : null;

      case "Meta":
        return !event.altKey && !event.ctrlKey ? "meta" : null;
  ...
  function stringifyModifier(event) {
    if (event.altKey && !event.ctrlKey && !event.metaKey) return "alt";

    if (!event.altKey && event.ctrlKey && !event.metaKey) return "control";

    if (!event.altKey && !event.ctrlKey && event.metaKey) return "meta";
  ...
  function stringifyMayTabSwitchModifier(event) {
    if (!/^(Tab|Shift|PageUp|PageDown)$/.test(event.key)) return null;
    return stringifyModifier(event);
  }
  ...
  function onKeyDown(event) {
    ...
    if (modifier || mayTabSwitchModifier)
  ...
  function onKeyUp(event) {
    ...
    const mayTabSwitchModifier = stringifyMayTabSwitchModifier(event);
    if (modifier || (!unshiftedModifier && !mayTabSwitchModifier))
      browser.runtime.sendMessage({
