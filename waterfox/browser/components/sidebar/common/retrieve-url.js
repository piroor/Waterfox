import { log as internalLogger } from "./common.js";
...
export async function fromDragEvent(event) {
  ...
  if (dt.files.length > 0) {
    ...
    for (const type of ACCEPTABLE_DATA_TYPES) {
      ...
      if (urlData) urls.push(...fromData(urlData, type));
      if (urls.length) break;
    }
    for (const type of dt.types) {
      if (!/^application\/x-ws-drag-data;(.+)$/.test(type)) continue;
      ...
      try {
        ...
        if (!dragData || typeof dragData !== "object") break;
        for (const type of ACCEPTABLE_DATA_TYPES) {
...
export async function fromClipboard({ selection } = {}) {
  ...
  if (
    !(await Permissions.isGranted(Permissions.CLIPBOARD_READ)) ||
  ...
  for (const item of clipboardContents) {
    for (const type of item.types) {
      ...
      if (maybeUrlString) urls.push(...fromData(maybeUrlString, type));
      if (urls.length) break;
    }
  }
  log(" => retrieved: ", urls);
...
function sanitizeURLs(urls) {
  const filteredUrls = urls.filter(
    (url) =>
      (url?.length && url.indexOf(kBOOKMARK_FOLDER) === 0) ||
...
function fixupURIFromText(maybeURI) {
  if (/^(ext\+)?\w+:/.test(maybeURI)) return maybeURI;

  if (/^([^.\s]+\.)+[^.\s]{2}/.test(maybeURI)) return `http://${maybeURI}`;
