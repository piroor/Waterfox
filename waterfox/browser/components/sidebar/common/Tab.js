...
export default class Tab {
  constructor(tab) {
    ...
    if (alreadyTracked) return alreadyTracked.$TST;
    ...
    const incompletelyTrackedTabsPerWindow =
      mIncompletelyTrackedTabs.get(tab.windowId) || new Set();
    incompletelyTrackedTabsPerWindow.add(tab);
    mIncompletelyTrackedTabs.set(
      tab.windowId,
      incompletelyTrackedTabsPerWindow
    );
  ...
  destroy() {
    ...
    if (this.temporaryMetadata.has("reservedCleanupNeedlessGroupTab")) {
      clearTimeout(
        this.temporaryMetadata.get("reservedCleanupNeedlessGroupTab")
      );
      this.temporaryMetadata.delete("reservedCleanupNeedlessGroupTab");
    }

    TabsStore.tabs.delete(this.id);
    if (this.uniqueId) TabsStore.tabsByUniqueId.delete(this.uniqueId.id);
  ...
  startMoving() {
    if (!this.tab) return Promise.resolve();
    let onTabMoved;
  ...
  updateUniqueId(options = {}) {
    if (!this.tab) {
      const error = new Error(
        "FATAL ERROR: updateUniqueId() is unavailable for an invalid tab"
      );
      console.log(error);
      throw error;
    }
    if (options.id) {
      if (this.uniqueId.id) TabsStore.tabsByUniqueId.delete(this.uniqueId.id);
    ...
    return UniqueId.request(this.tab, options)
      .then((uniqueId) => {
        if (uniqueId && TabsStore.ensureLivingTab(this.tab)) {
          // possibly removed from document while waiting
      ...
      .catch((error) => {
        console.log(
          `FATAL ERROR: Failed to get unique id for a tab ${this.id}: `,
          error
        );
        return {};
      });
  ...
  get maybeMuted() {
    return (
      this.muted ||
      (this.states.has(Constants.kTAB_STATE_HAS_MUTED_MEMBER) && this.hasChild)
    );
  }
  ...
  get sharingCamera() {
    return !!this.tab?.sharingState?.camera;
  }
  ...
  get sharingMicrophone() {
    return !!this.tab?.sharingState?.microphone;
  }
  ...
  get sharingScreen() {
    return !!this.tab?.sharingState?.screen;
  }
  ...
  get isSubtreeCollapsable() {
    return this.hasChild && !this.collapsed && !this.subtreeCollapsed;
  }
  ...
  get stuck() {
    return this.element?.parentNode?.classList.contains(
      "sticky-tabs-container"
    );
  }

  get isNewTabCommandTab() {
    if (!this.tab || !configs.guessNewOrphanTabAsOpenedByNewTabCommand)
      return false;

    if (this.tab.$isNewTabCommandTab) return true;
    ...
    if (isNewTabCommandTab(this.tab)) return true;
  ...
  get isGroupTab() {
    return (
      this.states.has(Constants.kTAB_STATE_GROUP_TAB) || this.hasGroupTabURL
    );
  }
  ...
  get isTemporaryGroupTab() {
    if (!this.tab || !this.isGroupTab) return false;
  ...
  get isTemporaryAggressiveGroupTab() {
    if (!this.tab || !this.isGroupTab) return false;
    return (
      new URL(this.tab.url).searchParams.get("temporaryAggressive") === "true"
    );
  }

  get replacedParentGroupTabCount() {
    if (!this.tab || !this.isGroupTab) return 0;
    const count = parseInt(
      new URL(this.tab.url).searchParams.get("replacedParentCount")
    );
  ...
  get mayBeReplacedWithContainer() {
    return !!(
      this.$possiblePredecessorPreviousTab || this.$possiblePredecessorNextTab
    );
  }
  ...
  get selected() {
    return (
      this.states.has(Constants.kTAB_STATE_SELECTED) ||
      (this.hasOtherHighlighted && !!this.tab?.highlighted)
    );
  }
  ...
  get hasOtherHighlighted() {
    const highlightedTabs =
      this.tab && TabsStore.highlightedTabsInWindow.get(this.tab.windowId);
  ...
  get canBecomeSticky() {
    ...
    for (const states of Tab.autoStickyStates.values()) {
      if (
        new Set([...this.states, ...states]).size <
        this.states.size + states.size
      )
        return true;
    }
  ...
  get promisedPossibleOpenerBookmarks() {
    ...
    return new Promise(async (resolve, _reject) => {
      ...
      try {
        ...
        log(
          `promisedPossibleOpenerBookmarks for tab ${this.id} (${url}): `,
          possibleBookmarks
        );
        resolve((this.possibleOpenerBookmarks = possibleBookmarks.flat()));
      } catch (error) {
        log(
          `promisedPossibleOpenerBookmarks for the tab {this.id} (${url}): `,
          error
        );
  ...
  async _safeSearchBookmstksWithUrl(url) {
    try {
      ...
      try {
        ...
        const bookmarks = await browser.bookmarks
          .search({ query: url })
          .catch((_error) => []);
  ...
  get cookieStoreName() {
    const identity =
      this.tab?.cookieStoreId &&
      ContextualIdentities.get(this.tab.cookieStoreId);
    return identity ? identity.name : null;
  }

  generateTooltipText() {
    return this.cookieStoreName
      ? `${this.tab.title} - ${this.cookieStoreName}`
      : this.tab.title;
  }
  ...
  generateTooltipTextWithDescendants() {
    ...
    for (const child of this.children) {
      if (!child) continue;
      tooltip.push(
        child.$TST.generateTooltipTextWithDescendants().replace(/^/gm, "  ")
      );
    }
    return tooltip.join("\n");
  }
  ...
  registerTooltipText(ownerId, text, isHighPriority = false) {
    ...
    if (Constants.IS_BACKGROUND) Tab.broadcastTooltipText(this.tab);
  }
  ...
  getHighPriorityTooltipText() {
    if (this.highPriorityTooltipTexts.size === 0) return null;
    return [...this.highPriorityTooltipTexts.values()][
      this.highPriorityTooltipTexts.size - 1
    ];
  }

  getLowPriorityTooltipText() {
    if (this.lowPriorityTooltipTexts.size === 0) return null;
    return [...this.lowPriorityTooltipTexts.values()][
      this.lowPriorityTooltipTexts.size - 1
    ];
  }
  ...
  get nearestVisibleFollowingTab() {
  ...
  get unsafeNearestExpandedFollowingTab() {
    // not-collapsed, possibly hidden
    return (
      this.tab &&
      TabsStore.query({
        windowId: this.tab.windowId,
  ...
  get nearestVisiblePrecedingTab() {
    // visible, not-collapsed
    return (
      this.tab &&
      TabsStore.query({
  ...
  get unsafeNearestExpandedPrecedingTab() {
    // not-collapsed, possibly hidden
    return (
      this.tab &&
      TabsStore.query({
  ...
  get nearestLoadedTab() {
    const tabs =
      this.tab && TabsStore.visibleTabsInWindow.get(this.tab.windowId);
    return (
      this.tab &&
  ...
  get nearestLoadedTabInTree() {
    ...
    while (tab) {
      ...
      if (loadedTab) return loadedTab;
      if (!parent.discarded) return parent;
  ...
  get nearestLoadedSiblingTab() {
    ...
    if (!parent || !this.tab) return null;
  ...
  get nearestSameTypeRenderedTab() {
    ...
    while (tab.$TST.unsafeNextTab) {
      ...
      if (tab.pinned !== pinned) return null;
      if (tab.$TST.element?.parentNode) return tab;
    }
    return null;
  }
  ...
  set parent(tab) {
    ...
    if (!this.tab || newParentId === this.parentId) return tab;
    ...
    if (oldParent && oldParent.id !== this.parentId) {
      ...
      oldParent.$TST.children = oldParent.$TST.childIds.filter(
        (id) => id !== this.id
      );
    }
    return tab;
  ...
  get parent() {
    return (
      this.tab &&
      this.parentId &&
      TabsStore.ensureLivingTab(Tab.get(this.parentId))
    );
  }
  ...
  get ancestorIds() {
    if (!this.cachedAncestorIds) this.updateAncestors();
  ...
  get nearestVisibleAncestorOrSelf() {
    for (const ancestor of this.ancestors) {
      if (!ancestor.$TST.collapsed) return ancestor;
    }
  ...
  set children(tabs) {
    ...
    const newChildIds = mapAndFilter(tabs, (tab) => {
      ...
      console.log(
        "FATAL ERROR: Cyclic tree structure has detected and prevented. ",
        {
    ...
    if (newChildIds.join("|") === this.childIds.join("|")) return tabs;
    ...
    for (const child of Array.from(
      new Set(this.children.concat(oldChildren))
    )) {
      if (this.childIds.includes(child.id)) child.$TST.parent = this.id;
      else child.$TST.parent = null;
    }
    return tabs;
  }
  get children() {
  ...
  get descendants() {
    if (!this.cachedDescendantIds) return this.updateDescendants();
    return mapAndFilter(
  ...
  updateDescendants() {
    ...
    for (const child of this.children) {
      ...
      this.cachedDescendantIds = this.cachedDescendantIds.concat(
        child.$TST.cachedDescendantIds
      );
    }
    return descendants;
  }
  ...
  invalidateCachedDescendants() {
    this.cachedDescendantIds = null;
    const parent = this.parent;
    if (parent) parent.$TST.invalidateCachedDescendants();
    this.invalidateCache();
  }
  ...
  get nextSiblingTab() {
    ...
    if (parent) {
      ...
      const siblingId =
        index < siblingIds.length - 1 ? siblingIds[index + 1] : null;
      ...
      if (nextSibling && nextSibling.pinned !== this.tab.pinned) return null;
      return nextSibling;
    }
  }
  ...
  get needToBeGroupedSiblings() {
    if (!this.tab) return [];
    const openerTabUniqueId = this.getAttribute(
      Constants.kPERSISTENT_ORIGINAL_OPENER_TAB_ID
    );
    if (!openerTabUniqueId) return [];
  ...
  get openerTab() {
    if (this.tab?.openerTabId === this.id) return null;

    if (!this.tab || !this.tab.openerTabId)
  ...
  get bundledTab() {
    ...
    if (this.tab.pinned) return Tab.getGroupTabForOpener(this.tab);
    if (this.isGroupTab) return Tab.getOpenerFromGroupTab(this.tab);
    return null;
  }
  ...
  set lastRelatedTab(relatedTab) {
    ...
    if (relatedTab) {
      ...
      successorTabLog(
        `set lastRelatedTab for ${this.id}: ${previousLastRelatedTabId} => ${relatedTab.id} (${this.newRelatedTabsCount})`
      );
    } else {
      win.lastRelatedTabs.delete(this.id);
      this.newRelatedTabsCount = 0;
      successorTabLog(
        `clear lastRelatedTab for ${this.id} (${previousLastRelatedTabId})`
      );
    }
  ...
  isAllPlacedBeforeSelf(tabs) {
    if (!this.tab || tabs.length === 0) return true;
    let nextTab = this.tab;
    if (tabs[tabs.length - 1] === nextTab) nextTab = nextTab.$TST.unsafeNextTab;
    if (!nextTab && !tabs[tabs.length - 1].$TST.unsafeNextTab) return true;

    tabs = Array.from(tabs);
    let previousTab = tabs.shift();
    for (const tab of tabs) {
      if (tab.$TST.unsafePreviousTab !== previousTab) return false;
      previousTab = tab;
    }
    return (
      !nextTab || !previousTab || previousTab.$TST.unsafeNextTab === nextTab
    );
  }
  ...
  isAllPlacedAfterSelf(tabs) {
    ...
    if (!previousTab && !tabs[0].$TST.unsafePreviousTab) return true;

    tabs = Array.from(tabs).reverse();
    let nextTab = tabs.shift();
    for (const tab of tabs) {
      if (tab.$TST.unsafeNextTab !== nextTab) return false;
      nextTab = tab;
    }
    return (
      !previousTab || !nextTab || nextTab.$TST.unsafePreviousTab === previousTab
    );
  }
  ...
  async toggleState(state, condition, { permanently, toTab, broadcast } = {}) {
    ...
    else return this.removeState(state, { permanently, toTab, broadcast });
  }
  ...
  async addState(state, { permanently, toTab, broadcast } = {}) {
    ...
    if (this.classList) this.classList.add(state);
    if (this.states) this.states.add(state);

    switch (state) {
      case Constants.kTAB_STATE_HIGHLIGHTED:
        TabsStore.addHighlightedTab(this.tab);
        if (this.element) this.element.setAttribute("aria-selected", "true");
        if (toTab) this.tab.highlighted = true;
        break;
      ...
      case Constants.kTAB_STATE_SOUND_PLAYING:
        {
          const parent = this.parent;
          if (parent) parent.$TST.soundPlayingChildrenIds.add(this.id);
        }
        break;
      case Constants.kTAB_STATE_HAS_SOUND_PLAYING_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeSoundPlayingChildrenIds.add(this.id);
        }
        break;

      case Constants.kTAB_STATE_AUDIBLE:
        if (toTab) this.tab.audible = true;
        break;

      case Constants.kTAB_STATE_MUTED:
        {
          const parent = this.parent;
          if (parent) parent.$TST.mutedChildrenIds.add(this.id);
          if (toTab) this.tab.mutedInfo.muted = true;
        }
        break;
      case Constants.kTAB_STATE_HAS_MUTED_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeMutedChildrenIds.add(this.id);
        }
        break;
      ...
      case Constants.kTAB_STATE_SHARING_CAMERA:
        {
          const parent = this.parent;
          if (parent) parent.$TST.sharingCameraChildrenIds.add(this.id);
      ...
      case Constants.kTAB_STATE_HAS_SHARING_CAMERA_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeSharingCameraChildrenIds.add(this.id);
        }
        break;
      ...
      case Constants.kTAB_STATE_SHARING_MICROPHONE:
        {
          const parent = this.parent;
          if (parent) parent.$TST.sharingMicrophoneChildrenIds.add(this.id);
          if (toTab && this.tab.sharingState)
      ...
      case Constants.kTAB_STATE_SHARING_SCREEN:
        {
          const parent = this.parent;
          if (parent) parent.$TST.sharingScreenChildrenIds.add(this.id);
          if (toTab && this.tab.sharingState)
      ...
      case Constants.kTAB_STATE_HAS_SHARING_SCREEN_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeSharingScreenChildrenIds.add(this.id);
        }
        break;
      ...
      case Constants.kTAB_STATE_PRIVATE_BROWSING:
        if (toTab) this.tab.incognito = true;
        break;

      case Constants.kTAB_STATE_ATTENTION:
        if (toTab) this.tab.attention = true;
        break;

      case Constants.kTAB_STATE_DISCARDED:
        if (toTab) this.tab.discarded = true;
        break;

      case "loading":
        TabsStore.addLoadingTab(this.tab);
        if (toTab) this.tab.status = state;
        break;
    ...
    if (this.tab && modified && Constants.IS_BACKGROUND && broadcast !== false)
    ...
    if (permanently) {
      ...
      if (!states.includes(state)) {
        ...
        await browser.sessions
          .setTabValue(this.id, Constants.kPERSISTENT_STATES, states)
          .catch(ApiTabs.createErrorSuppressor());
      }
    }
    if (modified) {
      this.invalidateCache();
      if (this.tab) Tab.onStateChanged.dispatch(this.tab, state, true);
    }
  }
  ...
  async removeState(state, { permanently, toTab, broadcast } = {}) {
    ...
    if (this.classList) this.classList.remove(state);
    if (this.states) this.states.delete(state);

    switch (state) {
      case Constants.kTAB_STATE_HIGHLIGHTED:
        TabsStore.removeHighlightedTab(this.tab);
        if (this.element) this.element.setAttribute("aria-selected", "false");
        if (toTab) this.tab.highlighted = false;
        break;
      ...
      case Constants.kTAB_STATE_HIDDEN:
        if (!this.collapsed) TabsStore.addVisibleTab(this.tab);
        TabsStore.addControllableTab(this.tab);
        if (toTab) this.tab.hidden = false;
        break;

      case Constants.kTAB_STATE_PINNED:
        TabsStore.removePinnedTab(this.tab);
        TabsStore.addUnpinnedTab(this.tab);
        if (toTab) this.tab.pinned = false;
        break;
      ...
      case Constants.kTAB_STATE_SOUND_PLAYING:
        {
          const parent = this.parent;
          if (parent) parent.$TST.soundPlayingChildrenIds.delete(this.id);
        }
        break;
      case Constants.kTAB_STATE_HAS_SOUND_PLAYING_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeSoundPlayingChildrenIds.delete(this.id);
        }
        break;

      case Constants.kTAB_STATE_AUDIBLE:
        if (toTab) this.tab.audible = false;
        break;

      case Constants.kTAB_STATE_MUTED:
        {
          const parent = this.parent;
          if (parent) parent.$TST.mutedChildrenIds.delete(this.id);
          if (toTab) this.tab.mutedInfo.muted = false;
        }
        break;
      case Constants.kTAB_STATE_HAS_MUTED_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeMutedChildrenIds.delete(this.id);
        }
        break;
      ...
      case Constants.kTAB_STATE_SHARING_CAMERA:
        {
          const parent = this.parent;
          if (parent) parent.$TST.sharingCameraChildrenIds.delete(this.id);
          if (toTab && this.tab.sharingState)
      ...
      case Constants.kTAB_STATE_HAS_SHARING_CAMERA_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeSharingCameraChildrenIds.delete(this.id);
        }
        break;
      ...
      case Constants.kTAB_STATE_SHARING_MICROPHONE:
        {
          const parent = this.parent;
          if (parent) parent.$TST.sharingMicrophoneChildrenIds.delete(this.id);
          if (toTab && this.tab.sharingState)
      ...
      case Constants.kTAB_STATE_SHARING_SCREEN:
        {
          const parent = this.parent;
          if (parent) parent.$TST.sharingScreenChildrenIds.delete(this.id);
          if (toTab && this.tab.sharingState)
      ...
      case Constants.kTAB_STATE_HAS_SHARING_SCREEN_MEMBER:
        {
          const parent = this.parent;
          if (parent) parent.$TST.maybeSharingScreenChildrenIds.delete(this.id);
        }
        break;
      ...
      case Constants.kTAB_STATE_PRIVATE_BROWSING:
        if (toTab) this.tab.incognito = false;
        break;

      case Constants.kTAB_STATE_ATTENTION:
        if (toTab) this.tab.attention = false;
        break;

      case Constants.kTAB_STATE_DISCARDED:
        if (toTab) this.tab.discarded = false;
        break;
    }

    if (modified && Constants.IS_BACKGROUND && broadcast !== false)
      Tab.broadcastState(this.tab, {
        remove: [state],
      });
    if (permanently) {
  ...
  async getPermanentStates() {
    const states =
      this.tab &&
      (await browser.sessions
        .getTabValue(this.id, Constants.kPERSISTENT_STATES)
  ...
  inheritSoundStateFromChildren() {
    ...
    this.delayedInheritSoundStateFromChildren = setTimeout(() => {
      this.delayedInheritSoundStateFromChildren = null;
      if (!TabsStore.ensureLivingTab(this.tab)) return;

      const parent = this.parent;
      let modifiedCount = 0;

      const soundPlayingCount =
        this.soundPlayingChildrenIds.size +
        this.maybeSoundPlayingChildrenIds.size;
      if (soundPlayingCount !== this.lastSoundStateCounts.soundPlaying) {
        this.lastSoundStateCounts.soundPlaying = soundPlayingCount;
        this.toggleState(
          Constants.kTAB_STATE_HAS_SOUND_PLAYING_MEMBER,
          soundPlayingCount > 0
        );
        if (parent) {
          if (soundPlayingCount > 0)
            parent.$TST.maybeSoundPlayingChildrenIds.add(this.id);
          else parent.$TST.maybeSoundPlayingChildrenIds.delete(this.id);
        }
        modifiedCount++;
      }

      const mutedCount =
        this.mutedChildrenIds.size + this.maybeMutedChildrenIds.size;
      if (mutedCount !== this.lastSoundStateCounts.muted) {
        ...
        if (parent) {
          if (mutedCount > 0) parent.$TST.maybeMutedChildrenIds.add(this.id);
          else parent.$TST.maybeMutedChildrenIds.delete(this.id);
        }
        modifiedCount++;
      }

      const autoplayBlockedCount =
        this.autoplayBlockedChildrenIds.size +
        this.maybeAutoplayBlockedChildrenIds.size;
      if (autoplayBlockedCount !== this.lastSoundStateCounts.autoplayBlocked) {
        ...
        this.toggleState(
          Constants.kTAB_STATE_HAS_AUTOPLAY_BLOCKED_MEMBER,
          autoplayBlockedCount > 0
        );
        if (parent) {
          if (autoplayBlockedCount > 0)
            parent.$TST.maybeAutoplayBlockedChildrenIds.add(this.id);
          else parent.$TST.maybeAutoplayBlockedChildrenIds.delete(this.id);
        }
        modifiedCount++;
      }
      ...
      if (parent) parent.$TST.inheritSoundStateFromChildren();

      SidebarConnection.sendMessage({
        type: Constants.kCOMMAND_NOTIFY_TAB_SOUND_STATE_UPDATED,
        windowId: this.tab.windowId,
        tabId: this.id,
        hasSoundPlayingMember: this.states.has(
          Constants.kTAB_STATE_HAS_SOUND_PLAYING_MEMBER
        ),
        hasMutedMember: this.states.has(Constants.kTAB_STATE_HAS_MUTED_MEMBER),
        hasAutoplayBlockedMember: this.states.has(
          Constants.kTAB_STATE_HAS_AUTOPLAY_BLOCKED_MEMBER
        ),
      });
    }, 100);
  ...
  inheritSharingStateFromChildren() {
    ...
    this.delayedInheritSharingStateFromChildren = setTimeout(() => {
      ...
      const sharingCameraCount =
        this.sharingCameraChildrenIds.size +
        this.maybeSharingCameraChildrenIds.size;
      if (sharingCameraCount !== this.lastSharingStateCounts.sharingCamera) {
        ...
        this.toggleState(
          Constants.kTAB_STATE_HAS_SHARING_CAMERA_MEMBER,
          sharingCameraCount > 0
        );
        if (parent) {
          if (sharingCameraCount > 0)
            parent.$TST.maybeSharingCameraChildrenIds.add(this.id);
          else parent.$TST.maybeSharingCameraChildrenIds.delete(this.id);
        }
        modifiedCount++;
      }

      const sharingMicrophoneCount =
        this.sharingMicrophoneChildrenIds.size +
        this.maybeSharingMicrophoneChildrenIds.size;
      if (
        sharingMicrophoneCount !== this.lastSharingStateCounts.sharingMicrophone
      ) {
        this.lastSharingStateCounts.sharingMicrophone = sharingMicrophoneCount;
        this.toggleState(
          Constants.kTAB_STATE_HAS_SHARING_MICROPHONE_MEMBER,
          sharingMicrophoneCount > 0
        );
        if (parent) {
          if (sharingMicrophoneCount > 0)
            parent.$TST.maybeSharingMicrophoneChildrenIds.add(this.id);
          else parent.$TST.maybeSharingMicrophoneChildrenIds.delete(this.id);
        }
        modifiedCount++;
      }

      const sharingScreenCount =
        this.sharingScreenChildrenIds.size +
        this.maybeSharingScreenChildrenIds.size;
      if (sharingScreenCount !== this.lastSharingStateCounts.sharingScreen) {
        ...
        this.toggleState(
          Constants.kTAB_STATE_HAS_SHARING_SCREEN_MEMBER,
          sharingScreenCount > 0
        );
        if (parent) {
          if (sharingScreenCount > 0)
            parent.$TST.maybeSharingScreenChildrenIds.add(this.id);
          else parent.$TST.maybeSharingScreenChildrenIds.delete(this.id);
        }
        modifiedCount++;
      }
      ...
      if (parent) parent.$TST.inheritSharingStateFromChildren();

      SidebarConnection.sendMessage({
        type: Constants.kCOMMAND_NOTIFY_TAB_SHARING_STATE_UPDATED,
        windowId: this.tab.windowId,
        tabId: this.id,
        hasSharingCameraMember: this.states.has(
          Constants.kTAB_STATE_HAS_SHARING_CAMERA_MEMBER
        ),
        hasSharingMicrophoneMember: this.states.has(
          Constants.kTAB_STATE_HAS_SHARING_MICROPHONE_MEMBER
        ),
        hasSharingScreenMember: this.states.has(
          Constants.kTAB_STATE_HAS_SHARING_SCREEN_MEMBER
        ),
      });
    }, 100);
  ...
  resolveOpened() {
    if (!mOpenedResolvers.has(this.id)) return;
  ...
  get movedInBulk() {
    ...
    if (this.lastPreviousTabId && this.lastPreviousTabId !== previousTab?.id) {
      log(
        `not bulkMoved lastPreviousTabId=${this.lastNextTabId}, previousTab=${previousTab?.id}`
      );
      return false;
    }

    const nextTab = this.unsafeNextTab;
    if (this.lastNextTabId && this.lastNextTabId !== nextTab?.id) {
      log(
        `not bulkMoved lastNextTabId=${this.lastNextTabId}, nextTab=${nextTab?.id}`
      );
      return false;
    }
  ...
  apply(exported) {
    ...
    for (const key of Object.keys(exported)) {
      if (key === "$TST") continue;
      if (key in this.tab) this.tab[key] = exported[key];
    }
  ...
  async exportForAPI({
    ...
    let exportedTab =
      configs.cacheAPITreeItems && light ? this.$exportedForAPI : null;
    let favIconUrl;
    if (!exportedTab) {
      ...
      exportedTab = {
        id: this.tab.id,
        windowId: this.tab.windowId,
        states: Constants.kTAB_SAFE_STATES_ARRAY.filter((state) =>
          tabStates.has(state)
        ),
      ...
      if (this.stuck) exportedTab.states.push(Constants.kTAB_STATE_STUCK);
      if (configs.cacheAPITreeItems && light)
        this.$exportedForAPI = exportedTab;
    }

    if (light) return exportedTab;
    ...
    if (
      ...
      (permissions.has(kPERMISSION_ACTIVE_TAB) &&
        (this.tab.active || (this.tab === this.tab && this.isContextTab)))
    ) {
    ...
    for (const property of allowedProperties) {
      if (property in this.tab) fullExportedTab[property] = this.tab[property];
    }
  ...
  invalidateElement(targets) {
    if (this.element?.invalidate) this.element.invalidate(targets);
  }

  updateElement(targets) {
    if (this.element?.update) this.element.update(targets);
  }
...
Tab.track = (tab) => {
  ...
  if (!trackedTab || !(tab.$TST instanceof Tab)) {
    new Tab(tab);
  } else {
...
Tab.needToWaitTracked = (windowId) => {
  ...
  for (const tabs of mIncompletelyTrackedTabs.values()) {
    if (tabs && tabs.size > 0) return true;
  }
  return false;
};
...
function onWaitingTabDestroyed(tab) {
  ...
  for (const task of tasks) {
    ...
    log(
      `Tab.waitUntilTracked: ${tab.id} is destroyed while waiting (in ${scope})\n${stack}`
    );
    resolve(null);
  }
}
...
function onWaitingTabRemoved(removedTabId, _removeInfo) {
  ...
  for (const task of tasks) {
    ...
    log(
      `Tab.waitUntilTracked: ${removedTabId} is removed while waiting (in ${scope})\n${stack}`
    );
    resolve(null);
  }
}
...
async function waitUntilTracked(tabId, options = {}) {
  ...
  if (tab) {
    ...
    if (options.element) return tab.$TST.promisedElement;
    return tab;
  }
  ...
  return new Promise((resolve, _reject) => {
    ...
    task.timeout = setTimeout(() => {
      ...
      if (resolve) {
        log(
          `Tab.waitUntilTracked for ${tabId} is timed out (in ${TabsStore.getCurrentWindowId() || "bg"})\b${stack}`
        );
        resolve(null);
      }
    ...
    browser.tabs
      ...
      .then((tab) => {
        if (tab) {
          if (Tab.get(tabId)) onWaitingTabTracked(tab);
          return;
        }
        const { resolve } = destroyWaitingTabTask(task);
        if (resolve) {
...
Tab.waitUntilTracked = async (tabId, options = {}) => {
  ...
  if (windowId) {
    ...
    if (tabs?.has(tabId)) return null; // already removed tab
  }

  const key = `${tabId}:${!!options.element}`;
  if (mPromisedTrackedTabs.has(key)) return mPromisedTrackedTabs.get(key);
...
Tab.getFirstVisibleTab = (windowId) => {
  // visible, not-collapsed, not-hidden
  return TabsStore.query({
    windowId,
...
Tab.getLastVisibleTab = (windowId) => {
  // visible, not-collapsed, not-hidden
  return TabsStore.query({
    windowId,
    tabs: TabsStore.visibleTabsInWindow.get(windowId),
...
Tab.getLastOpenedTab = (windowId) => {
  ...
  return tabs.length > 0 ? tabs.sort((a, b) => b.id - a.id)[0] : null;
};
...
Tab.getFirstUnpinnedTab = (windowId) => {
  // not-pinned
  return TabsStore.query({
    windowId,
...
Tab.getLastUnpinnedTab = (windowId) => {
  // not-pinned
  return TabsStore.query({
    windowId,
    tabs: TabsStore.unpinnedTabsInWindow.get(windowId),
...
Tab.getFirstNormalTab = (windowId) => {
  // visible, not-collapsed, not-pinned
  return TabsStore.query({
    windowId,
    tabs: TabsStore.unpinnedTabsInWindow.get(windowId),
    normal: true,
...
Tab.getGroupTabForOpener = (opener) => {
  ...
  if (!groupTab || groupTab === opener || groupTab.pinned === opener.pinned)
    return null;
  return groupTab;
};

Tab.getOpenerFromGroupTab = (groupTab) => {
  if (!groupTab.$TST.isGroupTab) return null;
...
Tab.getSubstanceFromAliasGroupTab = (groupTab) => {
  ...
  if (!aliasTab || aliasTab === groupTab || aliasTab.pinned === groupTab.pinned)
    return null;
  return aliasTab;
};
...
Tab.getActiveTabs = () => {
  return Array.from(
    TabsStore.activeTabInWindow.values(),
    TabsStore.ensureLivingTab
  );
};
...
Tab.getTabs = (windowId = null, options = {}) => {
  // only visible, including collapsed and pinned
  return TabsStore.queryAll({
    windowId,
...
Tab.getTabsBetween = (begin, end) => {
  if (
    !begin ||
    !TabsStore.ensureLivingTab(begin) ||
    !end ||
    !TabsStore.ensureLivingTab(end)
  )
  ...
  if (begin === end) return [];
  if (begin.index > end.index) [begin, end] = [end, begin];
  return TabsStore.queryAll({
    windowId: begin.windowId,
    tabs: TabsStore.getTabsMap(
      TabsStore.controllableTabsInWindow,
      begin.windowId
    ),
...
Tab.getNormalTabs = (windowId = null, options = {}) => {
  // only visible, including collapsed, not pinned
  return TabsStore.queryAll({
    windowId,
...
Tab.getVisibleTabs = (windowId = null, options = {}) => {
  // visible, not-collapsed, not-hidden
  return TabsStore.queryAll({
    windowId,
...
Tab.getPinnedTabs = (windowId = null, options = {}) => {
  // visible, pinned
  return TabsStore.queryAll({
    windowId,
...
Tab.getUnpinnedTabs = (windowId = null, options = {}) => {
  // visible, not pinned
  return TabsStore.queryAll({
    windowId,
    tabs: TabsStore.getTabsMap(TabsStore.unpinnedTabsInWindow, windowId),
...
Tab.collectRootTabs = (tabs) => {
  return tabs.filter((tab) => {
    if (!TabsStore.ensureLivingTab(tab)) return false;
    const parent = tab.$TST.parent;
...
Tab.getSubtreeCollapsedTabs = (windowId = null, options = {}) => {
  return TabsStore.queryAll({
    windowId,
    tabs: TabsStore.getTabsMap(
      TabsStore.subtreeCollapsableTabsInWindow,
      windowId
    ),
    living: true,
    hidden: false,
...
Tab.getSelectedTabs = (windowId = null, options = {}) => {
  ...
  const highlightedTabs = TabsStore.getTabsMap(
    TabsStore.highlightedTabsInWindow,
    windowId
  );
  if (!highlightedTabs || highlightedTabs.size < 2) return selectedTabs;
  ...
  if (options.iterator)
    return (function* () {
      ...
      for (const tab of highlightedTabs.values()) {
        if (!alreadyReturnedTabs.has(tab)) yield tab;
      }
    })();
  else
    return Tab.sort(
      Array.from(
        new Set([...selectedTabs, ...Array.from(highlightedTabs.values())])
      )
    );
};
...
Tab.getVirtualScrollRenderableTabs = (windowId = null) => {
  return TabsStore.queryAll({
    windowId,
    tabs: TabsStore.getTabsMap(
      TabsStore.virtualScrollRenderableTabsInWindow,
      windowId
    ),
    skipMatching: true,
    ordered: true,
...
Tab.hasDuplicatedTabs = (windowId, options = {}) => {
  ...
  for (const tab of tabs) {
    ...
    if (tabKeys.has(key)) return true;
    tabKeys.add(key);
  }
  return false;
};
...
Tab.getRecycledTabs = (windowId = null, options = {}) => {
  const userNewTabUrls = configs.guessNewOrphanTabAsOpenedByNewTabCommandUrl
    .split("|")
    .map((part) => sanitizeForRegExpSource(part.trim()))
    .join("|");
  return TabsStore.queryAll({
    windowId,
    tabs: TabsStore.getTabsMap(TabsStore.livingTabsInWindow, windowId),
    ...
    attributes: [
      Constants.kCURRENT_URI,
      new RegExp(
        `^(|${userNewTabUrls}|about:newtab|about:blank|about:privatebrowsing)$`
      ),
    ],
    ...options,
  });
...
Tab.broadcastTooltipText = (tabs) => {
  if (!Constants.IS_BACKGROUND || !Tab.broadcastTooltipText.enabled) return;
  ...
  (Constants.IS_BACKGROUND
    ...
    : // because window.requestAnimationFrame is decelerate for an invisible document.
      window.requestAnimationFrame)(() => {
    if (Tab.broadcastTooltipText.triedAt !== triedAt) return;
...
Tab.broadcastState = (tabs, { add, remove } = {}) => {
  if (!Constants.IS_BACKGROUND || !Tab.broadcastState.enabled) return;
  ...
  (Constants.IS_BACKGROUND
    ...
    : // because window.requestAnimationFrame is decelerate for an invisible document.
      window.requestAnimationFrame)(() => {
    if (Tab.broadcastState.triedAt !== triedAt) return;
    ...
    for (const message of Tab.bufferedStatesChanges.values()) {
      ...
      if (key !== lastKey) {
        if (unifiedMessage) unifiedMessages.push(unifiedMessage);
        unifiedMessage = null;
      }
      lastKey = key;
...
Tab.calculateNewTabIndex = ({ insertAfter, insertBefore, ignoreTabs } = {}) => {
  ...
  // should be.)
  if (insertAfter) return getTabIndex(insertAfter, { ignoreTabs }) + 1;
  if (insertBefore) return getTabIndex(insertBefore, { ignoreTabs });
  return -1;
};
...
Tab.doAndGetNewTabs = async (asyncTask, windowId) => {
  ...
  const beforeTabs = await browser.tabs
    .query(tabsQueryOptions)
    .catch(ApiTabs.createErrorHandler());
  const beforeIds = mapAndFilterUniq(beforeTabs, (tab) => tab.id, {
    set: true,
  });
  await asyncTask();
  const afterTabs = await browser.tabs
    .query(tabsQueryOptions)
    .catch(ApiTabs.createErrorHandler());
  const addedTabs = mapAndFilter(
    afterTabs,
...
Tab.uniqTabsAndDescendantsSet = (tabs) => {
  if (!Array.isArray(tabs)) tabs = [tabs];
  return Array.from(
    new Set(tabs.flatMap((tab) => [tab].concat(tab.$TST.descendants)))
  ).sort(Tab.compare);
};
...
Tab.registerAutoStickyState = (providerId, statesToAdd) => {
  ...
  if (!Array.isArray(statesToAdd)) statesToAdd = [statesToAdd];
...
Tab.unregisterAutoStickyState = (providerId, statesToRemove) => {
  ...
  if (!states) return;
  if (!Array.isArray(statesToRemove)) statesToRemove = [statesToRemove];
  for (const state of statesToRemove) {
    states.delete(state);
  }
  if (states.size > 0) Tab.autoStickyStates.set(providerId, states);
  else Tab.autoStickyStates.delete(providerId);
