...
Tab.onRemoving.addListener(async (tab, removeInfo = {}) => {
  ...
  if (successor) {
    log(
      "override closeParentBehaior with kPARENT_TAB_OPERATION_BEHAVIOR_PROMOTE_FIRST_CHILD for different container successor ",
      successor
    );
    closeParentBehavior =
    ...
    if (successor && successor !== tab.$TST.firstChild) newParent = successor;
  } else {
...
async function handleRemovingPostProcess({
  closeParentBehavior,
  windowId,
  parent,
  newParent,
...
async function tryGrantCloseTab(tab, closeParentBehavior) {
  log("tryGrantClose: ", {
    alreadyGranted: configs.grantedRemovingTabIds,
    closing: dumpTab(tab),
  });
  const alreadyGranted = configs.grantedRemovingTabIds.includes(tab.id);
  configs.grantedRemovingTabIds = configs.grantedRemovingTabIds.filter(
    (id) => id !== tab.id
  );
  ...
  if (
    ...
    self.closingDescendantTabIds = self.closingDescendantTabIds.concat(
      TreeBehavior.getClosingTabsFromParent(tab).map((tab) => tab.id)
    );
    self.closingDescendantTabIds = Array.from(
      new Set(self.closingDescendantTabIds)
    );
  }
  ...
  self.promisedGrantedToCloseTabs = wait(250)
    .then(async () => {
      ...
      self.closingDescendantTabIds = mapAndFilterUniq(
        self.closingDescendantTabIds,
        (id) => {
          if (closingTabIds.has(id)) return undefined;
      ...
      const restorableClosingTabsCount = countMatched(
        allClosingTabs,
        (tab) => tab.url !== "about:blank" && !tab.$TST.isNewTabCommandTab
      );
      ...
      if (restorableClosingTabsCount > 0) {
        ...
        return Background.confirmToCloseTabs(
          allClosingTabs.slice(1).map((tab) => tab.$TST.sanitized),
          {
            windowId: tab.windowId,
...
async function closeChildTabs(tabs, { triggerTab, originalStructure } = {}) {
  ...
  // close bottom to top!
  await TabsInternalOperation.removeTabs(tabs.slice(0).reverse(), {
    triggerTab,
    originalStructure,
  });
