...
export function startTracking() {
  ...
  Tab.onRemoved.addListener((tab, info) => {
    if (!info.isWindowClosing) reserveToSaveTreeStructure(tab.windowId);
  });
  Tab.onMoved.addListener((tab, _info) => {
    reserveToSaveTreeStructure(tab.windowId);
  });
  Tab.onUpdated.addListener((tab, info) => {
    if ("openerTabId" in info) reserveToSaveTreeStructure(tab.windowId);
  });
...
async function saveTreeStructure(windowId) {
  ...
  const structure = TreeBehavior.getTreeStructureFromTabs(
    Tab.getAllTabs(windowId)
  );
  browser.sessions
    .setWindowValue(windowId, Constants.kWINDOW_STATE_TREE_STRUCTURE, structure)
...
export async function loadTreeStructure(windows, restoredFromCacheResults) {
  ...
  return MetricsData.addAsync(
    "loadTreeStructure: restoration for windows",
    Promise.all(
      windows.map(async (win) => {
        if (restoredFromCacheResults?.get(win.id)) {
          log(
            `skip tree structure restoration for window ${win.id} (restored from cache)`
          );
          return;
        }
        ...
        try {
          const structure = await browser.sessions
            .getWindowValue(win.id, Constants.kWINDOW_STATE_TREE_STRUCTURE)
            .catch(ApiTabs.createErrorHandler());
          let uniqueIds = tabs.map(
            (tab) => (tab.$TST.uniqueId && tab.$TST.uniqueId) || "?"
          );
          ...
          if (
            ...
            if (tabsOffset > -1) {
              ...
              MetricsData.add(
                "loadTreeStructure: Tree.applyTreeStructureToTabs"
              );
            } else {
            ...
            MetricsData.add(
              "loadTreeStructure: no valid structure information"
            );
          }
        } catch (error) {
          console.log(
            `TreeStructure.loadTreeStructure: Fatal error, ${error}`,
            error.stack
          );
        ...
        if (!windowStateCompletelyApplied) {
          log(
            `Tree information for the window ${win.id} is not same to actual state. Fallback to restoration from tab relations.`
          );
          MetricsData.add(
            "loadTreeStructure: fallback to reserveToAttachTabFromRestoredInfo"
          );
          ...
          for (const tab of tabs) {
            reserveToAttachTabFromRestoredInfo(tab, {
              ...
              if (attached || tab.$TST.parent || tab.$TST.hasChild)
                unattachedTabs.delete(tab);
            });
          }
          await reserveToAttachTabFromRestoredInfo.promisedDone;
          MetricsData.add(
            "loadTreeStructure: attachTabFromRestoredInfo finish"
          );
          ...
          for (const tab of unattachedTabs) {
            ...
            switch (action.action) {
              ...
              case "detach":
                log(
                  "loadTreeStructure: apply action for unattached tab: ",
                  tab,
                  action
                );
...
async function reserveToAttachTabFromRestoredInfo(tab, options = {}) {
  ...
  if (!reserveToAttachTabFromRestoredInfo.promisedDone) {
    reserveToAttachTabFromRestoredInfo.promisedDone = new Promise(
      (resolve, _reject) => {
  ...
  reserveToAttachTabFromRestoredInfo.waiting = setTimeout(async () => {
    ...
    const attachedResults = await Promise.all(
      uniqueIds.map((uniqueId, index) => {
        ...
        return attachTabFromRestoredInfo(task.tab, {
          ...
          console.log(
            `TreeStructure.reserveToAttachTabFromRestoredInfo: Fatal error on processing task ${index}, ${error}`,
            error.stack
          );
          return false;
        });
    ...
    if (typeof reserveToAttachTabFromRestoredInfo.onDone === "function")
      reserveToAttachTabFromRestoredInfo.onDone(
        attachedResults.every((attached) => !!attached)
      );
...
async function attachTabFromRestoredInfo(tab, options = {}) {
  ...
  let uniqueId,
    ...
    children,
    states,
    collapsed /* for backward compatibility */;
  ...
  [
    ...
    browser.sessions
      .getTabValue(tab.id, Constants.kPERSISTENT_INSERT_BEFORE)
      .catch(ApiTabs.createErrorHandler()),
    browser.sessions
      .getTabValue(tab.id, Constants.kPERSISTENT_INSERT_AFTER)
      .catch(ApiTabs.createErrorHandler()),
    // This legacy should be removed after legacy data are cleared enough, maybe after Firefox 128 is released.
    browser.sessions
      .getTabValue(tab.id, Constants.kPERSISTENT_INSERT_AFTER_LEGACY)
      .catch(ApiTabs.createErrorHandler()),
    browser.sessions
      .getTabValue(tab.id, Constants.kPERSISTENT_ANCESTORS)
      .catch(ApiTabs.createErrorHandler()),
    browser.sessions
      .getTabValue(tab.id, Constants.kPERSISTENT_CHILDREN)
      .catch(ApiTabs.createErrorHandler()),
    tab.$TST.getPermanentStates(),
    browser.sessions
      .getTabValue(tab.id, Constants.kPERSISTENT_SUBTREE_COLLAPSED)
      .catch(ApiTabs.createErrorHandler()), // for backward compatibility
  ]);
  ancestors = ancestors || [];
  children = children || [];
  ...
  if (collapsed && !states.includes(Constants.kTAB_STATE_SUBTREE_COLLAPSED)) {
    ...
    browser.sessions
      .removeTabValue(tab.id, Constants.kPERSISTENT_SUBTREE_COLLAPSED)
      .catch(ApiTabs.createErrorSuppressor());
  }
  ...
  if (configs.fixupTreeOnTabVisibilityChanged) {
    ancestors = ancestors.filter(
      (ancestor) => ancestor && ancestor.hidden === tab.hidden
    );
  ...
  if (tab.pinned || insertBefore?.pinned) insertBefore = null;
  const nextOfInsertAfter = insertAfter?.$TST.nextTab;
  if (nextOfInsertAfter?.pinned) insertAfter = null;
  ...
  for (const ancestor of ancestors) {
    ...
    if (options.bulk) promises.push(promisedDone);
    else await promisedDone;
    attached = true;
    break;
  }
  if (!attached) {
    const opener = tab.$TST.openerTab;
    if (opener && configs.syncParentTabAndOpenerTab) {
  ...
  if (options.children && !options.bulk) {
    ...
    for (const child of children) {
      ...
      await Tree.attachTabTo(child, tab, {
        ...
        insertAt: Constants.kINSERT_NEAREST,
        dontMove:
          child.index >= firstInTree.index &&
          child.index <= lastInTree.index + 1,
        broadcast: true,
      });
      if (child.index < firstInTree.index) firstInTree = child;
      else if (child.index > lastInTree.index) lastInTree = child;
    }
  }

  const subtreeCollapsed = states.includes(
    Constants.kTAB_STATE_SUBTREE_COLLAPSED
  );
  log("restore subtree collapsed state: ", tab.id, {
    current: tab.$TST.subtreeCollapsed,
    expected: subtreeCollapsed,
    ...options,
  });
  if (
    (options.canCollapse || options.bulk) &&
  ...
  const updateCollapsedState = () => {
    const shouldBeCollapsed = tab.$TST.ancestors.some(
      (ancestor) => ancestor.$TST.collapsed || ancestor.$TST.subtreeCollapsed
    );
    log("update collapsed state: ", tab.id, {
      current: tab.$TST.collapsed,
      expected: shouldBeCollapsed,
    });
    if (
      (options.canCollapse || options.bulk) &&
  ...
  if (options.bulk) await Promise.all(promises).then(updateCollapsedState);
  else updateCollapsedState();
...
Tab.onRestored.addListener((tab) => {
  ...
  mProcessingTabRestorations.push(async () => {
    try {
      ...
      if (count === 0) {
        ...
        const [uniqueId, restoredUniqueId] = await Promise.all([
          ...
          browser.sessions
            .getTabValue(activeTab.id, Constants.kPERSISTENT_ID)
            .catch(ApiTabs.createErrorHandler()),
        ]);
      ...
      reserveToAttachTabFromRestoredInfo.promisedDone.then(() => {
        ...
        if (count === 0) {
          ...
          const countToBeRestored = mRecentlyClosedTabs.filter(
            (tab) => !mRestoredTabIds.has(tab.uniqueId)
          );
          ...
          UserOperationBlocker.setProgress(
            Math.round(maxCount - (count / maxCount) * 100),
            tab.windowId
          );
        }
      });
    ...
    if (mProcessingTabRestorations.length > 0) mProcessingTabRestorations[0]();
  });
  if (mProcessingTabRestorations.length === 1) mProcessingTabRestorations[0]();
});
...
Tab.onMultipleTabsRemoving.addListener(
  (tabs, { triggerTab, originalStructure } = {}) => {
    if (triggerTab) tabs = [triggerTab, ...tabs];
    ...
    mPendingRecentlyClosedTabsInfo.structure =
      originalStructure ||
      TreeBehavior.getTreeStructureFromTabs(tabs, {
        full: true,
...
Tab.onMultipleTabsRemoved.addListener((tabs, { triggerTab } = {}) => {
  ...
  mRecentlyClosedTabs = mPendingRecentlyClosedTabsInfo.tabs.filter((info) =>
    tabIds.has(info.originalId)
  );
  mRecentlyClosedTabsTreeStructure =
    mPendingRecentlyClosedTabsInfo.structure.filter((structure) =>
      tabIds.has(structure.originalId)
    );
...
async function tryRestoreClosedSetFor(tab, countToBeRestored) {
  ...
  if (!configs.undoMultipleTabsClose) return;

  const alreadRestoredIndex = lastRecentlyClosedTabs.findIndex(
    (info) =>
      info.uniqueId === tab.$TST.uniqueId.id && info.windowId === tab.windowId
  );
  log(
    "tryRestoreClosedSetFor ",
    tab,
    lastRecentlyClosedTabs,
  ...
  const toBeRestoredTabsCount = Math.min(
    typeof countToBeRestored === "number"
      ? countToBeRestored
      : Number.MAX_SAFE_INTEGER,
    lastRecentlyClosedTabs.filter(
      (tabInfo) => tabInfo.uniqueId !== tab.$TST.uniqueId.id
    ).length
  );
  ...
  if (canRestoreWithSession) {
    log(
      `tryRestoreClosedSetFor: restore ${toBeRestoredTabsCount} tabs with the sessions API`
    );
    const unsortedRestoredTabs = await Commands.restoreTabs(
    ...
    restoredTabs = await Promise.all(
      lastRecentlyClosedTabsTreeStructure.map((tabInfo) => {
        const restoredTab = tabsByUniqueId.get(tabInfo.id);
        if (restoredTab) return restoredTab;
    ...
    for (const tab of restoredTabs) {
      if (lastTab && tab.$TST.previousTab !== lastTab)
        await TabsMove.moveTabAfter(tab, lastTab, { broadcast: true });
      lastTab = tab;
    }
  } else {
    ...
    if (beforeTabs.length > 0)
      await TabsMove.moveTabsBefore(beforeTabs, tab, { broadcast: true });
    if (afterTabs.length > 0)
      await TabsMove.moveTabsAfter(afterTabs, tab, { broadcast: true });
    restoredTabs = [...beforeTabs, tab, ...afterTabs];
    await TabsInternalOperation.activateTab(restoredTabs[0]);
  }

  const rootTabs = restoredTabs.filter(
    (_tab, index) =>
      lastRecentlyClosedTabsTreeStructure[index].parent ===
        TreeBehavior.STRUCTURE_KEEP_PARENT ||
      lastRecentlyClosedTabsTreeStructure[index].parent ===
        TreeBehavior.STRUCTURE_NO_PARENT
  );
  log(
    `tryRestoreClosedSetFor: rootTabs, restoredTabs = `,
    rootTabs,
    restoredTabs
  );
  for (const rootTab of rootTabs) {
    const referenceTabs =
      TreeBehavior.calculateReferenceTabsFromInsertionPosition(rootTab, {
    ...
    log(
      `tryRestoreClosedSetFor: referenceTabs for ${rootTab.id} => `,
      referenceTabs
    );
