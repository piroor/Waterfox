...
browser.tabs.onUpdated.addListener(
  (tabId, updateInfo, _tab) => {
    if (
      !("successorTabId" in updateInfo) ||
...
TabsInternalOperation.onBeforeTabsRemove.addListener(async (tabs) => {
  ...
  if (activeTab) await updateInternal(activeTab.id, tabIds);
});
...
function setSuccessor(tabId, successorTabId = -1) {
  ...
  promisedUpdate.promisedSuccessorTabId = new Promise((resolve, _reject) => {
    ...
    setTimeout(() => {
      if (!mPromisedUpdatedSuccessorTabId.has(tabId)) return;
      mPromisedUpdatedSuccessorTabId.delete(tabId);
      resolve(null);
    }, 2000);
  });
  ...
  browser.tabs
    ...
    .catch(
      ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError, (error) => {
  ...
  if (mInProgressUpdates.size === 0) {
    const waitingUpdate = new Promise((resolve, _reject) => {
      const timer = setInterval(() => {
        if (mInProgressUpdates.size > 1) return;
        clearInterval(timer);
        resolve();
      }, 100);
    });
    waitingUpdate
      .catch((_error) => {})
      .then(() => mInProgressUpdates.delete(waitingUpdate));
    ...
    for (const id of ids) {
      ...
      try {
        ...
        promisedUpdateWithTimeout
          .catch((_error) => {})
          .then(() => mInProgressUpdates.delete(promisedUpdateWithTimeout));
      } catch (_error) {}
    }
  }, 0);
}
async function updateInternal(tabId, excludeTabIds = []) {
  ...
  const promisedUpdate = mPromisedUpdatedSuccessorTabId.get(tabId);
  await Promise.all([tab.$TST.opened, promisedUpdate?.promisedSuccessorTabId]);

  const renewedTab = await browser.tabs
    .get(tabId)
    .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
  if (!renewedTab || !TabsStore.ensureLivingTab(tab)) return;
  log("updateInternal: ", dumpTab(tab), {
    tabSuccessorTabId: tab.successorTabId,
    renewedSuccessorTabId: renewedTab.successorTabId,
    lastSuccessorTabIdByOwner: tab.$TST.temporaryMetadata.get(
      "lastSuccessorTabIdByOwner"
    ),
  ...
  if (tab.$TST.temporaryMetadata.has("lastSuccessorTabIdByOwner")) {
    ...
    if (successor) {
      log(
        `  ${dumpTab(tab)} is already prepared for "selectOwnerOnClose" behavior (successor=${renewedTab.successorTabId})`
      );
      return;
    }
  ...
  const lastSuccessorTab =
    tab.$TST.temporaryMetadata.has("lastSuccessorTabId") &&
    Tab.get(tab.$TST.temporaryMetadata.get("lastSuccessorTabId"));
  if (!lastSuccessorTab) {
    ...
    if (
      renewedTab.successorTabId !== -1 &&
      renewedTab.successorTabId !==
        tab.$TST.temporaryMetadata.get("lastSuccessorTabId")
    ) {
      log(
        `  ${dumpTab(tab)}'s successor is modified by someone! Now it is out of control.`
      );
      tab.$TST.temporaryMetadata.delete("lastSuccessorTabId");
      return;
    }
  ...
  if (renewedTab.active) {
    ...
    const findSuccessor = (...candidates) => {
      for (const candidate of candidates) {
        if (!excludeTabIdsSet.has(candidate?.id) && candidate) return candidate;
      }
      return null;
    };
    if (
      configs.successorTabControlLevel ===
      Constants.kSUCCESSOR_TAB_CONTROL_IN_TREE
    ) {
      const closeParentBehavior = TreeBehavior.getParentTabOperationBehavior(
        tab,
        {
      ...
      const firstChild = collapsedChildSuccessorAllowed
        ? tab.$TST.firstChild
        : tab.$TST.firstVisibleChild;
      ...
      if (
        ...
        log(`  ${dumpTab(successor)} is discarded.`);
        successor = findSuccessor(tab.$TST.nearestLoadedTab, successor);
        log(`  => redirected successor is: ${dumpTab(successor)}`);
      }
    }
  }
...
async function tryClearOwnerSuccessor(tab) {
  if (!tab || !tab.$TST.temporaryMetadata.get("lastSuccessorTabIdByOwner"))
    return;
  tab.$TST.temporaryMetadata.delete("lastSuccessorTabIdByOwner");
  const renewedTab = await browser.tabs
    .get(tab.id)
...
Tab.onActivated.addListener(async (newActiveTab, info = {}) => {
  ...
  if (info.previousTabId) {
    ...
    if (oldActiveTab) {
      ...
      if (lastRelatedTab) {
        log(
          `clear lastRelatedTabs for the window ${info.windowId} by tabs.onActivated on ${newActiveTab.id}`
        );
        TabsStore.windows.get(info.windowId).clearLastRelatedTabs();
        if (lastRelatedTab.id !== newActiveTab.id) {
          log(
            `non last-related-tab is activated: cancel "back to owner" behavior for ${lastRelatedTab.id}`
          );
      ...
      if (newRelatedTabsCount > 1) {
        log(
          `multiple related tabs were opened: cancel "back to owner" behavior for ${newActiveTab.id}`
        );
...
Tab.onCreating.addListener((tab, info = {}) => {
  ...
  log(
    `Tab.onCreating: should control succesor of ${tab.id}: `,
    shouldControlSuccesor
  );
  if (shouldControlSuccesor) {
    ...
    tryClearOwnerSuccessor(info.activeTab).then(() => {
      const ownerTabId =
        tab.openerTabId || tab.active ? info.activeTab.id : null;
      if (!ownerTabId) return;

      log(
        `${dumpTab(tab)} is prepared for "selectOwnerOnClose" behavior (successor=${ownerTabId})`
      );
      ...
      log(
        `opener ${dumpTab(opener)}'s lastRelatedTab: ${dumpTab(lastRelatedTab)})`
      );
      if (lastRelatedTab) {
        log(" => clear successor");
...
Tab.onRemoving.addListener((tab, removeInfo = {}) => {
  ...
  if (lastRelatedTab && !lastRelatedTab.active)
    tryClearOwnerSuccessor(lastRelatedTab);
});
...
Tab.onUpdated.addListener((tab, changeInfo = {}) => {
  if (!("discarded" in changeInfo)) return;
  updateActiveTab(tab.windowId);
});
