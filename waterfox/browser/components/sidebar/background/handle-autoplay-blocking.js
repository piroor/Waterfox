...
function log(...args) {
  if (configs.debug) console.log(...args);
}
...
browser.menus.onClicked.addListener((info, contextTab) => {
  ...
  switch (info.menuItemId.replace(/^grouped:/, "")) {
    ...
    case "unblockAutoplayTree":
      if (inverted) unblockAutoplayDescendants(contextTabs);
      else unblockAutoplayTree(contextTabs);
      break;

    case "context_topLevel_unblockAutoplayDescendants":
    case "unblockAutoplayDescendants":
      if (inverted) unblockAutoplayTree(contextTabs);
      else unblockAutoplayDescendants(contextTabs);
      break;
  }
});
    ...
    case Constants.kCOMMAND_UNBLOCK_AUTOPLAY_FROM_SOUND_BUTTON:
      {
        ...
        if (!multiselected && root.$TST.subtreeCollapsed) {
          ...
          const toBeUpdatedTabs = tabsInTree.filter(
            (tab) => tab.$TST.autoplayBlocked
          );
          log("  toBeUpdatedTabs: ", toBeUpdatedTabs);
          browser.waterfoxBridge.unblockAutoplay(
            toBeUpdatedTabs.map((tab) => tab.id)
          );
        } else {
          log("  tabs: ", tabs);
