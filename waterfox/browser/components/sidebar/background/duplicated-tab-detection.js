...
async function doTest() {
  ...
  for (let i = 0; i < maxTry; i++) {
    promises.push(
      browser.tabs.duplicate(tab.id).then(async (duplicated) => {
  ...
  const successCount = (await Promise.all(promises)).filter(
    (result) => !!result
  ).length;
...
async function autoDetectSuitableDelay() {
  ...
  if (successRate === 1) return;

  configs.delayForDuplicatedTabDetection = 10;
  while (successRate < 1) {
    configs.delayForDuplicatedTabDetection = Math.round(
      configs.delayForDuplicatedTabDetection * (1 / Math.max(successRate, 0.5))
    );
