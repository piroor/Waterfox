...
async function tryDetectTabBunches(win) {
  if (Tab.needToWaitTracked(win.id)) await Tab.waitUntilTrackedAll(win.id);
  if (Tab.needToWaitMoved(win.id)) await Tab.waitUntilMovedAll(win.id);
  ...
  tabReferences = tabReferences.filter((tabReference) => {
    if (!tabReference.id) return false;
  ...
  if (tabReferences.length > 1) {
    await Promise.all(
      tabReferences.map((tabReference) => {
        const tab = Tab.get(tabReference.id);
  ...
  if (
    ...
    const index = tabReferences
      .map((tabReference) => Tab.get(tabReference.id).index)
      .sort(compareAsNumber)[0];
...
async function tryGroupTabBunches() {
  if (tryGroupTabBunches.running) return;

  const tabReferences = mPossibleTabBunchesToBeGrouped.shift();
  if (!tabReferences) return;
  ...
  try {
    ...
    for (const tabReference of tabReferences) {
      if (tabReference.shouldNotGrouped) continue;
      const tab = Tab.get(tabReference.id);
      if (!tab) continue;
      ...
      if (tab.$TST.isGroupTab || uniqueId.duplicated || uniqueId.restored)
        continue;
      if (tabReference.openerIsPinned || tabReference.openerIsFirefoxView) {
        ...
        if (
          (tab.$TST.hasPinnedOpener && configs.autoGroupNewTabsFromPinned) ||
    ...
    const openedFromBookmarkFolder =
      fromOthers.length > 0 &&
      (await detectBookmarkFolderFromTabs(fromOthers, tabReferences.length));
    log(
      " => tryGroupTabBunches:openedFromBookmarkFolder: ",
      !!openedFromBookmarkFolder
    );
    const newRootTabs = Tab.collectRootTabs(
      Tab.sort(
        openedFromBookmarkFolder ? openedFromBookmarkFolder.tabs : fromOthers
      )
    );
    log(" newRootTabs: ", newRootTabs);
    if (
      newRootTabs.length > 1 &&
      ...
      if (granted)
        await TabsGroup.groupTabs(newRootTabs, {
          ...TabsGroup.temporaryStateParams(
            configs.groupTabTemporaryStateForNewTabsFromOthers
          ),
          broadcast: true,
        });
    }
    ...
    if (mPossibleTabBunchesToBeGrouped.length > 0) tryGroupTabBunches();
  }
}

async function confirmToAutoGroupNewTabsFromOthers(tabs) {
  if (tabs.length <= 1 || !configs.warnOnAutoGroupNewTabs) return true;
  ...
  switch (result.buttonIndex) {
    case 0:
      if (!result.checked) configs.warnOnAutoGroupNewTabs = false;
      return true;
    case 1:
      if (!result.checked) {
...
async function tryGroupTabBunchesFromPinnedOpener(rootTabs) {
  log(
    `tryGroupTabBunchesFromPinnedOpener: ${rootTabs.length} root tabs are opened from pinned tabs`
  );
  ...
  for (const tab of rootTabs) {
    ...
    if (!pinnedOpeners.includes(opener)) pinnedOpeners.push(opener);
  }
  ...
  for (const tab of allRootTabs) {
    ...
    const opener = Tab.getByUniqueId(
  ...
  pinnedOpeners = pinnedOpeners.filter((opener) => {
    return (
      childrenOfPinnedTabs[opener.id].length > 1 ||
  ...
  for (const tab of rootTabs
    ...
    try {
      refTabs = Tree.getReferenceTabsForNewChild(tab, null, {
        ...
        parent: siblings[0],
        children: siblings,
        descendants: siblings.flatMap((sibling) => [
          sibling,
          ...sibling.$TST.descendants,
        ]),
      });
    } catch (_error) {
    ...
    if (refTabs.insertAfter) {
      await Tree.moveTabSubtreeAfter(tab, refTabs.insertAfter, {
        broadcast: true,
      });
      log(
        `newly opened child ${tab.id} has been moved after ${refTabs.insertAfter?.id}`
      );
    } else if (refTabs.insertBefore) {
      await Tree.moveTabSubtreeBefore(tab, refTabs.insertBefore, {
        broadcast: true,
      });
      log(
        `newly opened child ${tab.id} has been moved before ${refTabs.insertBefore?.id}`
      );
    } else {
      continue;
    }
    tab.$TST.temporaryMetadata.set(
      "alreadyMovedAsOpenedFromPinnedOpener",
      true
    );
  }
  ...
  for (const opener of pinnedOpeners) {
    const children = childrenOfPinnedTabs[opener.id].sort(
      (a, b) => a.index - b.index
    );
    ...
    log(`trying to group children of ${dumpTab(opener)}: `, () =>
      children.map(dumpTab)
    );
    const uri = TabsGroup.makeGroupTabURI({
      ...
      ...TabsGroup.temporaryStateParams(
        isFirefoxViewTab(opener)
          ? configs.groupTabTemporaryStateForChildrenOfFirefoxView
          : configs.groupTabTemporaryStateForChildrenOfPinned
      ),
    });
...
async function tryHandlTabBunchesFromBookmarks() {
  if (tryHandlTabBunchesFromBookmarks.running) return;
  ...
  try {
    ...
    for (const tabReference of tabReferences) {
      ...
      if (tabReference.openerTabId)
        tab.openerTabId = parseInt(tabReference.openerTabId); // restore the opener information
      const uniqueId = tab.$TST.uniqueId;
      if (uniqueId.duplicated || uniqueId.restored) continue;
      tabs.push(tab);
    }
    log(" => ", { tabs });
    ...
    const openedFromBookmarkFolder =
      tabs.length > 0 &&
      (await detectBookmarkFolderFromTabs(tabs, tabReferences.length));
    log(
      " => tryHandlTabBunchesFromBookmarks:openedFromBookmarkFolder: ",
      openedFromBookmarkFolder
    );
    if (openedFromBookmarkFolder) {
      if (configs.restoreTreeForTabsFromBookmarks) {
        ...
        const structure = await Bookmark.getTreeStructureFromBookmarkFolder(
          openedFromBookmarkFolder.folder
        );
        log(" ==> structure:", structure);
        if (structure.length === openedFromBookmarkFolder.tabs.length) {
          log(" ===> apply");
          await Tree.applyTreeStructureToTabs(
            openedFromBookmarkFolder.tabs,
            structure
          );
        }
      }
      ...
      const newRootTabs = Tab.collectRootTabs(
        Tab.sort(openedFromBookmarkFolder.tabs)
      );
      if (
        newRootTabs.length > 1 &&
        configs.autoGroupNewTabsFromBookmarks &&
    ...
    log(
      "Error on tryHandlTabBunchesFromBookmarks: ",
      String(error),
      error.stack
    );
  } finally {
...
async function detectBookmarkFolderFromTabs(
  tabs,
  allNewTabsCount = tabs.length
) {
  ...
  return new Promise((resolve, _reject) => {
    ...
    for (const tab of tabs) {
      tab.$TST.promisedPossibleOpenerBookmarks.then(async (bookmarks) => {
        ...
        restCount--;
        if (bookmarks.length > 0) maybeFromBookmarks.push(tab);
        const folder = await tryDetectMostTabsContainedBookmarkFolder(
          maybeFromBookmarks,
          allNewTabsCount
        );
        if (folder) {
          log("detectBookmarkFolderFromTabs: found folder => ", {
            folder,
            tabs,
          });
          resolve({
...
async function tryDetectMostTabsContainedBookmarkFolder(
  bookmarkedTabs,
  allNewTabsCount = bookmarkedTabs.length
) {
  log("tryDetectMostTabsContainedBookmarkFolder ", {
    bookmarkedTabs,
    allNewTabsCount,
  });
  const parentIds = bookmarkedTabs.flatMap((tab) =>
    tab.$TST.possibleOpenerBookmarks.map((bookmark) => bookmark.parentId)
  );
  const counts = [];
  const countById = {};
  for (const id of parentIds) {
    if (!(id in countById)) counts.push((countById[id] = { id, count: 0 }));
    countById[id].count++;
  }
  log(" counts: ", counts);
  ...
  const minCount =
    (allNewTabsCount * configs.tabsFromSameFolderMinThresholdPercentage) / 100;
  log(" => ", { greatestCountParent, minCount });
  if (greatestCountParent.count <= minCount) return null;
...
function areTabsFromOtherDeviceWithInsertAfterCurrent(tabReferences) {
  if (tabReferences.length === 0) return false;

  const activeTab = Tab.getActiveTab(tabReferences[0].windowId);
  if (!activeTab) return false;

  const activeIndex = activeTab.index;
  const followingTabsCount = Tab.getTabs(activeTab.windowId).filter(
    (tab) => tab.index > activeIndex
  ).length;

  const createdCount = tabReferences.length;
  const expectedIndices = [activeIndex + 1];
  const actualIndices = tabReferences.map(
    (tabReference) => tabReference.indexOnCreated
  );

  const overTabsCount = Math.max(0, createdCount - followingTabsCount);
  const shouldCountDown = Math.min(
    createdCount - 1,
    createdCount - Math.floor(overTabsCount / 2)
  );
  ...
  log("areTabsFromOtherDeviceWithInsertAfterCurrent:", received, {
    overTabsCount,
    shouldCountUp,
    shouldCountDown,
    size: expectedIndices.length,
    actualIndices,
    expectedIndices,
  });
  return received;
