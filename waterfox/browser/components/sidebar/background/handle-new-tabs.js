...
Tab.onBeforeCreate.addListener(async (tab, info) => {
  ...
  const shouldAttachToPinnedOpener =
    ...
    Constants.kCONTROLLED_NEWTAB_POSITION.has(
      configs.autoAttachOnNewTabCommand
    ) &&
    ((activeTab?.pinned &&
      Constants.kCONTROLLED_INSERTION_POSITION.has(
        configs.insertNewTabFromPinnedTabAt
      )) ||
      (isFirefoxViewTab(activeTab) &&
        Constants.kCONTROLLED_INSERTION_POSITION.has(
          configs.insertNewTabFromFirefoxViewAt
        )));
  if (shouldAttachToPinnedOpener) tab.openerTabId = activeTab.id;
});

// this should return false if the tab is / may be moved while processing
Tab.onCreating.addListener((tab, info = {}) => {
  if (info.duplicatedInternally) return true;
  ...
  if (opener) {
    tab.$TST.setAttribute(
      Constants.kPERSISTENT_ORIGINAL_OPENER_TAB_ID,
      opener.$TST.uniqueId.id
    );
    if (!info.bypassTabControl) TabsStore.addToBeGroupedTab(tab);
  } else {
    let dontMove = false;
    if (
      !info.maybeOrphan &&
      ...
      if (tab.$TST.nextTab && activeTab === tab.$TST.previousTab) {
        ...
        switch (autoAttachBehavior) {
          ...
          case Constants.kNEWTAB_OPEN_AS_NEXT_SIBLING:
            ...
            else autoAttachBehavior = Constants.kNEWTAB_OPEN_AS_NEXT_SIBLING;
            break;
          default:
            break;
        }
      ...
      if (!info.fromExternal) tab.$TST.temporaryMetadata.set("isNewTab", true);
    }
    if (info.fromExternal && !info.bypassTabControl) {
      log("behave as a tab opened from external application");
      // we may need to reopen the tab with loaded URL
      if (
        configs.inheritContextualIdentityToTabsFromExternalMode !==
    ...
    if (
      ...
      if (
        configs.inheritContextualIdentityToTabsFromAnyOtherTriggerMode !==
        Constants.kCONTEXTUAL_IDENTITY_DEFAULT
      )
  ...
  log(
    `opener: ${dumpTab(opener)}, positionedBySelf = ${info.positionedBySelf}`
  );
  if (
    !info.bypassTabControl &&
    opener &&
    ...
    return handleTabsFromPinnedOpener(tab, opener, { activeTab }).then(
      (moved) => !moved
    );
...
async function notifyToTryHandleNewTab(
  tab,
  { context, activeTab, openerTab } = {}
) {
  const cache = {};
  const result = TSTAPI.tryOperationAllowed(
    TSTAPI.kNOTIFY_TRY_HANDLE_NEWTAB,
    { tab, activeTab, openerTab, context },
...
async function handleNewTabFromActiveTab(
  tab,
  {
    url,
    activeTab,
  ...
  log("handleNewTabFromActiveTab: activeTab = ", dumpTab(activeTab), {
    url,
    activeTab,
    autoAttachBehavior,
    dontMove,
  ...
  if (activeTab?.$TST.ancestors.includes(tab)) {
  ...
  if (openedWithCookieStoreId) {
    log(
      "handleNewTabFromActiveTab: do not reopen tab opened with contextual identity explicitly"
    );
    return moved;
  }
  if (tab.cookieStoreId && tab.cookieStoreId !== "firefox-default") {
    log(
      "handleNewTabFromActiveTab: do not reopen tab opened with non-default contextual identity ",
      tab.cookieStoreId
    );
    return moved;
  }
  ...
  switch (inheritContextualIdentityMode) {
    case Constants.kCONTEXTUAL_IDENTITY_FROM_PARENT:
      if (parent) cookieStoreId = parent.cookieStoreId;
      break;
  ...
  if (
    ...
    log(
      "handleNewTabFromActiveTab: no need to reopen with inherited contextual identity ",
      cookieStoreId
    );
    return moved;
  }
  ...
  if (
    !configs.inheritContextualIdentityToUnopenableURLTabs &&
    !TabsOpen.isOpenable(url)
  ) {
    log(
      "handleNewTabFromActiveTab: not openable URL, skip reopening ",
      cookieStoreId,
      url
    );
    return moved;
  ...
  log(
    "handleNewTabFromActiveTab: reopen with inherited contextual identity ",
    cookieStoreId
  );
...
async function handleTabsFromPinnedOpener(tab, opener, { activeTab } = {}) {
  ...
  if (parent) {
    ...
    const insertAt =
      configs.autoAttachOnOpenedWithOwner ===
      Constants.kNEWTAB_OPEN_AS_CHILD_NEXT_TO_LAST_RELATED_TAB
  ...
  switch (
    ...
    case Constants.kINSERT_NEXT_TO_LAST_RELATED_TAB: {
      ...
      if (lastRelatedTab) {
        log(
          `handleTabsFromPinnedOpener: place after last related tab ${dumpTab(lastRelatedTab)}`
        );
        tab.$TST.temporaryMetadata.set(
          "alreadyMovedAsOpenedFromPinnedOpener",
          true
        );
        return TabsMove.moveTabAfter(
          tab,
      ...
      if (lastPinnedTab) {
        log(
          `handleTabsFromPinnedOpener: place after last pinned tab ${dumpTab(lastPinnedTab)}`
        );
        tab.$TST.temporaryMetadata.set(
          "alreadyMovedAsOpenedFromPinnedOpener",
          true
      ...
      if (firstNormalTab) {
        log(
          `handleTabsFromPinnedOpener: place before first unpinned tab ${dumpTab(firstNormalTab)}`
        );
        tab.$TST.temporaryMetadata.set(
          "alreadyMovedAsOpenedFromPinnedOpener",
          true
    ...
    case Constants.kINSERT_TOP:
      {
        ...
        if (lastPinnedTab) {
          log(
            `handleTabsFromPinnedOpener: opened from pinned opener: place after last pinned tab ${dumpTab(lastPinnedTab)}`
          );
          tab.$TST.temporaryMetadata.set(
            "alreadyMovedAsOpenedFromPinnedOpener",
            true
        ...
        if (firstNormalTab) {
          log(
            `handleTabsFromPinnedOpener: opened from pinned opener: place before first pinned tab ${dumpTab(firstNormalTab)}`
          );
          tab.$TST.temporaryMetadata.set(
            "alreadyMovedAsOpenedFromPinnedOpener",
            true
    ...
    case Constants.kINSERT_END: {
      ...
      log(
        "handleTabsFromPinnedOpener: opened from pinned opener: place after the last tab ",
        lastTab
      );
      tab.$TST.temporaryMetadata.set(
        "alreadyMovedAsOpenedFromPinnedOpener",
...
Tab.onCreated.addListener((tab, info = {}) => {
  if (!info.duplicated || info.bypassTabControl) return;
...
Tab.onUpdated.addListener((tab, changeInfo) => {
  if (
    "openerTabId" in changeInfo &&
    configs.syncParentTabAndOpenerTab &&
    !tab.$TST.updatingOpenerTabIds.includes(
      changeInfo.openerTabId
    ) /* accept only changes from outside of TST */
  ) {
  ...
  if (
    tab.$TST.temporaryMetadata.has("openedCompletely") &&
    ...
    const possibleOpenerTab = Tab.get(
      tab.$TST.temporaryMetadata.get("possibleOpenerTab")
    );
    ...
    if (
      ...
      handleNewTabFromActiveTab(tab, {
        ...
        inheritContextualIdentityMode:
          configs.inheritContextualIdentityToSameSiteOrphanMode,
        context: TSTAPI.kNEWTAB_CONTEXT_WEBSITE_SAME_TO_ACTIVE_TAB,
      });
      return;
    }

    log("checking special openers (delayed)", {
      opener: possibleOpenerTab.url,
      child: tab.url,
    });
    for (const rule of Constants.kAGGRESSIVE_OPENER_TAB_DETECTION_RULES_WITH_URL) {
      if (rule.opener.test(possibleOpenerTab.url) && rule.child.test(tab.url)) {
...
Tab.onAttached.addListener(async (tab, attachInfo = {}) => {
  if (!attachInfo.windowId) return;

  const parentTabOperationBehavior = TreeBehavior.getParentTabOperationBehavior(
    tab,
    {
  ...
  log("descendants of attached tab: ", () =>
    attachInfo.descendants.map(dumpTab)
  );
