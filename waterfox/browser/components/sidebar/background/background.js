...
export async function init() {
  ...
  getAllWindows().then((windows) => {
    for (const win of windows) {
      browser.sessions
        .getWindowValue(win.id, Constants.kWINDOW_STATE_CACHED_TABS)
        .catch(ApiTabs.createErrorSuppressor())
        .then((cache) => mPreloadedCaches.set(`window-${win.id}`, cache));
      const tab = win.tabs[0];
      browser.sessions
        .getTabValue(tab.id, Constants.kWINDOW_STATE_CACHED_TABS)
        .catch(ApiTabs.createErrorSuppressor())
  ...
  await MetricsData.addAsync(
    "init: waiting for waitUntilCompletelyRestored, ContextualIdentities.init and configs.$loaded",
    Promise.all([
  ...
  const windows = await MetricsData.addAsync(
    "init: getting all tabs across windows",
    promisedWindows
  ); // wait at here for better performance
  const restoredFromCache = await MetricsData.addAsync(
    "init: rebuildAll",
    rebuildAll(windows)
  );
  mPreloadedCaches.clear();
  await MetricsData.addAsync(
    "init: TreeStructure.loadTreeStructure",
    TreeStructure.loadTreeStructure(windows, restoredFromCache)
  );
  ...
  MetricsData.addAsync(
    "init: initializing API for other addons",
    TSTAPI.initAsBackend()
  );
  ...
  await MetricsData.addAsync(
    "init: exporting tabs to sidebars",
    notifyReadyToSidebars()
  );

  log(
    `Startup metrics for ${TabsStore.tabs.size} tabs: `,
...
async function updatePanelUrl(theme) {
  ...
  url.searchParams.set(
    "reloadMaskImage",
    !!configs.enableWorkaroundForBug1763420_reloadMaskImage
  );
...
async function waitUntilCompletelyRestored() {
  ...
  await Promise.all([
    MetricsData.addAsync(
      "waitUntilCompletelyRestored: existing tabs ",
      Promise.all(
        initialTabs.map((tab) =>
          waitUntilPersistentIdBecomeAvailable(tab.id).catch((_error) => {})
        )
      )
    ),
    MetricsData.addAsync(
      "waitUntilCompletelyRestored: opening tabs ",
      new Promise((resolve, _reject) => {
        ...
        let onNewTabRestored = async (tab, _info = {}) => {
          ...
          browser.sessions
            .getWindowValue(tab.windowId, Constants.kWINDOW_STATE_CACHED_TABS)
            .catch(ApiTabs.createErrorSuppressor())
            .then((cache) =>
              mPreloadedCaches.set(`window-${tab.windowId}`, cache)
            );
          browser.sessions
            .getTabValue(tab.id, Constants.kWINDOW_STATE_CACHED_TABS)
            .catch(ApiTabs.createErrorSuppressor())
...
async function waitUntilPersistentIdBecomeAvailable(tabId, retryCount = 0) {
  ...
  const uniqueId = await browser.sessions.getTabValue(
    tabId,
    Constants.kPERSISTENT_ID
  );
  if (!uniqueId)
    return wait(100).then(() =>
      waitUntilPersistentIdBecomeAvailable(tabId, retryCount + 1)
    );
  return true;
}
...
async function rebuildAll(windows) {
  if (!windows) windows = await getAllWindows();
  const restoredFromCache = new Map();
  await Promise.all(
    windows.map(async (win) => {
      await MetricsData.addAsync(
        `rebuildAll: tabs in window ${win.id}`,
        async () => {
          let trackedWindow = TabsStore.windows.get(win.id);
          if (!trackedWindow) trackedWindow = Window.init(win.id);
          ...
          try {
            if (configs.useCachedTree) {
              ...
              const restored = await MetricsData.addAsync(
                `rebuildAll: restore tabs in window ${win.id} from cache`,
                BackgroundCache.restoreWindowFromEffectiveWindowCache(win.id, {
...
export async function reload(options = {}) {
  ...
  await MetricsData.addAsync(
    "reload: TreeStructure.loadTreeStructure",
    TreeStructure.loadTreeStructure(windows)
  );
  if (!options.all) return;
  for (const win of TabsStore.windows.values()) {
    if (!SidebarConnection.isOpen(win.id)) continue;
...
export async function tryStartHandleAccelKeyOnTab(tab) {
  ...
  if (!granted || /^(about|chrome|resource):/.test(tab.url)) return;
  try {
...
export function reserveToUpdateInsertionPosition(tabOrTabs) {
  ...
  for (const tab of tabs) {
    ...
    const reserved = reserveToUpdateInsertionPosition.reserved.get(
      tab.windowId
    ) || {
      timer: null,
      tabs: new Set(),
...
async function updateInsertionPosition(tab) {
  ...
  if (prev)
    ...
    browser.sessions
      .removeTabValue(tab.id, Constants.kPERSISTENT_INSERT_AFTER)
      .catch(
        ApiTabs.createErrorSuppressor(
          ApiTabs.handleMissingTabError // The tab can be closed while waiting.
        )
  ...
  browser.sessions
    .removeTabValue(tab.id, Constants.kPERSISTENT_INSERT_AFTER_LEGACY)
    .catch(
      ApiTabs.createErrorSuppressor(
        ApiTabs.handleMissingTabError // The tab can be closed while waiting.
      )
  ...
  if (next)
    ...
    browser.sessions
      .removeTabValue(tab.id, Constants.kPERSISTENT_INSERT_BEFORE)
      .catch(
        ApiTabs.createErrorSuppressor(
          ApiTabs.handleMissingTabError // The tab can be closed while waiting.
        )
...
async function updateAncestors(tab) {
  ...
  const ancestors = tab.$TST.ancestors.map(
    (ancestor) => ancestor.$TST.uniqueId.id
  );
  log(`updateAncestors: save persistent ancestors for ${tab.id}: `, ancestors);
  browser.sessions
    .setTabValue(tab.id, Constants.kPERSISTENT_ANCESTORS, ancestors)
    .catch(
      ApiTabs.createErrorSuppressor(
        ApiTabs.handleMissingTabError // The tab can be closed while waiting.
      )
...
async function updateChildren(tab) {
  ...
  browser.sessions
    .setTabValue(tab.id, Constants.kPERSISTENT_CHILDREN, children)
    .catch(
      ApiTabs.createErrorSuppressor(
        ApiTabs.handleMissingTabError // The tab can be closed while waiting.
      )
...
function reserveToUpdateSubtreeCollapsed(tab) {
  if (!mInitialized || !TabsStore.ensureLivingTab(tab)) return;
  const reserved = reserveToUpdateSubtreeCollapsed.reserved.get(
    tab.windowId
  ) || {
    timer: null,
    tabs: new Set(),
...
export async function confirmToCloseTabs(
  tabs,
  { windowId, configKey, messageKey, titleKey, minConfirmCount } = {}
) {
  if (!windowId) windowId = tabs[0].windowId;

  const grantedIds = new Set(configs.grantedRemovingTabIds);
  let count = 0;
  const tabIds = [];
  tabs = tabs
    .map((tab) => tab && Tab.get(tab.id))
    .filter((tab) => {
  ...
  const deltaFromLastConfirmation =
    Date.now() - configs.lastConfirmedToCloseTabs;
  log("confirmToCloseTabs ", {
    tabIds,
    count,
    windowId,
    configKey,
    grantedIds,
    shouldConfirm,
    deltaFromLastConfirmation,
    minConfirmCount,
  });
  if (
    count <= (typeof minConfirmCount === "number" ? minConfirmCount : 1) ||
    !shouldConfirm ||
  ...
  const result = await Dialog.show(win, {
    ...
    url: (await Permissions.isGranted(Permissions.ALL_URLS))
      ? null
      : "/resources/blank.html", // for popup
  ...
  switch (result.buttonIndex) {
    case 0:
      if (!result.checked) configs[configKey] = false;
...
Tab.onUpdated.addListener((tab, changeInfo) => {
  ...
  const url = changeInfo.url
    ...
    : status === "complete" && tab
      ? tab.url
      : "";
  if (tab && Constants.kSHORTHAND_ABOUT_URI.test(url)) {
    const shorthand = RegExp.$1;
    const oldUrl = tab.url;
    wait(100).then(() => {
      // redirect with delay to avoid infinite loop of recursive redirections.
      if (tab.url !== oldUrl) return;
      browser.tabs
        .update(tab.id, {
          url: url.replace(
            Constants.kSHORTHAND_ABOUT_URI,
            Constants.kSHORTHAND_URIS[shorthand] || "about:blank"
          ),
        })
  ...
  if (changeInfo.status || changeInfo.url) tryStartHandleAccelKeyOnTab(tab);
});
...
Tab.onMutedStateChanged.addListener((root, toBeMuted) => {
  ...
  for (const tab of tabs) {
    ...
    if (
      configs.spreadMutedStateOnlyToSoundPlayingTabs &&
      !playing &&
      playing !== toBeMuted
    )
      continue;
...
Tab.onMoved.addListener((tab, moveInfo) => {
  if (moveInfo.movedInBulk) return;
  reserveToUpdateInsertionPosition([
    tab,
...
Tree.onAttached.addListener(async (tab, attachInfo) => {
  ...
  if (
    !TabsStore.ensureLivingTab(tab) || // not removed while waiting
    tab.$TST.parent !== attachInfo.parent
  )
...
async function updateIconForBrowserTheme(theme) {
  ...
  if (theme.colors) {
    ...
    log(
      " => ",
      { toolbarIconColor, menuIconColor, sidebarIconColor },
      theme.colors
    );
    await Promise.all(
      Array.from(Object.entries(BASE_ICONS), async ([size, url]) => {
        const response = await fetch(url);
        const body = await response.text();
        const toolbarIconSource = body.replace(
          /transparent\s*\/\*\s*TO BE REPLACED WITH THEME COLOR\s*\*\//g,
          toolbarIconColor
        );
        toolbarIcons[size] =
          `data:image/svg+xml,${escape(toolbarIconSource)}#toolbar-theme`;
        const menuIconSource = body.replace(
          /transparent\s*\/\*\s*TO BE REPLACED WITH THEME COLOR\s*\*\//g,
          menuIconColor
        );
        menuIcons[size] =
          `data:image/svg+xml,${escape(menuIconSource)}#default-theme`;
        const sidebarIconSource = body.replace(
          /transparent\s*\/\*\s*TO BE REPLACED WITH THEME COLOR\s*\*\//g,
          sidebarIconColor
        );
        sidebarIcons[size] =
          `data:image/svg+xml,${escape(sidebarIconSource)}#default-theme`;
      })
    );
  } else {
  ...
  await Promise.all([
    ...ContextMenu.getItemIdsWithIcon().map((id) =>
      browser.menus.update(id, { icons: menuIcons })
    ),
