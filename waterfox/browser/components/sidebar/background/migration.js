...
export function migrateConfigs() {
  switch (configs.configsVersion) {
    ...
    case 2:
      if (
        configs.simulateSelectOwnerOnClose !== null &&
        !configs.simulateSelectOwnerOnClose
      )
        configs.successorTabControlLevel =
          Constants.kSUCCESSOR_TAB_CONTROL_NEVER;
    ...
    case 3:
      ...
      if (
        !(
          configs.tabDragBehaviorShift & Constants.kDRAG_BEHAVIOR_ALLOW_BOOKMARK
        )
      )
    ...
    case 4:
      ...
      if (configs.context_closeTabOptions_closeTree !== null)
        configs.context_topLevel_closeTree =
          configs.context_closeTabOptions_closeTree;
      if (configs.context_closeTabOptions_closeDescendants !== null)
        configs.context_topLevel_closeDescendants =
          configs.context_closeTabOptions_closeDescendants;
      if (configs.context_closeTabOptions_closeOthers !== null)
        configs.context_topLevel_closeOthers =
          configs.context_closeTabOptions_closeOthers;

    case 5:
      if (configs.scrollbarMode !== null) {
        switch (
          configs.scrollbarMode < 0
            ? isMacOS()
              ? 3
              : 1
            : configs.scrollbarMode
        ) {
...
:root.left #tabbar { direction: ltr; }`;
      ...
      if (configs.parentTabBehaviorForChanges !== null) {
        switch (configs.parentTabBehaviorForChanges) {
          case Constants.kPARENT_TAB_BEHAVIOR_ALWAYS:
            configs.parentTabOperationBehaviorMode =
              Constants.kPARENT_TAB_OPERATION_BEHAVIOR_MODE_CONSISTENT;
            break;
          default:
            configs.parentTabOperationBehaviorMode =
          ...
          case Constants.kPARENT_TAB_BEHAVIOR_ONLY_ON_SIDEBAR:
            configs.parentTabOperationBehaviorMode =
              Constants.kPARENT_TAB_OPERATION_BEHAVIOR_MODE_CUSTOM;
    ...
    case 7:
      if (
        ...
        configs.treeDoubleClickBehavior =
          Constants.kTREE_DOUBLE_CLICK_BEHAVIOR_TOGGLE_COLLAPSED;
    ...
    case 9:
      if (
        ...
        configs.treeDoubleClickBehavior =
          Constants.kTREE_DOUBLE_CLICK_BEHAVIOR_CLOSE;

    case 10:
      if (configs.style === "plain-dark" || configs.style === "metal")
    ...
    case 12:
      try {
        saveUserStyleRules(
          Array.from(new Uint8Array(8), (_, index) => {
    ...
    case 13:
      if (configs.style === "mixed" || configs.style === "vertigo")
    ...
    case 14:
      if (configs.inheritContextualIdentityToNewChildTab !== null)
        configs.inheritContextualIdentityToChildTabMode =
          configs.inheritContextualIdentityToNewChildTab
            ? Constants.kCONTEXTUAL_IDENTITY_FROM_PARENT
      ...
      if (configs.inheritContextualIdentityToSameSiteOrphan !== null)
        configs.inheritContextualIdentityToSameSiteOrphanMode =
          configs.inheritContextualIdentityToSameSiteOrphan
      ...
      if (configs.inheritContextualIdentityToTabsFromExternal !== null)
        configs.inheritContextualIdentityToTabsFromExternalMode =
          configs.inheritContextualIdentityToTabsFromExternal
            ? Constants.kCONTEXTUAL_IDENTITY_FROM_PARENT
            : Constants.kCONTEXTUAL_IDENTITY_DEFAULT;
    ...
    case 15:
      if (
        ...
        if (configs.tabDragBehavior & Constants.kDRAG_BEHAVIOR_TEAR_OFF)
          ...
          configs.tabDragBehavior & Constants.kDRAG_BEHAVIOR_ALLOW_BOOKMARK
        )
          configs.tabDragBehavior =
            configs.tabDragBehavior ^ Constants.kDRAG_BEHAVIOR_ALLOW_BOOKMARK;
    ...
    case 16:
      configs.maximumDelayForBug1561879 = Math.max(
        configs.$default.maximumDelayForBug1561879,
        configs.maximumDelayForBug1561879
      );
    ...
    case 18:
      if (configs.connectionTimeoutDelay === 5000)
        configs.connectionTimeoutDelay =
          configs.$default.connectionTimeoutDelay;
      ...
      if (configs.suppressGapFromShownOrHiddenToolbar !== null) {
        configs.suppressGapFromShownOrHiddenToolbarOnNewTab =
          configs.suppressGapFromShownOrHiddenToolbarOnFullScreen =
            configs.suppressGapFromShownOrHiddenToolbar;
      }
    ...
    case 21:
      if (configs.style === "plain") configs.style = "photon";

    case 22:
    case 23:
      if (configs.closeParentBehaviorMode !== null) {
        configs.parentTabOperationBehaviorMode =
          configs.closeParentBehaviorMode;
      }
    ...
    case 24:
      ...
      if (configs.autoGroupNewTabsDelayOnNewWindow !== null)
        configs.tabBunchesDetectionDelayOnNewWindow =
          configs.autoGroupNewTabsDelayOnNewWindow;

    case 25:
      if (configs.autoHiddenScrollbarPlaceholderSize !== null)
        configs.shiftTabsForScrollbarDistance =
          configs.autoHiddenScrollbarPlaceholderSize;

    case 26:
      if (
        !configs.guessNewOrphanTabAsOpenedByNewTabCommandUrl.includes(
    ...
    case 27:
      if (configs.openAllBookmarksWithGroupAlways !== null)
        configs.suppressGroupTabForStructuredTabsFromBookmarks =
          !configs.openAllBookmarksWithGroupAlways;
    ...
    case 29: {
      ...
      if (
        configs.insertNewChildAt !== configs.$default.insertNewChildAt &&
...
export function tryNotifyNewFeatures() {
  ...
  */
  ...
  if (configs.notifiedFeaturesVersion >= featuresVersion) return;
  ...
  const url = isInitialInstall
    ? Constants.kSHORTHAND_URIS.startup
    : browser.i18n.getMessage("message_startup_history_uri");
  notify({
    url,
    title: browser.i18n.getMessage(`startup_notification_title_${typeSuffix}`),
    message: browser.i18n.getMessage(
      `startup_notification_message_${typeSuffix}${platformSuffix}`
    ),
    timeout: 90 * 1000,
  });
}
...
export async function migrateBookmarkUrls() {
  ...
  for (const key in Constants.kSHORTHAND_URIS) {
    ...
    migrations.push(
      browser.bookmarks.search({ query: url }).then((bookmarks) => {
...
async function migrateBookmarkUrl(bookmark) {
  for (const key in Constants.kSHORTHAND_URIS) {
    const url = Constants.kSHORTHAND_URIS[key].split("?")[0];
    if (!bookmark.url.startsWith(url)) continue;
...
function onBookmarkCreated(_id, bookmark) {
  if (!mObservingBookmarks) return;

  if (bookmark.url) migrateBookmarkUrl(bookmark);
}

async function onBookmarkChanged(id, changeInfo) {
  if (!mObservingBookmarks) return;

  if (changeInfo.url?.startsWith(browser.runtime.getURL(""))) {
    const bookmark = await browser.bookmarks.get(id);
    if (Array.isArray(bookmark)) bookmark.forEach(migrateBookmarkUrl);
    else migrateBookmarkUrl(bookmark);
  }
}
...
configs.$loaded.then(() => {
  configs.$addObserver(async (key) => {
    ...
    await wait(100);
    if (key === "configsVersion" && configs.configsVersion === 0)
      migrateConfigs();
  });
});
