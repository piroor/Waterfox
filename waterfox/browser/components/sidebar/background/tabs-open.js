...
export async function loadURI(uri, options = {}) {
  ...
  try {
    ...
    if (SEARCH_PREFIX_MATCHER.test(uri)) {
      ...
      if (browser.search && typeof browser.search.search === "function")
        searchQuery = query;
      else uri = configs.defaultSearchEngine.replace(/%s/gi, query);
    }
    if (searchQuery) {
...
export async function openURIsInTabs(
  uris,
  {
    windowId,
    insertBefore,
  ...
  const actuallyOpenedTabIds = new Set(
    await Tab.doAndGetNewTabs(async () => {
      await Tab.waitUntilTrackedAll(windowId);
      await TabsMove.waitUntilSynchronized(windowId);
      const startIndex = Tab.calculateNewTabIndex({
        insertAfter,
        insertBefore,
      });
      ...
      if (cookieStoreId) win.toBeOpenedTabsWithCookieStoreId += uris.length;
      if (isOrphan) win.toBeOpenedOrphanTabs += uris.length;
      return Promise.all(
        uris.map(async (uri, index) => {
          ...
          if (uri && typeof uri === "object") {
            // tabs.create() compatible
            if ("active" in uri) params.active = uri.active;
            if ("cookieStoreId" in uri)
              params.cookieStoreId = uri.cookieStoreId;
            if ("discarded" in uri) params.discarded = uri.discarded;
            if ("index" in uri) params.index = uri.index;
            if ("openerTabId" in uri) params.openerTabId = uri.openerTabId;
            if ("openInReaderMode" in uri)
              params.openInReaderMode = uri.openInReaderMode;
            if ("pinned" in uri) params.pinned = uri.pinned;
            if ("selected" in uri) params.active = uri.selected;
            if ("title" in uri) params.title = uri.title;
            uri = uri.uri || uri.url;
          }
          let searchQuery = null;
          if (uri) {
          ...
          if (discarded && !params.active && !("discarded" in params))
            params.discarded = true;
          if (params.url === "about:newtab") delete params.url;
          if (params.url) params.url = sanitizeURL(params.url);
          if (
            !(("url" in params) /* about:newtab case */) ||
            /^about:/.test(params.url)
          )
            params.discarded = false; // discarded tab cannot be opened with any about: URL
          if (!params.discarded)
            // title cannot be set for non-discarded tabs
            params.title = null;
          if (opener && !params.openerTabId) params.openerTabId = opener.id;
          ...
          log("created tab: ", tab);
          if (!tab) throw new Error("tab is already closed");
          if (!opener && parent && !isOrphan)
  ...
  if (
    ...
    openedTabs.every(
      (tab, index) =>
        index === 0 || openedTabs[index - 1].index - tab.index === 1
    )
  ) {
    ...
    for (const tab of openedTabs.slice(0).reverse()) {
      if (lastTab) TabsMove.moveTabInternallyBefore(tab, lastTab);
      lastTab = tab;
    }
    await TabsMove.waitUntilSynchronized(windowId);
  }
...
function sanitizeURL(url) {
  if (ALLOWED_URL_MATCHER.test(url)) return url;
...
function onMessage(message, openerTab) {
  switch (message.type) {
    ...
    case Constants.kCOMMAND_OPEN_TAB:
      if (!message.parentId && openerTab) message.parentId = openerTab.id;
      if (!message.windowId && openerTab) message.windowId = openerTab.windowId;
...
SidebarConnection.onMessage.addListener((_windowId, message) => {
  onMessage(message);
});
