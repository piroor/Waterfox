...
Window.onInitialized.addListener((win) => {
  browser.tabs
    ...
    .then((activeTabs) => {
      ...
      if (activeTabs.length > 0 && !win.lastActiveTab)
...
Tab.onActivating.addListener(async (tab, info = {}) => {
  // return false if the activation should be canceled
  log("Tabs.onActivating ", {
    tab: dumpTab(tab),
    info,
  ...
  if (tab.$TST.collapsed) {
    if (!tab.$TST.parent) {
      ...
      if (!shouldSkipCollapsed) await handleNewActiveTab(tab, info);
    }
    if (shouldSkipCollapsed) {
      log(
        "=> reaction for focusing collapsed descendant while Ctrl-Tab/Ctrl-Shift-Tab"
      );
      let successor = tab.$TST.nearestVisibleAncestorOrSelf;
      if (!successor)
        // this seems invalid case...
        return false;
      log("successor = ", successor.id);
      if (
        shouldSkipCollapsed &&
        (win.lastActiveTab === successor.id ||
          successor.$TST.descendants.some(
            (tab) => tab.id === win.lastActiveTab
          )) &&
        focusDirection > 0
      ) {
        log(
          "=> redirect successor (focus moved from the successor itself or its descendants)"
        );
        ...
        if (
          ...
          successor =
            ...
            successor;
        if (!successor) successor = Tab.getFirstVisibleTab(tab.windowId);
        log("=> ", successor.id);
      } else if (
        !mTabSwitchedByShortcut && // intentional focus to a discarded tabs by Ctrl-Tab/Ctrl-Shift-Tab is always allowed!
        successor.discarded &&
        configs.avoidDiscardedTabToBeActivatedIfPossible
      ) {
      ...
      if (allowed) {
        win.lastActiveTab = successor.id;
        if (mMaybeTabSwitchingByShortcut) setupDelayedExpand(successor);
        ...
        if (tab.discarded)
          tab.$TST.temporaryMetadata.set(
            "discardURLAfterCompletelyLoaded",
            tab.url
          );
        return false;
      } else {
  ...
  if (mMaybeTabSwitchingByShortcut) setupDelayedExpand(tab);
  else
    tryHighlightBundledTab(tab, {
      ...info,
      shouldSkipCollapsed,
...
async function handleNewActiveTab(tab, { allowed, silently } = {}) {
  ...
  if (canExpandTree && allowed !== false) {
    const cache = {};
    const allowed = await TSTAPI.tryOperationAllowed(
      tab.active
    ...
    if (!allowed) return;
    if (canCollapseTree && configs.autoExpandIntelligently)
...
async function tryHighlightBundledTab(
  tab,
  { shouldSkipCollapsed, allowed, silently } = {}
) {
...
Tab.onUpdated.addListener((tab, changeInfo = {}) => {
  if ("url" in changeInfo) {
    if (
      tab.$TST.temporaryMetadata.has("discardURLAfterCompletelyLoaded") &&
      tab.$TST.temporaryMetadata.get("discardURLAfterCompletelyLoaded") !==
        changeInfo.url
    )
...
Tab.onStateChanged.addListener((tab) => {
  ...
  if (typeof browser.tabs.discard === "function") {
    if (
      ...
      log(
        "Try to discard accidentally restored tab (on restored) ",
        dumpTab(tab)
      );
      wait(configs.autoDiscardTabForUnexpectedFocusDelay).then(() => {
        if (!TabsStore.ensureLivingTab(tab) || tab.active) return;
        if (tab.status === "complete")
          browser.tabs
            .discard(tab.id)
            .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
        else tab.$TST.temporaryMetadata.set("discardOnCompletelyLoaded", true);
      });
    } else if (
      tab.$TST.temporaryMetadata.has("discardOnCompletelyLoaded") &&
      !tab.active
    ) {
...
async function setupDelayedExpand(tab) {
  ...
  if (
    !configs.autoExpandOnTabSwitchingShortcuts ||
    !tab.$TST.hasChild ||
    !tab.$TST.subtreeCollapsed ||
  ...
  tab.$TST.temporaryMetadata.set(
    "delayedExpand",
    setTimeout(async () => {
      if (!tab.$TST.temporaryMetadata.has("delayedExpand")) {
        // already canceled
...
function cancelDelayedExpand(tab) {
  if (!tab || !tab.$TST.temporaryMetadata.has("delayedExpand")) return;
...
Tab.onCollapsedStateChanged.addListener((tab, info = {}) => {
  tab.$TST.toggleState(Constants.kTAB_STATE_COLLAPSED_DONE, info.collapsed, {
    broadcast: false,
  });
});
...
function onMessage(message, sender) {
  ...
  switch (message.type) {
    case Constants.kNOTIFY_TAB_MOUSEDOWN:
      mMaybeTabSwitchingByShortcut = mTabSwitchedByShortcut = false;
      break;
    ...
    case Constants.kCOMMAND_NOTIFY_MAY_START_TAB_SWITCH:
      {
        if (
          message.modifier !==
          (configs.accelKey || (isMacOS() ? "meta" : "control"))
        )
          return;
        ...
        if (sender.tab)
          mLastTabsCountInWindow.set(
            sender.tab.windowId,
            Tab.getAllTabs(sender.tab.windowId).length
          );
      }
      break;
