...
const EXTERNAL_TOP_LEVEL_ITEM_MATCHER =
  /^external-top-level-item:([^:]+):(.+)$/;
...
const mItemsById = {
  ...
  context_toggleMuteTab: {
    ...
    titleMultiselectedMute: browser.i18n.getMessage(
      "tabContextMenu_mute_label_multiselected"
    ),
    titleMultiselectedUnmute: browser.i18n.getMessage(
  ...
  context_undoCloseTab: {
    ...
    titleMultipleTabsRestorable: browser.i18n.getMessage(
...
function getItemPlacementSignature(item) {
  if (item.placementSignature) return item.placementSignature;
  ...
  for (const id of itemIds) {
    ...
    if (item.type === "separator") {
      ...
      for (const id of itemIds) {
        ...
        if (
          getItemPlacementSignature(item) !==
          getItemPlacementSignature(possibleSibling)
        ) {
          if (beforeSeparator) continue;
          else break;
        }
        if (id === item.id) {
        ...
        if (beforeSeparator) {
          ...
          if (possibleSibling.type === "separator") break;
          else item.followingItems.push(id);
        }
      }
    }
    const info = {
    ...
    if (item.parentId) info.parentId = item.parentId;
    if (!item.fakeMenu) browser.menus.create(info);
    onMessageExternal(
      {
...
function updateContextualIdentities() {
  for (const item of mContextualIdentityItems) {
    const id = item.id;
    if (id in mItemsById) delete mItemsById[id];
  ...
  ContextualIdentities.forEach((identity) => {
    ...
    if (identity.iconUrl) item.icons = { 16: identity.iconUrl };
...
export async function updateSendToDeviceItems(parentId, { manage } = {}) {
  ...
  if (signature === mLastDevicesSignature.get(parentId)) return false;
...
async function updateSharingServiceItems(parentId, contextTab) {
  if (!mSharingService || !contextTab) return false;

  const services = await mSharingService.listServices(contextTab);
  const signature = JSON.stringify(services);
  if (signature === mLastSharingServicesSignature.get(parentId)) return false;
...
function updateItem(id, state = {}) {
  ...
  if ("checked" in state) updateInfo.checked = state.checked;
  const title = String(
    state.title ||
  ...
  if (!modified)
    modified =
      updateInfo.visible !== item.lastVisible ||
...
function updateSeparator(id, options = {}) {
  const item = mItemsById[id];
  const visible =
    (options.hasVisiblePreceding || hasVisiblePrecedingItem(item)) &&
    (options.hasVisibleFollowing ||
      item.followingItems.some(
        (id) =>
          mItemsById[id].type !== "separator" && mItemsById[id].lastVisible
      ));
  return updateItem(id, { visible });
}
function hasVisiblePrecedingItem(separator) {
  return (
    separator.precedingItems.some(
      (id) => mItemsById[id].type !== "separator" && mItemsById[id].lastVisible
    ) ||
...
async function onShown(info, contextTab) {
  ...
  try {
    ...
    const hasMultipleNormalTabs = Tab.hasMultipleTabs(windowId, {
      normal: true,
    });
    ...
    updateItem("context_topLevel_reloadDescendants", {
      visible:
        emulate && !!contextTab && configs.context_topLevel_reloadDescendants,
      enabled: hasChild,
      multiselected,
    }) && modifiedItemsCount++;
    updateItem("context_unblockAutoplay", {
      ...
      title:
        contextTab &&
        Commands.getMenuItemTitle(mItemsById.context_unblockAutoplay, {
          multiselected,
        }),
    }) && modifiedItemsCount++;
    updateItem("context_topLevel_unblockAutoplayTree", {
      ...
      title:
        contextTab &&
        Commands.getMenuItemTitle(
          mItemsById.context_topLevel_unblockAutoplayTree,
          {
            multiselected,
          }
        ),
    ...
    updateItem("context_topLevel_unblockAutoplayDescendants", {
      visible:
        ...
        configs.context_topLevel_unblockAutoplayDescendants,
      multiselected,
      title:
        contextTab &&
        Commands.getMenuItemTitle(
          mItemsById.context_topLevel_unblockAutoplayDescendants,
          {
            multiselected,
          }
        ),
    ...
    updateItem("context_toggleMuteTab", {
      ...
      title:
        contextTab &&
        Commands.getMenuItemTitle(mItemsById.context_toggleMuteTab, {
          multiselected,
    ...
    updateItem("context_topLevel_toggleMuteTree", {
      visible:
        emulate && !!contextTab && configs.context_topLevel_toggleMuteTree,
      enabled: hasChild,
      multiselected,
      title: Commands.getMenuItemTitle(
        mItemsById.context_topLevel_toggleMuteTree,
        { multiselected, hasUnmutedTab, hasUnmutedDescendant }
      ),
      hasUnmutedTab,
    ...
    updateItem("context_topLevel_toggleMuteDescendants", {
      visible:
        emulate &&
        !!contextTab &&
        configs.context_topLevel_toggleMuteDescendants,
      enabled: hasChild,
      multiselected,
      title: Commands.getMenuItemTitle(
        mItemsById.context_topLevel_toggleMuteDescendants,
        { multiselected, hasUnmutedTab, hasUnmutedDescendant }
      ),
      hasUnmutedTab,
      hasUnmutedDescendant,
    ...
    updateItem("context_topLevel_toggleSticky", {
      ...
      title:
        contextTab &&
        Commands.getMenuItemTitle(mItemsById.context_topLevel_toggleSticky, {
          multiselected,
    ...
    updateItem("context_moveTabToStart", {
      enabled:
        ...
        !!contextTab &&
        hasMultipleTabs &&
        (previousSiblingTab || previousTab) &&
        (previousSiblingTab || previousTab).pinned === contextTab.pinned,
      multiselected,
    }) && modifiedItemsCount++;
    updateItem("context_moveTabToEnd", {
      enabled:
        ...
        !!contextTab &&
        hasMultipleTabs &&
        (nextSiblingTab || nextTab) &&
        (nextSiblingTab || nextTab).pinned === contextTab.pinned,
      multiselected,
    }) && modifiedItemsCount++;
    ...
    updateItem("context_topLevel_sendTreeToDevice", {
      visible:
        ...
        !!contextTab &&
        contextTabs.filter(Sync.isSendableTab).length > 0 &&
        configs.context_topLevel_sendTreeToDevice &&
        hasChild,
      enabled: hasChild,
      multiselected,
    }) && modifiedItemsCount++;
    ...
    if (contextTab && !contextTab.incognito) {
      for (const item of mContextualIdentityItems.values()) {
        const id = item.id;
        let visible;
        if (!emulate) visible = false;
        else if (id === "context_reopenInContainer_separator")
          visible =
            !!contextTab && contextTab.cookieStoreId !== "firefox-default";
        else
          visible =
            !!contextTab &&
            id !== `context_reopenInContainer:${contextTab.cookieStoreId}`;
        updateItem(id, { visible }) && modifiedItemsCount++;
        if (visible) showContextualIdentities = true;
      }
    }
    updateItem("context_reopenInContainer", {
      visible:
        emulate &&
        !!contextTab &&
        showContextualIdentities &&
        !contextTab.incognito,
    ...
    updateItem("context_selectAllTabs", {
      ...
      enabled:
        contextTab &&
        Tab.getSelectedTabs(windowId).length !==
          Tab.getVisibleTabs(windowId).length,
      multiselected,
    }) && modifiedItemsCount++;
    ...
    updateItem("context_topLevel_collapseTreeRecursively", {
      visible:
        emulate &&
        !!contextTab &&
        configs.context_topLevel_collapseTreeRecursively,
      enabled: hasChild,
      multiselected,
    }) && modifiedItemsCount++;
    updateItem("context_topLevel_collapseAll", {
      visible:
        ...
        !multiselected &&
        !!contextTab &&
        configs.context_topLevel_collapseAll,
    }) && modifiedItemsCount++;
    ...
    updateItem("context_topLevel_expandTreeRecursively", {
      visible:
        emulate &&
        !!contextTab &&
        configs.context_topLevel_expandTreeRecursively,
      enabled: hasChild,
      multiselected,
    }) && modifiedItemsCount++;
    updateItem("context_topLevel_expandAll", {
      visible:
        ...
        !multiselected &&
        !!contextTab &&
        configs.context_topLevel_expandAll,
    }) && modifiedItemsCount++;
    ...
    updateItem("context_topLevel_closeDescendants", {
      visible:
        emulate && !!contextTab && configs.context_topLevel_closeDescendants,
      enabled: hasChild,
      multiselected,
    }) && modifiedItemsCount++;
    ...
    const undoCloseTabLabel =
      mItemsById.context_undoCloseTab[
        configs.undoMultipleTabsClose && mMultipleTabsRestorable
          ? "titleMultipleTabsRestorable"
          : "titleRegular"
      ];
    ...
    updateSeparator("context_separator:afterSelectAllTabs") &&
      modifiedItemsCount++;
    updateSeparator("context_separator:afterCollapseExpand") &&
      modifiedItemsCount++;
    ...
    updateSeparator("lastSeparatorBeforeExtraItems", {
      hasVisibleFollowing:
        contextTab &&
        flattenExtraItems.some(
          (item) => !item.parentId && item.visible !== false
        ),
    }) && modifiedItemsCount++;
    ...
    await Promise.all([
      ...
      mItemsById.context_topLevel_sendTreeToDevice.lastVisible &&
        updateSendToDeviceItems("context_topLevel_sendTreeToDevice"),
      ...
      for (const modified of results) {
        if (modified) modifiedItemsCount++;
      }
    });
    if (mLastContextTabId !== contextTabId) return; // Skip further operations if the menu was already reopened on a different context tab.
...
function onOverriddenMenuShown(info, contextTab, windowId) {
  if (!mLastOverriddenContextOwner) {
    for (const itemId of Object.keys(mItemsById)) {
      if (!mItemsById[itemId].lastVisible) continue;
  ...
  for (const item of mExtraItems.get(mLastOverriddenContextOwner) || []) {
    if (item.$topLevel && item.lastVisible) {
...
function cleanupOverriddenMenu() {
  if (!mLastOverriddenContextOwner) return 0;
  ...
  for (const item of mExtraItems.get(owner) || []) {
    if (item.$topLevel && item.lastVisible) {
      browser.menus.update(getExternalTopLevelItemId(owner, item.id), {
        visible: false,
      });
      modifiedItemsCount++;
    }
...
function onHidden() {
  ...
  if (mLastOverriddenContextOwner && owner === mLastOverriddenContextOwner) {
...
async function onClick(info, contextTab) {
  ...
  const isMultiselected = contextTab
    ? contextTab.$TST.multiselected
    : multiselectedTabs.length > 1;
  if (!isMultiselected) multiselectedTabs = null;
  ...
  switch (info.menuItemId.replace(/^noContextTab:/, "")) {
    ...
    case "context_toggleMuteTab":
      {
        ...
        if (multiselectedTabs) {
          for (const tab of multiselectedTabs) {
            browser.tabs
              .update(tab.id, { muted: unmuted })
              .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
          }
        } else {
          browser.tabs
            .update(contextTab.id, { muted: unmuted })
            .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
        }
      }
      break;
    case "context_pinTab":
      if (multiselectedTabs) {
        ...
        browser.tabs
          .update(contextTab.id, { pinned: true })
          .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
      }
      break;
    case "context_unpinTab":
      if (multiselectedTabs) {
        for (const tab of multiselectedTabs) {
          browser.tabs
            .update(tab.id, { pinned: false })
            .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
        }
      } else {
        browser.tabs
          .update(contextTab.id, { pinned: false })
          .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
      }
      break;
    case "context_toggleSticky":
      Commands.toggleSticky(
        multiselectedTabs,
        !(contextTab || activeTab).$TST.sticky
      );
      break;
    ...
    case "context_shareTabURL":
      if (mSharingService) mSharingService.share(contextTab);
      break;
    case "context_shareTabURL:more":
      if (mSharingService) mSharingService.openPreferences();
      break;
    ...
    case "context_selectAllTabs":
      {
        ...
        TabsInternalOperation.highlightTabs(
          [activeTab].concat(
            mapAndFilter(tabs, (tab) => (!tab.active ? tab : undefined))
          )
        ).catch(ApiTabs.createErrorSuppressor());
      }
      break;
    ...
    case "context_closeTabsToTheStart":
      {
        ...
        for (const tab of tabs) {
          if (keptTabIds.has(tab.id)) break;
          if (!tab.pinned && !tab.hidden) closeTabs.push(Tab.get(tab.id));
        }
        const canceled =
          (await browser.runtime
            .sendMessage({
    ...
    case "context_closeTabsToTheEnd":
      {
        ...
        const closeTabs = [];
        const keptTabIds = new Set(
          (multiselectedTabs ? multiselectedTabs : [contextTab]).reduce(
            (tabIds, tab, _index) => {
              if (tab.$TST.subtreeCollapsed)
                tabIds.push(
                  tab.id,
                  ...tab.$TST.descendants.map((tab) => tab.id)
                );
              else tabIds.push(tab.id);
              return tabIds;
            },
            []
          )
    ...
    case "context_closeOtherTabs":
      {
        ...
        const closeTabs = mapAndFilter(
          tabs,
          (tab) =>
            (!tab.pinned &&
              !tab.hidden &&
              !keptTabIds.has(tab.id) &&
              Tab.get(tab.id)) ||
            undefined
        );
        const canceled =
    ...
    case "context_undoCloseTab":
      {
        const sessions = await browser.sessions
          .getRecentlyClosed({ maxResults: 1 })
          .catch(ApiTabs.createErrorHandler());
        if (sessions.length && sessions[0].tab)
          browser.sessions
            .restore(sessions[0].tab.sessionId)
            .catch(ApiTabs.createErrorSuppressor());
      }
      break;
    case "context_closeTab":
      {
        const closeTabs = (
          multiselectedTabs ||
          TreeBehavior.getClosingTabsFromParent(contextTab, {
    ...
    default:
      {
        const contextualIdentityMatch = info.menuItemId.match(
          /^context_reopenInContainer:(.+)$/
        );
        if (contextTab && contextualIdentityMatch)
          Commands.reopenInContainer(contextTab, contextualIdentityMatch[1]);

        const shareTabsMatch = info.menuItemId.match(
          /^context_shareTabURL:service:(.+)$/
        );
        if (mSharingService && contextTab && shareTabsMatch)
          mSharingService.share(contextTab, shareTabsMatch[1]);

        const sendTabsToDeviceMatch = info.menuItemId.match(
          /^context_sendTabsToDevice:device:(.+)$/
        );
        if (contextTab && sendTabsToDeviceMatch)
          Sync.sendTabsToDevice(multiselectedTabs || [contextTab], {
            to: sendTabsToDeviceMatch[1],
          });
        const sendTreeToDeviceMatch = info.menuItemId.match(
          /^context_topLevel_sendTreeToDevice:device:(.+)$/
        );
        if (contextTab && sendTreeToDeviceMatch)
          Sync.sendTabsToDevice(multiselectedTabs || [contextTab], {
        ...
        if (EXTERNAL_TOP_LEVEL_ITEM_MATCHER.test(info.menuItemId)) {
          ...
          if (owner === browser.runtime.id) {
            ...
            const cache = {};
            TSTAPI.sendMessage(owner, message, {
              tabProperties: ["tab"],
              cache,
              isContextTab: true,
            }).catch(ApiTabs.createErrorSuppressor());
...
function reserveNotifyUpdated() {
  return new Promise((resolve, _aReject) => {
    ...
    if (mReservedNotifyUpdate) clearTimeout(mReservedNotifyUpdate);
...
function reserveRefresh() {
  if (reserveRefresh.reserved) clearTimeout(reserveRefresh.reserved);
  ...
  switch (message.type) {
    ...
    case Constants.kCOMMAND_NOTIFY_CONTEXT_ITEM_CHECKED_STATUS_CHANGED:
      for (const itemData of mExtraItems.get(message.ownerId)) {
        if (!itemData.id !== message.id) continue;
...
export function onMessageExternal(message, sender) {
  ...
  switch (message.type) {
    ...
    case TSTAPI.kFAKE_CONTEXT_MENU_CREATE:
      {
        log("TSTAPI.kCONTEXT_MENU_CREATE:", message, {
          id: sender.id,
          url: sender.url,
        });
        ...
        const parent =
          params.parentId &&
          items.filter((item) => item.id === params.parentId)[0];
        if (params.parentId && !parent) break;
        let shouldAdd = true;
        if (params.id) {
        ...
        if (sender.id !== browser.runtime.id && params.$topLevel) {
          ...
          const createParams = {
            ...
            visible,
            viewTypes: ["sidebar", "tab", "popup"],
            contexts: (params.contexts || []).filter(
              (context) => context === "tab" || context === "bookmark"
            ),
            documentUrlPatterns: SIDEBAR_URL_PATTERN,
          };
          if (params.parentId)
            createParams.parentId = getExternalTopLevelItemId(
              sender.id,
              params.parentId
            );
          for (const property of SAFE_MENU_PROPERTIES) {
            if (property in params) createParams[property] = params[property];
          }
    ...
    case TSTAPI.kFAKE_CONTEXT_MENU_UPDATE:
      {
        log("TSTAPI.kCONTEXT_MENU_UPDATE:", message, {
          id: sender.id,
          url: sender.url,
        });
        const items = getItemsFor(sender.id);
        for (let i = 0, maxi = items.length; i < maxi; i++) {
          const item = items[i];
          if (item.id !== message.params[0]) continue;
          ...
          if (sender.id !== browser.runtime.id && item.$topLevel) {
    ...
    case TSTAPI.kFAKE_CONTEXT_MENU_REMOVE:
      {
        log("TSTAPI.kCONTEXT_MENU_REMOVE:", message, {
          id: sender.id,
          url: sender.url,
        });
        ...
        if (!item) break;
        const parent =
          item.parentId && items.filter((item) => item.id === item.parentId)[0];
