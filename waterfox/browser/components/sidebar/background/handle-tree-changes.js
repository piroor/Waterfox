  ...
  reserveDetachHiddenTab.reserved = setTimeout(async () => {
    ...
    for (const tab of tabs) {
      ...
      for (const descendant of tab.$TST.descendants) {
        if (descendant.hidden) continue;
        const nearestVisibleAncestor = descendant.$TST.ancestors.find(
          (ancestor) => !ancestor.hidden && !tabs.has(ancestor)
        );
        if (
        ...
        for (const ancestor of descendant.$TST.ancestors) {
          if (!ancestor.hidden && !ancestor.$TST.collapsed) break;
          if (!ancestor.$TST.subtreeCollapsed) continue;
        ...
        if (nearestVisibleAncestor) {
          log(
            ` => reattach descendant ${descendant.id} to ${nearestVisibleAncestor.id}`
          );
      ...
      if (tab.$TST.hasParent && !tab.$TST.parent.hidden) {
...
Tab.onHidden.addListener((tab) => {
  if (configs.fixupTreeOnTabVisibilityChanged) reserveDetachHiddenTab(tab);
});
...
function reserveAttachShownTab(tab) {
  ...
  reserveAttachShownTab.reserved = setTimeout(async () => {
    ...
    for (const tab of tabs) {
      if (!TabsStore.ensureLivingTab(tab) || tab.$TST.hasParent) {
...
Tab.onShown.addListener((tab) => {
  if (configs.fixupTreeOnTabVisibilityChanged) reserveAttachShownTab(tab);
});
...
Tree.onSubtreeCollapsedStateChanged.addListener((tab, _info) => {
  if (mInitialized) TreeStructure.reserveToSaveTreeStructure(tab.windowId);
  BackgroundCache.markWindowCacheDirtyFromTab(
    tab,
