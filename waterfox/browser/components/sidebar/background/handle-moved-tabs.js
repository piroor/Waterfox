...
Tab.onCreated.addListener((tab, info = {}) => {
  if (
    ...
    (info.duplicated ||
      ...
      (tab.openerTabId && tab.$TST.parent === Tab.get(tab.openerTabId)))
  ) {
  ...
  if (
    !(info.positionedBySelf || info.movedBySelfWhileCreation) &&
...
Tab.onMoving.addListener((tab, moveInfo) => {
  ...
  const positionControlled =
    configs.insertNewChildAt !== Constants.kINSERT_NO_CONTROL;
  if (
    !isNewlyOpenedTab ||
    !positionControlled ||
  ...
  if (!tab.$TST.openerTab) return true;

  log(
    "onTabMove for new child tab: move back " +
      moveInfo.toIndex +
      " => " +
      moveInfo.fromIndex
  );
...
async function tryFixupTreeForInsertedTab(tab, moveInfo = {}) {
  ...
  if (
    ...
    const replacedGroupTab =
      ...
      Constants.kPARENT_TAB_OPERATION_BEHAVIOR_REPLACE_WITH_GROUP_TAB
        ? await TabsGroup.tryReplaceTabWithGroup(tab, {
            insertBefore: tab.$TST.firstChild,
          })
        : null;
    if (!replacedGroupTab && tab.$TST.hasChild) {
      if (tab.$TST.isGroupTab) await TabsGroup.clearTemporaryState(tab);
      await Tree.detachAllChildren(tab, {
        behavior: parentTabOperationBehavior,
        nearestFollowingRootTab:
          tab.$TST.firstChild.$TST.nearestFollowingRootTab,
        broadcast: true,
      });
    }
    if (tab.$TST.parentId)
  ...
  log(
    "The tab can be placed inside existing tab unexpectedly, so now we are trying to fixup tree."
  );
  ...
  if (!allowed) {
    ...
    switch (action.action) {
      ...
      default:
        log(
          "tryFixupTreeForInsertedTab: apply action for unattached tab: ",
          tab,
          action
        );
        await action.apply();
...
function reserveToEnsureRootTabVisible(tab) {
  ...
  reserveToEnsureRootTabVisible.reserved = setTimeout(() => {
    ...
    for (const tab of tabs) {
      if (!tab.$TST || tab.$TST.parent || !tab.$TST.collapsed) continue;
...
TreeStructure.onTabAttachedFromRestoredInfo.addListener((tab, moveInfo) => {
  tryFixupTreeForInsertedTab(tab, moveInfo);
});
