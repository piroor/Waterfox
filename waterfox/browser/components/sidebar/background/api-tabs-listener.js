...
function addTabOperationQueue(metric = null) {
  ...
  queue.then(() => {
    ...
    if (metric) metric.add("TabOperationQueue proceeded");
  });
...
function warnTabDestroyedWhileWaiting(tabId, tab) {
  if (configs.debug)
    console.log(
      `WARNING: tab ${tabId} is destroyed while waiting. `,
      tab,
      new Error().stack
    );
}
...
async function onActivated(activeInfo) {
  ...
  TabsStore.activeTabInWindow.set(
    activeInfo.windowId,
    Tab.get(activeInfo.tabId)
  );

  const [onCompleted, previous] = addTabOperationQueue();
  if (!configs.acceleratedTabOperations && previous) await previous;

  try {
    const win = Window.init(activeInfo.windowId);

    const byInternalOperation = win.internallyFocusingTabs.has(
      activeInfo.tabId
    );
    win.internallyFocusingTabs.delete(activeInfo.tabId);
    const byMouseOperation = win.internallyFocusingByMouseTabs.has(
      activeInfo.tabId
    );
    ...
    if (!newActiveTab || !TabsStore.ensureLivingTab(newActiveTab)) {
    ...
    if (!TabsStore.ensureLivingTab(newActiveTab)) {
      // it can be removed while waiting
      onCompleted();
      warnTabDestroyedWhileWaiting(activeInfo.tabId);
      return;
    }
    ...
    // don't do await if not needed, to process things synchronously
    if (onActivatedReuslt instanceof Promise) await onActivatedReuslt;
...
async function onUpdated(tabId, changeInfo, tab) {
  ...
  if (!Tab.isTracked(tabId)) await Tab.waitUntilTracked(tabId);
  ...
  try {
    ...
    if (!updatedTab || !TabsStore.ensureLivingTab(updatedTab)) {
      onCompleted();
    ...
    */
    for (const key of Object.keys(changeInfo)) {
      ...
      if ("key" in updatedTab) oldState[key] = updatedTab[key];
      if (key === "openerTabId") {
        if (changeInfo.openerTabId === updatedTab.openerTabId)
          // already processed
          continue;
        log(
          `openerTabId of ${tabId} is changed by someone (notified via changeInfo)!: ${updatedTab.openerTabId} (original) => ${changeInfo[key]} (changed by someone)`,
    ...
    if (changeInfo.url || changeInfo.status === "complete") {
      ...
      browser.tabs
        ...
        .catch(
          ApiTabs.createErrorSuppressor(
            ApiTabs.handleMissingTabError // the tab can be closed while waiting
          )
        );
    }
    const updatedOpenerTabId =
      updatedTab.$TST.temporaryMetadata.get("updatedOpenerTabId");
    if (
      configs.enableWorkaroundForBug1409262 &&
      tab.openerTabId !== updatedOpenerTabId
    ) {
      log(
        `openerTabId of ${tabId} is changed by someone!: ${updatedOpenerTabId} (original) => ${tab.openerTabId} (changed by someone) `,
        configs.debug && new Error().stack
      );
      updatedTab.$TST.temporaryMetadata.set(
        "updatedOpenerTabId",
        (updatedTab.openerTabId = changeInfo.openerTabId = tab.openerTabId)
      );
    }
    ...
    // don't do await if not needed, to process things synchronously
    if (onUpdatedResult instanceof Promise) await onUpdatedResult;
...
async function onHighlighted(highlightInfo) {
  ...
  const unifiedHighlightedTabs = new Set([
    ...win.highlightingTabs,
    ...highlightInfo.tabIds,
  ]);
  if (unifiedHighlightedTabs.size === win.highlightingTabs.size) {
    log(
      `Internal highlighting is in progress: ${Math.ceil((highlightInfo.tabIds.length / win.highlightingTabs.size) * 100)} %`
    );
  ...
  if (
    (mLastHighlightedCount.get(highlightInfo.windowId) || 0) <= 1 &&
  ...
  timer = setTimeout(() => {
    ...
    TabsUpdate.updateTabsHighlighted(highlightInfo);
    mLastHighlightedCount.set(
      highlightInfo.windowId,
      highlightInfo.tabIds.length
    );
    SidebarConnection.sendMessage({
      type: Constants.kCOMMAND_NOTIFY_HIGHLIGHTED_TABS_CHANGED,
      windowId: highlightInfo.windowId,
...
async function onNewTabTracked(tab, info) {
  ...
  if (isNewTabCommandTab) tab.$isNewTabCommandTab = true;
  ...
  if (fromExternal) tab.$TST.addState(Constants.kTAB_STATE_FROM_EXTERNAL);
  ...
  log(`onNewTabTracked(${dumpTab(tab)}): `, tab, {
    win,
    positionedBySelf,
  ...
  const onTreeModified = (_child, _info) => {
    if (!treeForActionDetection || !TabsStore.ensureLivingTab(tab)) return;
    treeForActionDetection = Tree.snapshotForActionDetection(tab);
    log(
      "Tree modification is detected while waiting. Cached tree for action detection is updated: ",
  ...
  try {
    ...
    log(`onNewTabTracked(${dumpTab(tab)}): `, {
      duplicated,
      restored,
      skipFixupTree,
    });
    if (duplicated) tab.$TST.addState(Constants.kTAB_STATE_DUPLICATED);

    const maybeNeedToFixupTree =
      (info.mayBeReplacedWithContainer ||
        (!duplicated && !restored && !skipFixupTree)) &&
    ...
    treeForActionDetection = maybeNeedToFixupTree
      ? Tree.snapshotForActionDetection(tab)
      : null;

    if (bypassTabControl) win.bypassTabControlCount--;
    if (isNewTabCommandTab) win.toBeOpenedNewTabCommandTab--;
    if (positionedBySelf) win.toBeOpenedTabsWithPositions--;
    if (openedWithCookieStoreId) win.toBeOpenedTabsWithCookieStoreId--;
    if (maybeOrphan) win.toBeOpenedOrphanTabs--;
    if (duplicatedInternally) win.duplicatingTabsCount--;
    ...
    if (restored) {
      ...
      if (!win.promisedAllTabsRestored) {
        log(
          `onNewTabTracked(${dumpTab(tab)}): Maybe starting to restore window`
        );
    ...
    if (TabsStore.ensureLivingTab(tab)) {
      ...
      setTimeout(() => {
        ...
        if (!TabsStore.windows.get(tab.windowId))
          // it can be removed while waiting
          return;
    ...
    if (!TabsStore.ensureLivingTab(tab)) {
      ...
      metric.add("untracked");
      log(
        "  tab is untracked while tracking after notified to sidebar, metric: ",
        metric
      );
      return;
    }
    ...
    if (TSTAPI.hasListenerForMessageType(TSTAPI.kNOTIFY_NEW_TAB_PROCESSED)) {
      ...
      TSTAPI.broadcastMessage(
        ...
        { tabProperties: ["tab", "originalTab"], cache }
      ).catch((_error) => {});
    ...
    const renewedTab = await browser.tabs
      .get(tab.id)
      .catch(ApiTabs.createErrorHandler(ApiTabs.handleMissingTabError));
    metric.add("renewedTab");
    if (!renewedTab) {
      log(
        `onNewTabTracked(${dumpTab(tab)}): tab ${tab.id} is closed while tracking`
      );
      onCompleted(uniqueId);
      tab.$TST.rejectOpened();
      Tab.untrack(tab.id);
    ...
    for (const key of Object.keys(renewedTab)) {
      ...
      if (
        ...
        log(
          `openerTabId of ${tab.id} is different from the raw value but it has been updated by TST while attaching, so don't detect as updated for now`
        );
        continue;
      }
    ...
    if ("openerTabId" in changedProps) {
      log(
        `openerTabId of ${tab.id} is changed while creating: ${tab.openerTabId} (changed by someone) => ${changedProps.openerTabId} (original) `,
        configs.debug && new Error().stack
      );
      if (
        duplicated &&
        ...
        log(
          `restore original openerTabId of ${tab.id} for duplicated active tab: ${updatedOpenerTabId}`
        );
    ...
    if (renewedTab.active && currentActiveTab.id !== tab.id) {
      onActivated({
        tabId: tab.id,
        windowId: tab.windowId,
...
function checkRecycledTab(windowId) {
  ...
  for (const tab of possibleRecycledTabs) {
    if (!TabsStore.ensureLivingTab(tab)) continue;
...
async function onRemoved(tabId, removeInfo) {
  ...
  try {
    ...
    log(
      "tabs.onRemoved, tab is found: ",
      oldTab,
      `openerTabId=${oldTab.openerTabId}`
    );

    const nearestTabs = [
      oldTab.$TST.unsafePreviousTab,
    ...
    // don't do await if not needed, to process things synchronously
    if (onRemovingResult instanceof Promise) await onRemovingResult;
    ...
    // don't do await if not needed, to process things synchronously
    if (onRemovedReuslt instanceof Promise) await onRemovedReuslt;
...
async function onMoved(tabId, moveInfo) {
  ...
  const maybeInternalOperation =
    internalExpectedIndex < 0 || internalExpectedIndex === moveInfo.toIndex;
  ...
  try {
    ...
    let oldPreviousTab = movedTab.hidden
      ? movedTab.$TST.unsafePreviousTab
      : movedTab.$TST.previousTab;
    let oldNextTab = movedTab.hidden
      ? movedTab.$TST.unsafeNextTab
      : movedTab.$TST.nextTab;
    if (
      movedTab.index !== moveInfo.toIndex ||
      oldPreviousTab?.index === movedTab.index - 1 ||
      oldNextTab?.index === movedTab.index + 1
    ) {
      // already moved
      oldPreviousTab = Tab.getTabAt(
        moveInfo.windowId,
        moveInfo.toIndex < moveInfo.fromIndex
          ? moveInfo.fromIndex
          : moveInfo.fromIndex - 1
      );
      oldNextTab = Tab.getTabAt(
        moveInfo.windowId,
        moveInfo.toIndex < moveInfo.fromIndex
          ? moveInfo.fromIndex + 1
          : moveInfo.fromIndex
    ...
    const alreadyMoved =
      expectedIndex < 0 || expectedIndex === moveInfo.toIndex;
    if (win.alreadyMovedTabs.has(tabId)) win.alreadyMovedTabs.delete(tabId);
    ...
    if (!canceled && TabsStore.ensureLivingTab(movedTab)) {
      ...
      if (!alreadyMoved && movedTab.$TST.nextTab !== nextTab) {
        ...
        log(
          "Tab nodes rearranged by tabs.onMoved listener:\n" +
            (!configs.debug
              ? ""
              : toLines(
      ...
      // don't do await if not needed, to process things synchronously
      if (onMovedResult instanceof Promise) await onMovedResult;
      if (!alreadyMoved)
        SidebarConnection.sendMessage({
    ...
    if (win.internalMovingTabs.has(tabId)) win.internalMovingTabs.delete(tabId);
    completelyMoved();
...
async function onAttached(tabId, attachInfo) {
  ...
  try {
    ...
    if (!attachedTab) {
      ...
      const newWindow = await browser.windows
        .get(attachInfo.newWindowId, { populate: true })
        .then((_error) => null);
      attachedTab = newWindow?.tabs.find((tab) => tab.id === tabId);
      if (!newWindow || !attachedTab) {
        if (!("$TST_retryCount" in attachInfo)) attachInfo.$TST_retryCount = 0;
        ...
        console.log(
          `tabs.onAttached: the tab ${tabId} or the window ${attachInfo.newWindowId} is already closed. `
        );
        onCompleted();
        return;
      }
    ...
    if (byInternalOperation) win.toBeAttachedTabs.delete(tab.id);
    info.byInternalOperation = info.byInternalOperation || byInternalOperation;

    if (!byInternalOperation) {
      // we should process only tabs attached by others.
      const onAttachedResult = Tab.onAttached.dispatch(tab, info);
      // don't do await if not needed, to process things synchronously
      if (onAttachedResult instanceof Promise) await onAttachedResult;
    }
...
async function onDetached(tabId, detachInfo) {
  ...
  try {
    ...
    if (byInternalOperation) oldWindow.toBeDetachedTabs.delete(tabId);
    ...
    const info = {
      ...
      structure: TreeBehavior.getTreeStructureFromTabs([
        oldTab,
        ...descendants,
      ]),
      descendants,
    };
    const alreadyMovedAcrossWindows = Array.from(
      mTreeInfoForTabsMovingAcrossWindows.values(),
      (info) => info.descendants.map((tab) => tab.id)
    ).some((tabIds) => tabIds.includes(tabId));
    if (!alreadyMovedAcrossWindows)
      mTreeInfoForTabsMovingAcrossWindows.set(tabId, info);

    if (!byInternalOperation)
      // we should process only tabs detached by others.
    ...
    if (
      !TabsStore.getCurrentWindowId() && // only in the background page - the sidebar has no need to destroy itself manually.
      oldWindow.tabs &&
      oldWindow.tabs.size === 0
    ) {
      // not destroyed yet case
      if (oldWindow.delayedDestroy) clearTimeout(oldWindow.delayedDestroy);
      oldWindow.delayedDestroy = setTimeout(
        () => {
          ...
          if (oldWindow.tabs && oldWindow.tabs.size === 0) oldWindow.destroy();
        },
