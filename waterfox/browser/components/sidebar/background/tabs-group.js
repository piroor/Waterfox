...
export function makeGroupTabURI({
  title,
  temporary,
  temporaryAggressive,
  openerTabId,
  ...
  if (temporaryAggressive) url.searchParams.set("temporaryAggressive", "true");
  else if (temporary) url.searchParams.set("temporary", "true");

  if (openerTabId) url.searchParams.set("openerTabId", openerTabId);

  if (aliasTabId) url.searchParams.set("aliasTabId", aliasTabId);
...
export async function groupTabs(
  tabs,
  { broadcast, parent, withDescendants, ...groupTabOptions } = {}
) {
  const rootTabs = Tab.collectRootTabs(tabs);
  if (rootTabs.length <= 0) return null;
...
function reserveToCleanupNeedlessGroupTab(tabOrTabs) {
  ...
  for (const tab of tabs) {
    ...
    if (tab.$TST.temporaryMetadata.has("reservedCleanupNeedlessGroupTab"))
      clearTimeout(
        tab.$TST.temporaryMetadata.get("reservedCleanupNeedlessGroupTab")
      );
    tab.$TST.temporaryMetadata.set(
      "reservedCleanupNeedlessGroupTab",
      setTimeout(() => {
        if (!tab.$TST) return;
...
function cleanupNeedlssGroupTab(tabs) {
  ...
  log("trying to clanup needless temporary group tabs from ", () =>
    tabs.map(dumpTab)
  );
  const tabsToBeRemoved = [];
  for (const tab of tabs) {
    if (tab.$TST.temporaryMetadata.has("movingAcrossWindows")) continue;

    if (tab.$TST.isTemporaryGroupTab) {
      if (tab.$TST.childIds.length > 1) break;
...
export async function tryReplaceTabWithGroup(
  tab,
  { windowId, parent, children, insertBefore, newParent } = {}
) {
  if (tab) {
  ...
  if (
    ...
    countMatched(
      children,
      (tab) => !tab.$TST.states.has(Constants.kTAB_STATE_TO_BE_REMOVED)
    ) <= 1
  )
    return null;
...
async function tryInitGroupTab(tab) {
  if (!tab.$TST.isGroupTab && !tab.$TST.hasGroupTabURL) return;
  ...
  try {
    const getPageState = function getPageState() {
      return [
        window.prepared,
        document.documentElement.matches(".initialized"),
      ];
    };
    const [prepared, initialized, reloaded] = (
      browser.scripting
        ? browser.scripting
            .executeScript({
              // Manifest V3
      ...
      if (
        ApiTabs.isMissingHostPermissionError(error) &&
        tab.$TST.hasGroupTabURL
      ) {
        log(
          "  tryInitGroupTab: failed to run script for restored/discarded tab, reload the tab for safety ",
          tab.id
        );
    ...
    log("  tryInitGroupTab: groupt tab state ", tab.id, {
      prepared,
      initialized,
      reloaded,
    });
  ...
  try {
    ...
    const titleElementExists = (
      ...
      ApiTabs.createErrorHandler(
        ApiTabs.handleMissingTabError,
        ApiTabs.handleMissingHostPermissionError
      )
    );
    if (!titleElementExists && tab.status === "complete") {
      // we need to load resources/group-tab.html at first.
      log("  => title element exists, load again ", tab.id);
      return browser.tabs
        .update(tab.id, { url: tab.url })
        .catch(ApiTabs.createErrorSuppressor());
    }
  } catch (error) {
    log(
      "  tryInitGroupTab error while checking title element: ",
      tab.id,
      error
    );
  }
...
function reserveToUpdateRelatedGroupTabs(tab, changedInfo) {
  ...
  const updatingTabs =
    tabMetadata.get("reserveToUpdateRelatedGroupTabsUpdatingTabs") || new Set();
  if (!tabMetadata.has("reserveToUpdateRelatedGroupTabsUpdatingTabs"))
    tabMetadata.set(
      "reserveToUpdateRelatedGroupTabsUpdatingTabs",
      updatingTabs
    );
  ...
  for (const updatingTab of ancestorGroupTabs) {
    ...
    const reservedChangedInfo =
      updatingMetadata.get("reservedUpdateRelatedGroupTabChangedInfo") ||
      new Set();
    for (const info of changedInfo) {
      reservedChangedInfo.add(info);
    }
    if (updatingTabs.has(updatingTab.id)) continue;
    updatingTabs.add(updatingTab.id);
    const triggeredUpdates =
      updatingMetadata.get("reservedUpdateRelatedGroupTabTriggeredUpdates") ||
      new Set();
    triggeredUpdates.add(updatingTabs);
    updatingMetadata.set(
      "reservedUpdateRelatedGroupTabTriggeredUpdates",
      triggeredUpdates
    );
    ...
    updatingMetadata.set(
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          const triggerUpdates = updatingMetadata.get(
            "reservedUpdateRelatedGroupTabTriggeredUpdates"
          );
          updatingMetadata.delete(
            "reservedUpdateRelatedGroupTabTriggeredUpdates"
          );
...
async function updateRelatedGroupTab(groupTab, changedInfo = []) {
  if (!TabsStore.ensureLivingTab(groupTab)) return;
  ...
  if (changedInfo.includes("tree")) {
    try {
      await browser.tabs
        ...
        .catch((error) => {
          if (ApiTabs.isMissingHostPermissionError(error)) throw error;
          return ApiTabs.createErrorSuppressor(
            ApiTabs.handleMissingTabError,
            ApiTabs.handleUnloadedError
          )(error);
        });
    } catch (error) {
      if (ApiTabs.isMissingHostPermissionError(error)) {
        log(
          "  updateRelatedGroupTab: failed to run script for restored/discarded tab, reload the tab for safety ",
          groupTab.id
        );
  ...
  if (!firstChild)
    // the tab can be closed while waiting...
    return;
  ...
  if (changedInfo.includes("title")) {
    ...
    if (Constants.kGROUP_TAB_DEFAULT_TITLE_MATCHER.test(groupTab.title)) {
      ...
      Constants.kGROUP_TAB_FROM_PINNED_DEFAULT_TITLE_MATCHER.test(
        groupTab.title
      )
    ) {
      const opener = groupTab.$TST.openerTab;
      if (opener) {
        if (opener?.favIconUrl) {
          SidebarConnection.sendMessage({
        ...
        newTitle = browser.i18n.getMessage(
...
Tab.onUpdated.addListener((tab, changeInfo) => {
    ...
    if (tab && status === "complete") {
      if (url.indexOf(Constants.kGROUP_TAB_URI) === 0) {
        ...
        tab.$TST.getPermanentStates().then(async (states) => {
          if (url.indexOf(Constants.kGROUP_TAB_URI) === 0) return;
          ...
          if (
            states.includes(Constants.kTAB_STATE_GROUP_TAB) &&
            pathPart.split("?")[0] ===
              Constants.kGROUP_TAB_URI.replace(PREFIX_REMOVER, "")
          ) {
            ...
            tab.$TST.removeState(Constants.kTAB_STATE_GROUP_TAB, {
              permanently: true,
            });
          }
        });
  ...
  if ("title" in changeInfo) {
    ...
    if (group) reserveToUpdateRelatedGroupTabs(group, ["title", "tree"]);
  }
});
...
Tab.onPinned.addListener(async (tab) => {
  ...
  log(
    "  childIdsBeforeMoved: ",
    tab.$TST.temporaryMetadata.get("childIdsBeforeMoved")
  );
  log(
    "  parentIdBeforeMoved: ",
    tab.$TST.temporaryMetadata.get("parentIdBeforeMoved")
  );

  const children = (
    tab.$TST.temporaryMetadata.has("childIdsBeforeMoved")
      ? tab.$TST.temporaryMetadata
          .get("childIdsBeforeMoved")
          .map((id) => Tab.get(id))
      : tab.$TST.children
  ...
  const shouldGroupChildren =
    configs.autoGroupNewTabsFromPinned || tab.$TST.isGroupTab;
  if (shouldGroupChildren) {
    ...
    openedGroupTab = await groupTabs(children, {
      ...
      title: tab.$TST.isGroupTab
        ? tab.title
        : browser.i18n.getMessage("groupTab_fromPinnedTab_label", tab.title),
    ...
    const modifiedChildren = children.filter((child) =>
      children.includes(child.$TST.parent)
    );
...
Tree.onDetached.addListener((_tab, detachInfo) => {
  if (!detachInfo.oldParentTab) return;
