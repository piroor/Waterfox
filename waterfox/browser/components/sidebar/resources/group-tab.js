(async function prepare(retryCount = 0) {
  if (retryCount > 10) throw new Error("could not prepare group tab contents");
  ...
  function getTitle() {
    ...
    if (!title) {
      ...
      if (matched) title = decodeURIComponent(matched[1]);
    }
  ...
  function updateParameters({ title } = {}) {
    ...
    if (gTemporaryCheck.checked) url.searchParams.set("temporary", "true");
    else url.searchParams.delete("temporary");

    if (gTemporaryAggressiveCheck.checked)
      url.searchParams.set("temporaryAggressive", "true");
    else url.searchParams.delete("temporaryAggressive");

    const opener = getOpenerTabId();
    if (opener) url.searchParams.set("openerTabId", opener);
    else url.searchParams.delete("openerTabId");

    const aliasTabId = getAliasTabId();
    if (aliasTabId) url.searchParams.set("aliasTabId", aliasTabId);
    else url.searchParams.delete("aliasTabId");
    ...
    else url.searchParams.delete("replacedParentCount");
  ...
  async function init(retryCount = 0) {
    ...
    gBrowserThemeDefinition = document.querySelector(
    ...
    gTitleField.addEventListener("keydown", (event) => {
      ...
      if (hasModifier(event) || event.isComposing) return;
    ...
    window.addEventListener(
      ...
      (event) => {
        ...
        if (tabId) {
          ...
          if (
            (event.button === 0 && isAcceled(event)) ||
    ...
    window.addEventListener("keyup", (event) => {
      if (event.key === "F2" && !hasModifier(event)) enterTitleEdit();
    });
    ...
    gTemporaryCheck.addEventListener("change", (_event) => {
      if (gTemporaryCheck.checked) gTemporaryAggressiveCheck.checked = false;
      updateParameters();
    });

    gTemporaryAggressiveCheck.checked = isTemporaryAggressive();
    gTemporaryAggressiveCheck.addEventListener("change", (_event) => {
      if (gTemporaryAggressiveCheck.checked) gTemporaryCheck.checked = false;
      updateParameters();
    });
    ...
    const contextualIdentitiesMarkerDeclarations = Object.keys(
      contextualIdentitiesColorInfo.colors
    )
      .map(
        (id) =>
    ...
    const optionPageSection = getOpenerTabId()
      ? "autoGroupNewTabsFromPinned"
      : "autoGroupNewTabsSection";
    const optionKey = getOpenerTabId()
      ? "showAutoGroupOptionHintWithOpener"
      : "showAutoGroupOptionHint";
    let show = configs[optionKey];
    if (!isTemporary() && !isTemporaryAggressive()) show = false;
  ...
  async function updateTree() {
    ...
    */
    ...
    if (runAt !== updateTree.lastRun) return;
    ...
    if (aliasTab) thisTab.children = aliasTab.children;
    ...
    } else tree = buildChildren(thisTab);
  ...
  function buildItem(tab) {
    ...
    link.setAttribute(
      "title",
      tab.cookieStoreName ? `${tab.title} - ${tab.cookieStoreName}` : tab.title
    );
    link.dataset.tabId = tab.id;
    link.dataset.cookieStoreId = tab.cookieStoreId;

    const contextualIdentityMarker = link.appendChild(
      document.createElement("span")
    );
    contextualIdentityMarker.setAttribute(
      "id",
      `tab-contextual-identity-marker-${tab.id}`
    );
    ...
    if (Array.isArray(tab.states) && tab.states.includes("group-tab"))
  ...
  function columnizeTree(aTree, options) {
    ...
    const containerRect =
      options.containerRect || aTree.parentNode.getBoundingClientRect();
    ...
    if (totalContentsHeight > containerRect.height) {
      ...
      aTree.columnWidth = Number(
        (computedStyle.MozColumnWidth || computedStyle.columnWidth).replace(
          /px/,
          ""
        )
      );
      ...
      const overflow =
        treeContentsRange.getBoundingClientRect().width > window.innerWidth;
      treeContentsRange.detach();
      style.maxWidth = "";
      const blankSpace = overflow ? 2 : 1;
      style.height =
        style.maxHeight = `calc(${containerRect.height}px - ${blankSpace}em)`;
    } else {
