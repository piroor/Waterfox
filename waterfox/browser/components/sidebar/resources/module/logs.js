...
window.addEventListener(
  ...
  () => {
    browser.runtime.onMessage.addListener((message, _sender) => {
      ...
      switch (message.type) {
        case Constants.kCOMMAND_RESPONSE_QUERY_LOGS:
          {
            if (!message.logs || message.logs.length < 1) return;
            document.getElementById("queryLogs").textContent +=
              `${message.logs.reduce((output, log, index) => {
                if (log) {
        ...
        case Constants.kCOMMAND_RESPONSE_CONNECTION_MESSAGE_LOGS:
          {
            ...
            logs.textContent += message.windowId
              ? `"${message.windowId} => background"`
              : '"background => sidebar"';
    ...
    browser.runtime.sendMessage({
      type: Constants.kCOMMAND_REQUEST_QUERY_LOGS,
    });
    browser.runtime.sendMessage({
      type: Constants.kCOMMAND_REQUEST_CONNECTION_MESSAGE_LOGS,
    });
  },
  { once: true }
);
...
function analyzeQueryLogs() {
  const logs = JSON.parse(
    `[${document.getElementById("queryLogs").textContent.replace(/,\s*$/, "")}]`
  );
  ...
  function normalize(log) {
    ...
    if (log.fromIndex) log.fromIndex = "given";
    if (log.logicalIndex) log.logicalIndex = "given";
  ...
  results.push(
    "Top 10 slowest queries:\n" +
      logs
        .sort(
          (a, b) =>
            (b.elasped || b.elapsed || 0) - (a.elasped || a.elapsed || 0)
        )
        .slice(0, 10)
        .map(toString)
        .join("\n")
  ...
  results.push(
    "Sorted in total elapsed time:\n" +
      uniq(normalizedLogs)
        .sort((a, b) => b.totalElapsed - a.totalElapsed)
        .map(toString)
        .join("\n")
  );
...
function analyzeConnectionMessageLogs() {
  const logs = JSON.parse(
    `{${document.getElementById("connectionMessageLogs").textContent.replace(/,\s*$/, "")}}`
  );
  ...
  results.push(
    "Top 10 message types:\n" +
      sortableCounts
        ...
        .map(
          (count) =>
            `${count.type}: ${count.count} (${parseInt((count.count / totalCount) * 100)} %)`
        )
        .join("\n")
  );
