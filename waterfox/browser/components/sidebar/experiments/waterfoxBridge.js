...
const BrowserWindowWatcher = {
  ...
  handleWindow(win) {
    ...
    if (
      win.location.href.startsWith("chrome://browser/content/browser.xhtml")
    ) {
      ...
      if (installed) {
        ...
        win.addEventListener("DOMAudioPlaybackBlockStarted", this, {
          capture: true,
        });
        win.addEventListener("DOMAudioPlaybackBlockStopped", this, {
          capture: true,
        });
      ...
      win.location.href.startsWith("chrome://browser/content/places/")
    ) {
  ...
  unhandleWindow(win) {
    ...
    if (
      ...
      try {
        ...
        win.removeEventListener("DOMAudioPlaybackBlockStarted", this, {
          capture: true,
        });
        win.removeEventListener("DOMAudioPlaybackBlockStopped", this, {
          capture: true,
        });
        ...
        if (sidebar?.contentWindow?.PlacesControllerDragHelper) {
          // Or check a flag set during patching
  ...
  installTabsSidebar(win) {
    ...
    if (document.querySelector("#tabs-sidebar-style")) return true;
    ...
    document.querySelector("#mainCommandSet").appendChild(
      element(document, XUL, "command", {
    ...
    if (!document.querySelector("#toggle-tabs-sidebar-key")) {
      document.querySelector("#mainKeyset").appendChild(
        element(document, XUL, "key", {
    ...
    document.querySelector("#viewSidebarMenu").appendChild(
    ...
    const width =
      Services.xulStore.getValue(document.URL, "tabs-sidebar-box", "width") ||
      200;
                  ...
                  element(
                    ...
                    [
                      element(document, XUL, "toolbarbutton", {
                        ...
                        accesskey: this.locale.get(
                          "optionsButton_moveLeft_accesskey"
                        ),
                      }),
                      element(document, XUL, "toolbarbutton", {
                        ...
                        accesskey: this.locale.get(
                          "optionsButton_moveRight_accesskey"
                        ),
                      }),
                      ...
                      element(document, XUL, "toolbarbutton", {
                        ...
                        class: "subviewbutton",
                        label: this.sanitizeMenuLabel(
                          this.locale.get("tabContextMenu_newTab_label")
                        ),
                      ...
                      element(document, XUL, "toolbarbutton", {
                        ...
                        class: "subviewbutton",
                        label: this.sanitizeMenuLabel(
                          this.locale.get("tabContextMenu_reloadSelected_label")
                        ),
                        accesskey: this.extractAccessKey(
                          this.locale.get("tabContextMenu_reloadSelected_label")
                        ),
                      ...
                      element(document, XUL, "toolbarbutton", {
                        ...
                        class: "subviewbutton",
                        label: this.sanitizeMenuLabel(
                          this.locale.get("tabContextMenu_selectAllTabs_label")
                        ),
                        accesskey: this.extractAccessKey(
                          this.locale.get("tabContextMenu_selectAllTabs_label")
                        ),
                      ...
                      element(document, XUL, "toolbarbutton", {
                        ...
                        class: "subviewbutton",
                        label: this.sanitizeMenuLabel(
                          this.locale.get("tabContextMenu_undoClose_label")
                        ),
                        accesskey: this.extractAccessKey(
                          this.locale.get("tabContextMenu_undoClose_label")
                        ),
                      ...
                      element(document, XUL, "toolbarbutton", {
                        ...
                        label: this.locale.get(
                          "optionsButton_moreOptions_label"
                        ),
                        accesskey: this.locale.get(
                          "optionsButton_moreOptions_accesskey"
                        ),
                      }),
                    ]
                  ),
    ...
    if (targetElement) {
      ...
      console.error(
        "WaterfoxBridge: #tabbrowser-tabbox element not found in window:",
        win.location.href,
        ". Cannot insert sidebar elements."
      );
    ...
    if (tabsSidebarElement) {
      tabsSidebarElement.addEventListener(
        "load",
        () => {
          ...
          if (tabsSidebarElement.contentWindow) {
            ...
            console.error(
              "WaterfoxBridge: #tabs-sidebar contentWindow is not available."
            );
          }
        },
      ...
      tabsSidebarElement.addEventListener(
        ...
        (event) => {
          ...
          for (const file of event.dataTransfer.files) {
            const fileInternal = Cc["@mozilla.org/file/local;1"].createInstance(
      ...
      console.error(
        "WaterfoxBridge: #tabs-sidebar element not found. Cannot attach event listeners or load panel."
      );
    }
    ...
    document
      .querySelector("#tabs-sidebar-splitter")
  ...
  updateOptionsPopup(menupopup) {
    ...
    menupopup.querySelector("#tabs-sidebar-bookmarkTabs").hidden =
      !multiselected;
  },
  ...
  shouldShowSidebar(document) {
    ...
    return (
      Services.xulStore.getValue(document.URL, "tabs-sidebar-box", "shown") ===
      "true"
    );
  },
  ...
  uninstallTabsSidebar(win) {
    ...
    document.removeEventListener("customizationchange", this, {
      capture: true,
    });
  ...
  patchToPlacesModules(win) {
    if (!win.PlacesControllerDragHelper.__ws_orig__getMostRelevantFlavor)
      win.PlacesControllerDragHelper.__ws_orig__getMostRelevantFlavor =
        win.PlacesControllerDragHelper.getMostRelevantFlavor;
    win.PlacesControllerDragHelper.getMostRelevantFlavor = function (flavours) {
      if (flavours.contains(TYPE_TREE)) return TYPE_TREE;

      return this.__ws_orig__getMostRelevantFlavor(flavours);
    };

    if (!win.PlacesControllerDragHelper.__ws_orig__canDrop)
      win.PlacesControllerDragHelper.__ws_orig__canDrop =
        win.PlacesControllerDragHelper.canDrop;
    win.PlacesControllerDragHelper.canDrop = function (
      insertionPoint,
      dataTransfer
    ) {
      if (dataTransfer.mozTypesAt(0).contains(TYPE_TREE)) return true;
  ...
  patchToTabHoverPreviewModules(win) {
    ...
    if (!tabs._previewPanel) {
      ...
      tabs._previewPanel = new TabHoverPreviewPanel(
        win.document.getElementById("tab-preview-panel")
      );
    }

    const tabPreview = tabs._previewPanel;
    if (!tabPreview || !tabPreview.activate) return;
    ...
    tabPreview.__ws__calculateCoordinates = () => {
      ...
      const align = win.document.documentElement.classList.contains(
        "tabs-sidebar-right"
      )
        ? "right"
        : "left";
      ...
      const y = Math.min(
        tabsSidebar.screenY + tabPreview.__ws__top,
        tabsSidebar.screenY + tabsSidebarRect.height - previewRect.height
      );
      return [x, y];
    };
    ...
    tabPreview.activate = function (tab) {
      ...
      this._timer = this._win.setTimeout(() => {
        this._timer = null;
        if (
          self.shouldShowSidebar(win.document) &&
    ...
    if (!tabPreview._panel.__ws_orig__moveToAnchor)
      tabPreview._panel.__ws_orig__moveToAnchor =
        tabPreview._panel.moveToAnchor;
  ...
  unpatchTabHoverPreviewModules(win) {
    const tabPreview =
      win.document.getElementById("tabbrowser-tabs")?._previewPanel;
    if (!tabPreview || !tabPreview.activate) return;
  ...
  patchToFullScreenModules(win) {
    ...
    const tabsSidebarSplitter = win.document.querySelector(
      "#tabs-sidebar-splitter"
    );
    ...
    FullScreen.toggle = function (...args) {
      ...
      if (enterFS) {
        if (!win.document.fullscreenElement) {
          if (
            win.gNavToolbox.getAttribute("fullscreenShouldAnimate") === "true"
          )
    ...
    if (!FullScreen.__ws_sidebar) {
      FullScreen.__ws_sidebar = {
        isRightSide() {
          return win.document.documentElement.classList.contains(
            "tabs-sidebar-right"
          );
        },

        updateMouseTargetRect() {
          const contentsAreaRect =
            win.gBrowser.tabpanels.getBoundingClientRect();
          ...
          this._mouseTargetRect = {
            ...
            left:
              Math.min(
                sidebarMainBoxRect.left,
                sidebarBoxRect.left,
                contentsAreaRect.left
              ) + (isRight ? 0 : tabsSidebarBoxRect.width + 50),
            right:
              Math.max(
                sidebarMainBoxRect.right,
                sidebarBoxRect.right,
                contentsAreaRect.right
              ) - (isRight ? tabsSidebarBoxRect.width + 50 : 0),
          };
        ...
        startListenToShow() {
          if (this.listeningToShow) return;
          this.listeningToShow = true;
          tabsSidebarSplitter.addEventListener(
            "mouseover",
            FullScreen.__ws_sidebar
          );
          tabsSidebarSplitter.addEventListener(
        ...
        endListenToShow() {
          ...
          tabsSidebarSplitter.removeEventListener(
            "mouseover",
            FullScreen.__ws_sidebar
          );
          tabsSidebarSplitter.removeEventListener(
  ...
  updateToggleButton(document, button) {
    ...
    const viewMenuItem = document.querySelector(
      "#viewmenu-toggle-tabs-sidebar"
    );
    if (this.shouldShowSidebar(document)) {
      ...
      button.setAttribute(
        "tooltiptext",
        this.locale.get("closeButton_tooltip")
      );
  ...
  updateTabsSidebarPositionIn(document) {
    const isRight =
      Services.prefs.getStringPref(`${this.BASE_PREF}sidebarPosition`, "") ===
      "2";
  ...
  *iteratePlacesOwnerWindows() {
    ...
    while (browserWindows.hasMoreElements()) {
      const win =
        browserWindows.getNext(); /*.QueryInterface(Components.interfaces.nsIDOMWindow)*/
      yield win;
      const sidebar = win.document?.querySelector("#sidebar");
      if (sidebar) yield sidebar.contentWindow;
    }

    const organizerWindows = Services.wm.getEnumerator("Places:Organizer");
    while (organizerWindows.hasMoreElements()) {
      const win =
        organizerWindows.getNext(); /*.QueryInterface(Components.interfaces.nsIDOMWindow)*/
      yield win;
    }
  ...
  openOptions(win, full = false) {
    const url = full
      ? `${this.BASE_URL}options/options.html#!`
      : "about:preferences#tabsSidebar";
    ...
    (win || Services.wm.getMostRecentBrowserWindow()).openLinkIn(url, "tab", {
  ...
  toggleTabsToolbar(document) {
    const shouldOpen =
      document.querySelector("#tabs-sidebar-box").getAttribute("hidden") ===
      "true";
    if (shouldOpen) this.openTabsSidebar(document);
    else this.closeTabsSidebar(document);
    this.updateToggleButton(document);
    this.updateHorizontalTabsState(document);
  },
  ...
  updateHorizontalTabsState(document) {
    ...
    if (sidebarBox) {
      ...
      console.error(
        "WaterfoxBridge: #tabs-sidebar-box element not found in window:",
        document.defaultView.location.href,
        ". Cannot update horizontal tabs state."
      );
      // Defaulting to 'active = false' as a fallback
    }
    const hideHorizontalTabsWhileActive = Services.prefs.getBoolPref(
      `${this.BASE_PREF}hideHorizontalTabsWhileActive`,
      true
    );
  ...
  handleEvent(event) {
    const win =
      event.target.ownerDocument?.defaultView || event.target.defaultView;
    switch (event.type) {
      case "SidebarShown":
        ...
        switch (event.target.id) {
          ...
          case "tabs-sidebar-moveLeft":
            Services.prefs.setStringPref(
              `${this.BASE_PREF}sidebarPosition`,
              "1"
            );
            this.tryHidePopup(event);
            break;
          ...
          case "tabs-sidebar-bookmarkTabs":
            {
              const pages = win.PlacesCommandHook.getUniquePages(
                win.gBrowser.selectedTabs
              ).map((page) =>
                Object.assign(page, {
                  uri: Services.io.createExposableURI(page.uri),
                })
              );
      ...
      case "mouseup":
        Services.xulStore.persist(
          event.target.ownerDocument.querySelector("#tabs-sidebar-box"),
          "width"
        );
        break;
  ...
  tryHidePopup(event) {
    if (event.target.closest) event.target.closest("panel")?.hidePopup();
  },

  moveToolbarButtonToDefaultPosition() {
    try {
      const rawState = Services.prefs.getStringPref(
      ...
      for (const [name, items] of Object.entries(state.placements)) {
        if (!items.includes(this.id)) continue;
        if (name === "nav-bar") foundInNavBar = true;
        found = true;
        break;
      }
      ...
      win.setTimeout(() => {
        if (win.document.getElementById(this.id)) return;
        console.log(
          "failed to insert tabs sidebar button due to unhandled error in CustomizableUI module: retrying with reset"
        );
  ...
  // nsIContentPolicy
  shouldLoad(contentLocation, loadInfo, _mimeTypeGuess) {
    ...
    if (
      this.WATCHING_URLS.some((url) => contentLocation.spec.startsWith(url))
    ) {
      ...
      const timer = topWin.setInterval(() => {
        ...
        if (!win) return;
        try {
          if (this.handleWindow(win)) topWin.clearInterval(timer);
        } catch (_error) {}
      }, 250);
    }
  ...
  shouldProcess(_contentLocation, _loadInfo, _mimeTypeGuess) {
    return Components.interfaces.nsIContentPolicy.ACCEPT;
  },
  ...
  async tryConfirmUsingTST() {
    ...
    if (Services.prefs.getBoolPref(ignorePrefKey, false)) return;
  ...
  onInstallEnded(install) {
    if (install.addon.id === TST_ID) this.tryConfirmUsingTST();
  },
  ...
  onEnabled(addon) {
    if (addon.id === TST_ID) this.tryConfirmUsingTST();
  },
  onDisabled(_addon) {},
};
      ...
      waterfoxBridge: {
        async initUI() {
          ...
          BrowserWindowWatcher.locale = {
            get(key) {
              key = key.toLowerCase();
              if (this.selected.has(key)) return this.selected.get(key);
              return this.default.get(key) || key;
            },
            default: context.extension.localeData.messages.get(
              context.extension.localeData.defaultLocale
            ),
            selected: context.extension.localeData.messages.get(
              context.extension.localeData.selectedLocale
            ),
          };
          ...
          if (
            !Services.prefs.getBoolPref(
              `${BrowserWindowWatcher.BASE_PREF}toolbarButtonInserted`,
              false
            ) &&
          ...
          if (!lazy.PlacesUtils.__ws_orig__unwrapNodes)
            lazy.PlacesUtils.__ws_orig__unwrapNodes =
              lazy.PlacesUtils.unwrapNodes;
          ...
          if (
            !Services.prefs.getBoolPref(
              `${BrowserWindowWatcher.BASE_PREF}permissionsGranted`,
              false
            )
          ) {
            lazy.ExtensionPermissions.add(
          ...
          if (installedTST?.isActive) BrowserWindowWatcher.tryConfirmUsingTST();
        },
        ...
        async getTabPreview(tabId) {
          ...
          try {
            ...
            */
            ...
            // tabPreviews.capture() raises error if the tab is discarded.
        ...
        async showPreviewPanel(tabId, top) {
          ...
          const tabPreview =
            document.getElementById("tabbrowser-tabs")?._previewPanel;
        ...
        onWindowVisibilityChanged: new EventManager({
          ...
          register: (fire) => {
            const onChanged = (win) => {
              const wrappedWindow =
                context.extension.windowManager.getWrapper(win);
              if (wrappedWindow)
                fire
                  .async(wrappedWindow.id, win.document.visibilityState)
        ...
        onSidebarShown: new EventManager({
          ...
          register: (fire) => {
            const onShown = (win) => {
              const wrappedWindow =
                context.extension.windowManager.getWrapper(win);
              if (wrappedWindow) fire.async(wrappedWindow.id).catch(() => {}); // ignore Message Manager disconnects
            };
        ...
        async listSyncDevices() {
          const devices = [];
          const targets = Services.wm
            .getMostRecentBrowserWindow()
            .gSync.getSendTabTargets();
        ...
        async sendToDevice(tabIds, deviceId) {
          ...
          const targets = gSync
            .getSendTabTargets()
            .filter((target) => !deviceId || target.id === deviceId);
        ...
        async openSyncDeviceSettings(windowId) {
          ...
          try {
            const win =
              windowId && context.extension.windowManager.get(windowId);
            DOMWin = win?.window;
          } catch (_error) {}
          (
            DOMWin || Services.wm.getMostRecentBrowserWindow()
        ...
        async listSharingServices(tabId) {
          ...
          if (!sharingService) return services;
        ...
        async share(tabIds, shareName) {
          ...
          if (shareName) {
            // for macOS
            win.gBrowser.MacSharingService.shareUrl(
              shareName,
              uri,
              tab.nativeTab.linkedBrowser.contentTitle
            );
        ...
        async listAutoplayBlockedTabs(windowId) {
          ...
          for (const win of windows) {
            if (!win.window.gBrowser) continue;
            for (const tab of win.window.document.querySelectorAll(
              'tab[activemedia-blocked="true"]'
            )) {
              const wrappedTab = context.extension.tabManager.getWrapper(tab);
              if (wrappedTab) tabs.add(wrappedTab.convert());
            }
          }
        ...
        onAutoplayBlocked: new EventManager({
          ...
          register: (fire) => {
            const onBlocked = (tab) => {
              const wrappedTab = context.extension.tabManager.getWrapper(tab);
              if (wrappedTab) fire.async(wrappedTab.convert()).catch(() => {}); // ignore Message Manager disconnects
            };
        ...
        onAutoplayUnblocked: new EventManager({
          ...
          register: (fire) => {
            ...
            return () => {
              BrowserWindowWatcher.autoplayUnblockedListeners.delete(
                onUnblocked
              );
            };
          },
