...
const mContextualIdentitySelector = document.getElementById(
  Constants.kCONTEXTUAL_IDENTITY_SELECTOR
);
const mNewTabActionSelector = document.getElementById(
  Constants.kNEWTAB_ACTION_SELECTOR
);
...
Sidebar.onBuilt.addListener(async () => {
  ...
  if (!mRootClasses.contains("incognito"))
    mContextualIdentitySelector.ui = new MenuUI({
      ...
      animationDuration: shouldApplyAnimation()
...
function updateSpecialEventListenersForAPIListeners() {
  ...
  const shouldListenMouseOut = TSTAPI.hasListenerForMessageType(
    TSTAPI.kNOTIFY_TAB_MOUSEOUT
  );
...
const mCloseBoxSizeBox = document.querySelector(
  `#dummy-tab ${kTAB_CLOSE_BOX_ELEMENT_NAME}`
);
const mFaviconSizeBox = document.querySelector(
  `#dummy-tab ${kTAB_FAVICON_ELEMENT_NAME}`
);
const mTwistySizeBox = document.querySelector(
  `#dummy-tab ${kTAB_TWISTY_ELEMENT_NAME}`
);
const mDistanceBox = document.querySelector(
...
function onMouseMove(event) {
  ...
  if (mTabBar.classList.contains(Constants.kTABBAR_STATE_SCROLLBAR_AUTOHIDE)) {
    ...
    const leftAreaSize =
      ...
      (isRightSide
        ? closeRect.width
        : Math.max(twistyRect.right, faviconRect.right) -
          Math.min(twistyRect.left, faviconRect.left)) +
    ...
    mRootClasses.toggle(
      "on-scrollbar-area",
      onTabBar && isRightSide
        ? event.clientX >= tabbarRect.right - rightAreaSize
        : event.clientX <= tabbarRect.left + leftAreaSize
    );
  }

  if (TSTAPI.hasListenerForMessageType(TSTAPI.kNOTIFY_TAB_MOUSEMOVE) && tab) {
...
function onMouseOver(event) {
  ...
  if (!mHasMouseOverListeners) return;
...
function onMouseDown(event) {
  ...
  if (
    ...
    const selector = document.getElementById(
      EventUtils.getElementTarget(event).closest("[data-menu-ui]").dataset
        .menuUi
    );
    selector.ui.open({
      anchor: event.target,
  ...
  const tab =
    ...
    EventUtils.getTabFromTabbarEvent(event);
  log("onMouseDown: found target tab: ", tab, event);

  const extraContentsInfo =
    TSTAPIFrontend.getOriginalExtraContentsTarget(event);
  ...
  if (mousedownDetail.targetType === "selector") return;
  ...
  mousedown.promisedMousedownNotified = Promise.all([
    ...
    (async () => {
      ...
      const allowed =
        await TSTAPIFrontend.tryMouseOperationAllowedWithExtraContents(
  ...
  mousedown.promisedMousedownNotified.then((canceled) => {
    if (canceled) EventUtils.cancelHandleMousedown(event.button);

    if (
      !EventUtils.getLastMousedown(event.button) ||
      mousedown.expired ||
      canceled
    )
      return;
    ...
    const wasMultiselectionAction =
      mousedown.detail.ctrlKey || mousedown.detail.shiftKey;
    if (
      mousedown.detail.button === 0 &&
      onRegularArea &&
      ...
      if (
        tab.active ||
        tab.$TST.states.has(Constants.kTAB_STATE_BUNDLED_ACTIVE)
      )
        // sticky active tab
  ...
  mousedown.timeout = setTimeout(async () => {
    if (!EventUtils.getLastMousedown(event.button)) return;
    ...
    if (
      ...
      if (selector) {
        const anchor = target.parentNode.querySelector(
          `[data-menu-ui="${selector.id}"]`
        );
        const anchorVisible =
          anchor && window.getComputedStyle(anchor, null).display !== "none";
        selector.ui.open({
    ...
    if (
      TSTAPI.getListenersForMessageType(TSTAPI.kNOTIFY_TAB_DRAGREADY).length ===
      0
    )
      return;

    if (event.button === 0 && tab) {
...
async function onMouseUp(event) {
  ...
  if (
    EventUtils.isEventFiredOnMenuOrPanel(event) ||
    EventUtils.isEventFiredOnAnchor(event)
  ) {
  ...
  if (
    ...
    (tab && tab !== Tab.get(lastMousedown.detail.tab))
  ) {
    // when the tab was already removed
...
async function handleDefaultMouseUp({ lastMousedown, tab, event }) {
  ...
  if (
    ...
    Math.abs(mLastMouseUpX - event.clientX) <
      configs.acceptableFlickerToIgnoreClickOnTabAndTabbar / 2 &&
    Math.abs(mLastMouseUpY - event.clientY) <
      configs.acceptableFlickerToIgnoreClickOnTabAndTabbar / 2
  ) {
  ...
  if (lastMousedown.detail.isMiddleClick) {
    // Ctrl-click does nothing on Firefox's tab bar!
...
async function handleDefaultMouseUpOnTab({ lastMousedown, tab, event } = {}) {
  ...
  const wasMultiselectionAction = updateMultiselectionByTabClick(
    tab,
    lastMousedown.detail
  );
  ...
  if (lastMousedown.detail.isMiddleClick) {
    // Ctrl-click doesn't close tab on Firefox's tab bar!
    log(
      `onMouseUp: middle click on the tab ${tab.id}: `,
      lastMousedown.detail.targetType
    );
    if (lastMousedown.detail.targetType !== "tab")
      // ignore middle click on blank area
      return false;
    const tabs = TreeBehavior.getClosingTabsFromParent(tab, {
      byInternalOperation: true,
    });
    Sidebar.confirmToCloseTabs(tabs.map((tab) => tab.$TST.sanitized)).then(
      async (confirmed) => {
        if (!confirmed) return;
    ...
    if (tab.$TST.hasChild) {
      if (!tab.$TST.subtreeCollapsed)
        ...
        await Scroll.tryLockPosition(
          tab.$TST.descendants
            .filter((tab) => !tab.$TST.collapsed)
            .map((tab) => tab.id),
          Scroll.LOCK_REASON_COLLAPSE
        );
      // going to expand
      else Scroll.tryUnlockPosition(tab.$TST.descendants.map((tab) => tab.id));
      BackgroundConnection.sendMessage({
    ...
    if (
      tab.$TST.states.has(Constants.kTAB_STATE_AUTOPLAY_BLOCKED) ||
    ...
    Sidebar.confirmToCloseTabs(
      tabsToBeClosed.map((tab) => tab.$TST.sanitized),
      {
...
handleDefaultMouseUpOnTab = EventUtils.wrapWithErrorHandler(
  handleDefaultMouseUpOnTab
);
...
function updateMultiselectionByTabClick(tab, event) {
  ...
  const highlightedTabIds = new Set(
    Tab.getHighlightedTabs(tab.windowId).map((tab) => tab.id)
  );
  log(`updateMultiselectionByTabClick on ${tab.id} `, {
    ctrlKeyPressed,
    activeTab,
    highlightedTabIds,
  ...
  if (event.shiftKey) {
    ...
    try {
      if (!ctrlKeyPressed) {
        const alreadySelectedTabs = Tab.getHighlightedTabs(tab.windowId, {
          iterator: true,
        });
      ...
      log(
        `set selection by shift-click on ${tab.id}: `,
        configs.debug && Array.from(targetTabs, dumpTab)
      );
      ...
      const rootTabs = [tab];
      if (tab !== activeTab && !mIsInSelectionSession) rootTabs.push(activeTab);
      for (const root of rootTabs) {
        if (!root.$TST.subtreeCollapsed) continue;
      ...
      // not implemented on old Firefox
      return false;
    }
    ...
    try {
      ...
      if (
        ...
        const highlightedCount = countMatched(
          activeTabDescendants,
          (tab) => tab.highlighted
        );
        const partiallySelected =
          highlightedCount !== 0 &&
          highlightedCount !== activeTabDescendants.length;
        toBeHighlighted =
          partiallySelected || !activeTabDescendants[0].highlighted;
        log(" => ", toBeHighlighted, { partiallySelected });
      }
      if (toBeHighlighted) highlightedTabIds.add(tab.id);
      else highlightedTabIds.delete(tab.id);

      if (tab.$TST.subtreeCollapsed) {
        const descendants =
          tab === activeTab ? activeTabDescendants : tab.$TST.descendants;
        for (const descendant of descendants) {
          if (toBeHighlighted) highlightedTabIds.add(descendant.id);
          else highlightedTabIds.delete(descendant.id);
        }
      }
...
Tab.onActivated.addListener((tab, _info = {}) => {
  if (tab.windowId !== mTargetWindow) return;
...
function onDragStart(event) {
  ...
  const modifiers = String(
    configs.newTabButtonDragGestureModifiers
  ).toLowerCase();
  if (
    !configs.allowDragNewTabButton ||
...
async function onDblClick(event) {
  if (EventUtils.isEventFiredOnNewTabButton(event)) return;
  ...
  if (
    ...
    if (
      !EventUtils.isEventFiredOnClosebox(event) && // closebox action is already processed by onclick listener, so we should not handle it here!
      configs.treeDoubleClickBehavior !==
        Constants.kTREE_DOUBLE_CLICK_BEHAVIOR_NONE
    ) {
  ...
  if (tab)
    // ignore dblclick on closing tab or something
    return;
    ...
    switch (item.dataset.value) {
      ...
      case "child": {
        ...
        if (
          ...
          action = Constants.kNEWTAB_OPEN_AS_CHILD_END;
        else action = Constants.kNEWTAB_OPEN_AS_CHILD;
        break;
      }
      case "sibling":
...
function onContextualIdentitySelect(item, event) {
  ...
  if (mContextualIdentitySelector.ui) mContextualIdentitySelector.ui.close();
}
  ...
  switch (message.type) {
    case Constants.kNOTIFY_TAB_MOUSEDOWN_EXPIRED:
      if (message.windowId === mTargetWindow) {
        ...
        if (lastMousedown) lastMousedown.expired = true;
      }
      break;

    case Constants.kCOMMAND_SHOW_CONTAINER_SELECTOR:
      {
        if (!mContextualIdentitySelector.ui) return;
