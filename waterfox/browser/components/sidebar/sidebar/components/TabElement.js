...
export class TabElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const sharingState = document.createElement(
      kTAB_SHARING_STATE_ELEMENT_NAME
    );
  ...
  initializeContents() {
    ...
    if (this._labelElement) {
      if (!this._labelElement.owner) {
        this._labelElement.addOverflowChangeListener(() => {
          if (!this.$TST || this.$TST.tab.pinned) return;
    ...
    if (this._counterElement) this._counterElement.owner = this;
    if (this._sharingStateElement) this._sharingStateElement.owner = this;
  ...
  get tab() {
    return (
      this._tab ||
      (this._tab = Tab.get(parseInt(this.getAttribute(Constants.kAPI_TAB_ID))))
    );
  }
  set tab(value) {
  ...
  get $TST() {
    return this._$TST || (this._$TST = this.tab?.$TST);
  }
  set $TST(value) {
  ...
  applyAttributes() {
    ...
    this.setAttribute(
      "aria-selected",
      this.classList.contains(Constants.kTAB_STATE_HIGHLIGHTED)
        ? "true"
        : "false"
    );

    // for convenience on customization with custom user styles
    this._substanceElement.setAttribute(
    ...
    if (this.tab)
      this.dataset.index =
        ...
        this._labelElement.dataset.index =
          this.tab.index;
  ...
  invalidate(targets) {
    ...
    if (targets & TabInvalidationTarget.Twisty) this.twisty?.invalidate();
    ...
    if (targets & TabInvalidationTarget.CloseBox) this.closeBox?.invalidate();

    if (targets & TabInvalidationTarget.Tooltip) this.invalidateTooltip();
  ...
  invalidateTooltip() {
    if (this._reservedUpdateTooltip) return;
  ...
  update(targets) {
    ...
    if (targets & TabUpdateTarget.Counter) this._counterElement?.update();

    if (targets & TabUpdateTarget.Overflow) this._updateOverflow();
    ...
    if (targets & TabUpdateTarget.TabProperties) this._updateTabProperties();
  }
  ...
  _updateTooltip() {
    if (!this.$TST)
      // called before binding on restoration from cache
      return;
...
`.trim();
    ...
    this.tooltipWithDescendants =
      this.$TST.generateTooltipTextWithDescendants();
    ...
    if (typeof highPriorityTooltipText === "string") {
      if (highPriorityTooltipText)
        this.$TST.setAttribute("title", this.tooltip);
      else this.$TST.removeAttribute("title");
      return;
    }
    ...
    if (
      typeof lowPriorityTooltipText === "string" &&
  ...
  _initExtraItemsContainers() {
    if (!this.extraItemsContainerIndentRoot) {
      this.extraItemsContainerIndentRoot = this.querySelector(
        `.${Constants.kEXTRA_ITEMS_CONTAINER}.indent`
      ).attachShadow({ mode: "open" });
      this.extraItemsContainerIndentRoot.itemById = new Map();
    }
    if (!this.extraItemsContainerBehindRoot) {
      this.extraItemsContainerBehindRoot = this.querySelector(
        `.${Constants.kEXTRA_ITEMS_CONTAINER}.behind`
      ).attachShadow({ mode: "open" });
      this.extraItemsContainerBehindRoot.itemById = new Map();
    }
    if (!this.extraItemsContainerFrontRoot) {
      this.extraItemsContainerFrontRoot = this.querySelector(
        `.${Constants.kEXTRA_ITEMS_CONTAINER}.front`
      ).attachShadow({ mode: "open" });
      this.extraItemsContainerFrontRoot.itemById = new Map();
    }
    if (!this.extraItemsContainerAboveRoot) {
      this.extraItemsContainerAboveRoot = this.querySelector(
        `.${Constants.kEXTRA_ITEMS_CONTAINER}.above`
      ).attachShadow({ mode: "open" });
      this.extraItemsContainerAboveRoot.itemById = new Map();
    }
    if (!this.extraItemsContainerBelowRoot) {
      this.extraItemsContainerBelowRoot = this.querySelector(
        `.${Constants.kEXTRA_ITEMS_CONTAINER}.below`
      ).attachShadow({ mode: "open" });
  ...
  _startListening() {
    if (this.__onMouseOver) return;
    this.addEventListener(
      "mouseover",
      (this.__onMouseOver = this._onMouseOver.bind(this))
    );
    ...
    configs.$addObserver(
      (this.__onConfigChange = this._onConfigChange.bind(this))
    );
  }
  ...
  _updateTabAndAncestorsTooltip(tab) {
    ...
    for (const updateTab of [tab].concat(tab.$TST.ancestors)) {
      ...
      if (configs.labelOverflowStyle !== "fade") tabElement.updateOverflow();
    }
  }
    ...
    for (const child of children) {
      if (child.$TST.states.has(Constants.kTAB_STATE_HIGHLIGHTED)) {
        ...
        allHighlighted =
          allHighlighted &&
          (!child.$TST.hasChild ||
            child.$TST.states.has(
              Constants.kTAB_STATE_ALL_DESCENDANTS_HIGHLIGHTED
            ));
      } else {
        if (
          !someHighlighted &&
          child.$TST.states.has(
            Constants.kTAB_STATE_SOME_DESCENDANTS_HIGHLIGHTED
          )
        ) {
          someHighlighted = true;
        }
  ...
  _updateCollapseExpandState() {
    ...
    if (
      this.$TST.collapsed ||
      (parent && (parent.$TST.collapsed || parent.$TST.subtreeCollapsed))
    ) {
  ...
  _updateTabProperties() {
    ...
    const openerOfGroupTab =
      this.$TST.isGroupTab && Tab.getOpenerFromGroupTab(tab);
    this.favIconUrl = openerOfGroupTab?.favIconUrl || tab.favIconUrl;

    for (const state of classList) {
      if (IGNORE_CLASSES.has(state) || NATIVE_PROPERTIES.has(state)) continue;
      if (!this.$TST.states.has(state)) classList.remove(state);
    }
    for (const state of this.$TST.states) {
      if (IGNORE_CLASSES.has(state)) continue;
      if (!classList.contains(state)) classList.add(state);
    }

    for (const state of NATIVE_PROPERTIES) {
      if (tab[state] === classList.contains(state)) continue;
    ...
    if (this.$TST.childIds.length > 0)
      this.setAttribute(
        Constants.kCHILDREN,
        `|${this.$TST.childIds.join("|")}|`
      );
    else this.removeAttribute(Constants.kCHILDREN);

    if (this.$TST.parentId)
      this.setAttribute(Constants.kPARENT, this.$TST.parentId);
    else this.removeAttribute(Constants.kPARENT);

    const alreadyGrouped =
      this.$TST.getAttribute(
    ...
    const opener =
      this.$TST.getAttribute(Constants.kPERSISTENT_ORIGINAL_OPENER_TAB_ID) ||
      "";
    if (
      this.getAttribute(Constants.kPERSISTENT_ORIGINAL_OPENER_TAB_ID) !== opener
    )
    ...
    const favIconUri =
      this.$TST.getAttribute(Constants.kCURRENT_FAVICON_URI) || tab.favIconUrl;
  ...
  set favIconUrl(url) {
    ...
    if (!this.initialized || !this.$TST) return url;

    if (!url || url.startsWith("data:")) {
      // we don't need to use the helper for data: URI.
