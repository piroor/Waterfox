...
export async function setCollapsed(tab, info = {}) {
  ...
  const manager =
    tab.$TST.collapsedStateChangedManager || new EventListenerManager();
  ...
  const onCompleted = (tab, info = {}) => {
    ...
    if (cancelled || !TabsStore.ensureLivingTab(tab))
      // do nothing for closed tab!
      return;
    ...
    case Constants.kCOMMAND_NOTIFY_SUBTREE_COLLAPSED_STATE_CHANGED:
      {
        ...
        tab.$TST.invalidateElement(
          TabInvalidationTarget.Twisty | TabInvalidationTarget.Tooltip
        );
      }
      break;
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_COLLAPSED_STATE_CHANGED:
      {
        ...
        if (tab.$TST.collapsedOnCreated) {
          // it may be already expanded by others!
          if (!tab.$TST.collapsed)
            // expanded by someone, so clear the flag.
