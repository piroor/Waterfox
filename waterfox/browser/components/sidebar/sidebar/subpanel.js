...
export async function init() {
  ...
  const [providerId, height] = await Promise.all([
    browser.sessions
      .getWindowValue(
        mTargetWindow,
        Constants.kWINDOW_STATE_SUBPANEL_PROVIDER_ID
      )
      .catch(ApiTabs.createErrorHandler()),
    browser.sessions
      .getWindowValue(mTargetWindow, Constants.kWINDOW_STATE_SUBPANEL_HEIGHT)
      .catch(ApiTabs.createErrorHandler()),
  ]);
  const providerSpecificHeight =
    providerId &&
    (await browser.sessions
      .getWindowValue(
        mTargetWindow,
        `${Constants.kWINDOW_STATE_SUBPANEL_HEIGHT}:${providerId}`
      )
      .catch(ApiTabs.createErrorHandler()));
  mHeight =
    typeof providerSpecificHeight === "number"
    ...
    switch (message.type) {
      case TSTAPI.kCOMMAND_BROADCAST_API_REGISTERED:
        wait(0).then(async () => {
          ...
          if (
            provider?.subPanel &&
            (mProviderId === provider.id || provider.newlyInstalled)
          ) {
            if (mHeight === 0) mHeight = getDefaultHeight();
      ...
      case TSTAPI.kCOMMAND_BROADCAST_API_UNREGISTERED:
        wait(0).then(() => {
          // wait until addons are updated
          updateSelector();
          if (message.sender.id === mProviderId) restoreLastProvider();
        });
        break;
    }
  });
...
TSTAPI.onInitialized.addListener(async () => {
  ...
  const providerId = await browser.sessions
    .getWindowValue(mTargetWindow, Constants.kWINDOW_STATE_SUBPANEL_PROVIDER_ID)
    .catch(ApiTabs.createErrorHandler());
  if (providerId) await applyProvider(providerId);
  else restoreLastProvider();
});

function getProviderIconUrl(provider) {
  if (!provider.icons || typeof provider.icons !== "object") return null;

  if ("16" in provider.icons) return provider.icons["16"];
...
async function applyProvider(id) {
  ...
  if (provider?.subPanel) {
    ...
    const lastHeight = await browser.sessions
      .getWindowValue(
        mTargetWindow,
        `${Constants.kWINDOW_STATE_SUBPANEL_HEIGHT}:${id}`
      )
      .catch(ApiTabs.createErrorHandler());
    ...
    if (activeItem) activeItem.classList.add("checked");
    browser.sessions
      .setWindowValue(
        mTargetWindow,
        Constants.kWINDOW_STATE_SUBPANEL_PROVIDER_ID,
    ...
    if (mHeight > 0) load(provider.subPanel);
    else load();
  } else {
...
async function restoreLastProvider() {
  ...
  if (lastProvider?.subPanel) await applyProvider(lastProvider.id);
  else if (mSelector.hasChildNodes())
    await applyProvider(mSelector.firstChild.dataset.value);
  else await applyProvider((mProviderId = null));
}
...
async function load(params) {
  ...
  if (url === mSubPanel.src && url !== "about:blank") {
...
function updateLayout() {
  if (!mProviderId && !mSelector.hasChildNodes()) {
    ...
    const maxHeight =
      window.innerHeight *
      Math.max(0, Math.min(1, configs.maxSubPanelSizeRatio));
    const appliedHeight = Math.min(maxHeight, mHeight);
    document.documentElement.style.setProperty(
      "--subpanel-content-size",
      `${appliedHeight}px`
    );
    document.documentElement.style.setProperty(
      "--subpanel-area-size",
      `${appliedHeight + headerSize}px`
    );

    if (mHeight > 0 && (!mSubPanel.src || mSubPanel.src === "about:blank")) {
      // delayed load
      const provider = TSTAPI.getAddon(mProviderId);
      if (provider?.subPanel) mSubPanel.src = provider.subPanel.url;
    } else if (mHeight === 0) {
  ...
  if (mProviderId) {
    browser.sessions
      .setWindowValue(
        mTargetWindow,
        Constants.kWINDOW_STATE_SUBPANEL_HEIGHT,
        mHeight
      )
      .catch(ApiTabs.createErrorHandler());
    browser.sessions
      .setWindowValue(
        mTargetWindow,
        `${Constants.kWINDOW_STATE_SUBPANEL_HEIGHT}:${mProviderId}`,
        mHeight
      )
      .catch(ApiTabs.createErrorHandler());
  }
...
async function toggle() {
  const lastEffectiveHeight = await browser.sessions
    .getWindowValue(
      mTargetWindow,
      Constants.kWINDOW_STATE_SUBPANEL_EFFECTIVE_HEIGHT
    )
    .catch(ApiTabs.createErrorHandler());
  if (mHeight > 0)
    browser.sessions
...
function saveLastHeight() {
  configs.lastSubPanelHeight = mContainer.classList.contains("collapsed")
    ? 0
    : mHeight;
}
...
function isResizable() {
  ...
  return (
    !provider || !provider.subPanel || !("fixedHeight" in provider.subPanel)
  );
}

mHeader.addEventListener("mousedown", (event) => {
  if (isFiredOnClickable(event)) return;
...
function updateSelector() {
  ...
  for (const [id, addon] of TSTAPI.getAddons()) {
    ...
    if (url) icon.src = url;
    item.appendChild(
      document.createTextNode(addon.subPanel.title || addon.name || id)
    );
    items.push(item);
  }
