...
import { TabElement, TabInvalidationTarget } from "./components/TabElement.js";
...
const mBrowserThemeDefinition = document.querySelector(
  "#browser-theme-definition"
);
const mUserStyleRules = document.querySelector("#user-style-rules");
const mContextualIdentitiesStyle = document.querySelector(
  "#contextual-identity-styling"
);
...

{
  ...
  if (Number.isNaN(mTargetWindow) || mTargetWindow < 1) mTargetWindow = null;
...
export async function init() {
  ...
  await Promise.all([
    MetricsData.addAsync("getting native tabs", async () => {
      ...
      if (win.focused) document.documentElement.classList.add("active");
      const trackedWindow = TabsStore.windows.get(win.id) || new Window(win.id);
      trackedWindow.incognito = win.incognito;
      if (win.incognito) document.documentElement.classList.add("incognito");
      ...
      promisedAllTabsTracked = MetricsData.addAsync(
        "tracking all native tabs",
        async () => {
          ...
          for (const tab of tabs.slice(1)) {
            ...
            if (
              Date.now() - lastDraw >
              configs.intervalToUpdateProgressForBlockedUserOperation
            ) {
              UserOperationBlocker.setProgress(
                Math.round((++count / maxCount) * 16) + 16
              ); // 2/6: track all tabs
              await nextFrame();
              lastDraw = Date.now();
            }
  ...
  const promisedScrollPosition = browser.sessions
    .getWindowValue(mTargetWindow, Constants.kWINDOW_STATE_SCROLL_POSITION)
    .catch(ApiTabs.createErrorHandler());
  ...
  const promisedResults = Promise.all([
    MetricsData.addAsync(
      "importTabsFromBackground()",
      importTabsFromBackground()
    ),
  ...
  await Promise.all([
    MetricsData.addAsync("parallel initialization: main", async () => {
      await MetricsData.addAsync(
        "parallel initialization: main: rebuildAll",
        rebuildAll(importedTabs)
      );
    ...
    MetricsData.addAsync(
      "parallel initialization: contextual identities",
      async () => {
    ...
    MetricsData.addAsync(
      "parallel initialization: TabContextMenu",
      async () => {
        TabContextMenu.init();
      }
    ),
  ...
  await MetricsData.addAsync(
    "parallel initialization: post process",
    Promise.all([
      MetricsData.addAsync(
        "parallel initialization: post process: main",
        async () => {
      ...
      MetricsData.addAsync(
        "parallel initialization: post process: Scroll.init",
        async () => {
          ...
          Scroll.onVirtualScrollViewportUpdated.addListener((resized) => {
            ...
            updateTabbarLayout({
              reason:
                Constants.kTABBAR_UPDATE_REASON_VIRTUAL_SCROLL_VIEWPORT_UPDATE,
            });
          });
        }
      ),
  ...
  if (configs.debug)
    log(
      `Startup metrics for ${Tab.getTabs(mTargetWindow).length} tabs: `,
...
function applyAnimationState(active) {
  ...
  if (active) rootClasses.add("animation");
  else rootClasses.remove("animation");
}
...
async function applyTheme({ style } = {}) {
  ...
  applyBrowserTheme(theme);
  applyUserStyleRules();
  if (mReloadMaskImage) reloadAllMaskImages();

  Size.updateTabs();
}

async function applyOwnTheme(style) {
  if (!style) style = configs.style;
  switch (style) {
    case "proton":
  ...
  processAllStyleRulesIn(mUserStyleRules.sheet, (rule) => {
    ...
    rule.selectorText = CssSelectorParser.splitSelectors(rule.selectorText)
      .map((selector) => {
        ...
        parts[0] = CssSelectorParser.appendPart(
          parts[0],
          CSS_SPECIFICITY_INCREASER
        );
        return parts.join(" ");
      })
      .join(", ");
...
function processAllStyleRulesIn(sheetOrRule, processor) {
  for (const rule of sheetOrRule.cssRules) {
    if (rule.styleSheet) processAllStyleRulesIn(rule.styleSheet, processor);
...
async function applyBrowserTheme(theme) {
  ...
  const baseColor = Color.parseCSSColor(
    window.getComputedStyle(
      document.querySelector("#dummy-tab-color-box"),
      null
    ).backgroundColor
  );
  const highlightColor = Color.parseCSSColor(
    window.getComputedStyle(
      document.querySelector("#dummy-highlight-color-box"),
      null
    ).backgroundColor
  );
  ...
  mBrowserThemeDefinition.textContent = [defaultColors, browserThemeStyle].join(
    "\n"
  );
}
...
function updateContextualIdentitiesStyle() {
  ...
  const definitions = Object.keys(colorInfo.colors).map(
    (id) =>
...
function reloadAllMaskImages() {
  ...
  for (const sheet of document.styleSheets) {
    processAllStyleRulesIn(sheet, (rule) => {
      ...
      if (background) rule.style.background = "none";
      rule.style.maskImage = "";

      delayedTasks.push(() => {
        rule.style.maskImage = image;
        if (background) rule.style.background = background;
      });
    });
  }
  setTimeout(() => {
...
function updateContextualIdentitiesSelector() {
  const disabled =
    ...
    ContextualIdentities.getCount() === 0;

  const anchors = document.querySelectorAll(
    `.${Constants.kCONTEXTUAL_IDENTITY_SELECTOR}-marker`
  );
  for (const anchor of anchors) {
    if (disabled) anchor.setAttribute("disabled", true);
    else anchor.removeAttribute("disabled");
  }

  const selector = document.getElementById(
    Constants.kCONTEXTUAL_IDENTITY_SELECTOR
  );
  ...
  const fragment = ContextualIdentities.generateMenuItems({
    hasDefault:
      configs.inheritContextualIdentityToChildTabMode !==
...
async function rebuildAll(importedTabs) {
  ...
  if (!trackedWindow) Window.init(mTargetWindow);

  if (!importedTabs)
    importedTabs = await MetricsData.addAsync(
      "rebuildAll: import tabs",
      browser.runtime
        .sendMessage({
  ...
  for (const tab of Tab.getAllTabs()) {
    if (!importedTabIds.has(tab.id)) Tab.untrack(tab.id);
  }
  ...
  for (const tab of tabs) {
    ...
    if (tab.active) TabsInternalOperation.setTabActive(trackedTab);
    if (trackedTab.pinned) SidebarTabs.renderTab(trackedTab);
    if (
      Date.now() - lastDraw >
      configs.intervalToUpdateProgressForBlockedUserOperation
    ) {
      UserOperationBlocker.setProgress(
        Math.round((++count / maxCount) * 33) + 66
      ); // 3/3: build tab elements
      await nextFrame();
      lastDraw = Date.now();
    }
  }
...
const mImportedTabs = new Promise((resolve, _reject) => {
  ...
  const onBackgroundIsReady = (message) => {
    if (mGiveUpImportTabs) {
      log(
        "mImportedTabs (${windowId}): give up to import, unregister onBackgroundIsReady listener"
      );
    ...
    mPromisedTargetWindow.then((windowId) => {
      if (mGiveUpImportTabs) {
        log(
          "mImportedTabs (${windowId}): give up to import, unregister onBackgroundIsReady listener (with promised target window)"
        );
        browser.runtime.onMessage.removeListener(onBackgroundIsReady);
        resolve([]);
        return;
      ...
      log(
        `mImportedTabs (${windowId}): onBackgroundIsReady `,
        message?.type,
        message?.windowId
      );
      if (
...
async function importTabsFromBackground() {
  ...
  try {
    const importedTabs = await MetricsData.addAsync(
      "importTabsFromBackground: kCOMMAND_PING_TO_BACKGROUND",
      browser.runtime
        .sendMessage({
          type: Constants.kCOMMAND_PING_TO_BACKGROUND,
          windowId: mTargetWindow,
    ...
    if (importedTabs) {
      log(
        "importTabsFromBackground: use response of kCOMMAND_PING_TO_BACKGROUND"
      );
  ...
  return MetricsData.addAsync(
    "importTabsFromBackground: kCOMMAND_PING_TO_SIDEBAR",
    mImportedTabs
  );
}
...
async function startWatchingWindowState() {
  if (!configs.enableWorkaroundForBug1875100) return;

  const browserInfo = await browser.runtime
    .getBrowserInfo()
    .catch(ApiTabs.createErrorHandler());
  if (parseInt(browserInfo.version.split(".")[0]) >= 124) return;
...
export async function confirmToCloseTabs(tabs, { configKey } = {}) {
  const tabIds = [];
  if (!configKey) configKey = "warnOnCloseTabs";
  ...
  if (count <= 1 || !configs[configKey]) return true;
  ...
  try {
    ...
    if (granted) {
      ...
      configs.grantedRemovingTabIds = Array.from(
        new Set((configs.grantedRemovingTabIds || []).concat(tabIds))
      );
...
export function reserveToUpdateTabbarLayout({ reason, timeout } = {}) {
  ...
  if (typeof timeout !== "number") timeout = 10;
  reserveToUpdateTabbarLayout.timeout = Math.max(
    timeout,
...
function updateTabbarLayout({ reason, reasons, timeout, justNow } = {}) {
  ...
  if (configs.debug) {
    ...
    if (
      reasons & Constants.kTABBAR_UPDATE_REASON_VIRTUAL_SCROLL_VIEWPORT_UPDATE
    )
  ...
  const previousLastVisibleTab =
    mLastVisibleTabId && Tab.get(mLastVisibleTabId);
  if (
    previousLastVisibleTab &&
    (!lastVisibleTab || lastVisibleTab.id !== previousLastVisibleTab.id)
  )
  ...
  const visibleNewTabButton = document.querySelector(
    "#tabbar:not(.overflow) .after-tabs .newtab-button-box, #tabbar.overflow ~ .after-tabs .newtab-button-box"
  );
  const newTabButtonSize = visibleNewTabButton.offsetHeight;
  const extraTabbarTopContainerSize =
    document.querySelector("#tabbar-top > *").offsetHeight;
  const extraTabbarBottomContainerSize =
    document.querySelector("#tabbar-bottom > *").offsetHeight;
  log("height: ", {
    newTabButtonSize,
    extraTabbarTopContainerSize,
    extraTabbarBottomContainerSize,
  });

  document.documentElement.style.setProperty(
    "--tabbar-top-area-size",
    `${extraTabbarTopContainerSize}px`
  );
  document.documentElement.style.setProperty(
    "--tabbar-bottom-area-size",
    `${extraTabbarBottomContainerSize}px`
  );
  document.documentElement.style.setProperty(
    "--after-tabs-area-size",
    `${newTabButtonSize}px`
  );
  ...
  if (
    SidebarTabs.normalContainer.classList.contains(
      Constants.kTABBAR_STATE_OVERFLOW
    )
  ) {
    const updatedAt = (updateTabbarLayout.lastScrollbarAutohideUpdatedAt =
      Date.now());
    window.requestAnimationFrame(() => {
      if (
        updatedAt !== updateTabbarLayout.lastScrollbarAutohideUpdatedAt ||
        !SidebarTabs.normalContainer.classList.contains(
          Constants.kTABBAR_STATE_OVERFLOW
        )
      )
      ...
      const virtualScrollContainer = document.querySelector(
        ".virtual-scroll-container"
      );
      const scrollbarOffset =
        mTabBar.offsetWidth - virtualScrollContainer.offsetWidth;

      const lastState = mTabBar.classList.contains(
        Constants.kTABBAR_STATE_SCROLLBAR_AUTOHIDE
      );
  ...
  if (justNow) PinnedTabs.reposition({ reasons, timeout, justNow });
  else PinnedTabs.reserveToReposition({ reasons, timeout, justNow });
}
...
Scroll.onNormalTabsOverflow.addListener(() => {
  ...
  window.requestAnimationFrame(() => {
    ...
    if (
      updateTabbarLayout.lastUpdateReasons &
        Constants.kTABBAR_UPDATE_REASON_TAB_OPEN &&
      !Scroll.isTabInViewport(lastOpenedTab)
    ) {
      log(
        "scroll to last opened tab on updateTabbarLayout ",
        updateTabbarLayout.lastUpdateReasons
      );
...
Scroll.onNormalTabsUnderflow.addListener(() => {
  ...
  SidebarTabs.normalContainer.classList.remove(
    Constants.kTABBAR_STATE_OVERFLOW
  );
...
function onTransisionEnd(event) {
  if (
    event.pseudoElement || // ignore size change of pseudo elements because they won't change height of tabbar contents
...
function onBrowserThemeChanged(updateInfo) {
  if (
    !updateInfo.windowId || // reset to default
...
CollapseExpand.onUpdated.addListener((_tab, options) => {
  const reason = options.collapsed
    ? Constants.kTABBAR_UPDATE_REASON_COLLAPSE
...
async function onConfigChange(changedKey) {
  ...
  switch (changedKey) {
    ...
    case "maxPinnedTabsRowsAreaPercentage":
      rootClasses.toggle(
        Constants.kTABBAR_STATE_FAVICONIZE_PINNED_TABS,
        configs[changedKey]
      );
    ...
    case "showContextualIdentitiesSelector":
      rootClasses.toggle(
        Constants.kTABBAR_STATE_CONTEXTUAL_IDENTITY_SELECTABLE,
        configs[changedKey]
      );
      break;
    ...
    case "showNewTabActionSelector":
      rootClasses.toggle(
        Constants.kTABBAR_STATE_NEWTAB_ACTION_SELECTABLE,
        configs[changedKey]
      );
      break;
    ...
    case "shiftTabsForScrollbarOnlyOnHover":
      document.documentElement.classList.toggle(
        "shift-tabs-for-scrollbar-only-on-hover",
        !!configs[changedKey]
      );
      break;

    default:
      if (changedKey.startsWith("chunkedUserStyleRules")) applyUserStyleRules();
      break;
  }
}
...
async function isSidebarRightSide() {
  ...
  if (window.screenX < 0 && window.devicePixelRatio > 1) return false;
  const mayBeRight =
    window.mozInnerScreenX - window.screenX >
    (window.outerWidth - window.innerWidth) / 2;
  if (
    configs.sidebarPosition === Constants.kTABBAR_POSITION_AUTO &&
    mayBeRight &&
    !configs.sidebarPositionRighsideNotificationShown
  ) {
    if (mTargetWindow !== (await browser.windows.getLastFocused({})).id) return;
...
function onMessage(message, _sender, _respond) {
  ...
  if (message.windowId && message.windowId !== mTargetWindow) return;
  ...
  switch (message.type) {
    ...
    case Constants.kCOMMAND_SHOW_DIALOG:
      return RichConfirm.show({
        ...message.params,
        onHidden() {
          UserOperationBlocker.unblockIn(
            mTargetWindow,
            message.userOperationBlockerParams || {}
          );
        },
      });
    ...
    case Constants.kCOMMAND_GET_SIDEBAR_POSITION:
      return Promise.resolve(
        document.documentElement.classList.contains("right")
...
BackgroundConnection.onMessage.addListener(async (message) => {
  switch (message.type) {
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_ATTACHED_TO_WINDOW:
      if (message.tabId) await Tab.waitUntilTracked(message.tabId);
      reserveToUpdateTabbarLayout({
    ...
    case Constants.kCOMMAND_BOOKMARK_TABS_WITH_DIALOG:
      {
        Bookmark.bookmarkTabs(
          mapAndFilter(message.tabIds, (id) => Tab.get(id)),
          {
            ...(message.options || {}),
            showDialog: true,
          }
