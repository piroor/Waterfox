...
export function init() {
  document
    .querySelector("#sync-throbber")
    .addEventListener("animationiteration", synchronizeThrobberAnimation);

  document.documentElement.setAttribute(
    Constants.kLABEL_OVERFLOW,
    configs.labelOverflowStyle
  );
...
export function getTabFromDOMNode(node, options = {}) {
  ...
  const tabSubstance = node?.closest(kTAB_SUBSTANCE_ELEMENT_NAME);
  const tab = tabSubstance?.closest(kTAB_ELEMENT_NAME);
  if (options.force) return tab?.apiTab;
  return TabsStore.ensureLivingTab(tab?.apiTab);
}
...
function updateLoadingState() {
  ...
  classList.toggle(
    Constants.kTABBAR_STATE_HAVE_LOADING_TAB,
    Tab.hasLoadingTab(windowId)
  );
  classList.toggle(
    Constants.kTABBAR_STATE_HAVE_UNSYNCHRONIZED_THROBBER,
...
async function synchronizeThrobberAnimation() {
  ...
  for (const tab of Tab.getNeedToBeSynchronizedTabs(
...
export function reserveToSyncTabsOrder() {
  ...
  if (reserveToSyncTabsOrder.timer) clearTimeout(reserveToSyncTabsOrder.timer);
...
async function syncTabsOrder() {
  ...
  if (expectedTabs !== nativeTabs) {
    if (reserveToSyncTabsOrder.retryCount > 10) {
      console.error(
        new Error(
          `Fatal error: native tabs are not same to the tabs tracked by the background process, for the window ${windowId}. Reloading all...`
        )
      );
      reserveToSyncTabsOrder.retryCount = 0;
      browser.runtime
    ...
    log(
      `syncTabsOrder: retry / Native tabs are not same to the tabs tracked by the background process, but this can happen when synchronization and tab removing are done in parallel. Retry count = ${reserveToSyncTabsOrder.retryCount}`
    );
  ...
  if (expectedTabs !== actualTabs) {
    log(
      `syncTabsOrder: retry / Native tabs are not same to the tabs tracked by the background process, but this can happen on synchronization and tab removing are. Retry count = ${reserveToSyncTabsOrder.retryCount}`
    );
    if (reserveToSyncTabsOrder.retryCount > 10) {
      console.error(
        new Error(
          `Error: tracked tabs are not same to pulled tabs, for the window ${windowId}. Rebuilding...`
        )
      );
      reserveToSyncTabsOrder.retryCount = 0;
    ...
    log(
      `syncTabsOrder: retry / Native tabs are not same to tab elements, but this can happen on synchronization and tab removing are. Retry count = ${reserveToSyncTabsOrder.retryCount}`
    );
    reserveToSyncTabsOrder.retryCount++;
    return reserveToSyncTabsOrder();
  }
...
export function renderTab(tab, { containerElement, insertBefore } = {}) {
  if (!tab) {
    console.log(
      "WARNING: Null tab has requested to be rendered! ",
      new Error().stack
    );
    return false;
  }
  if (!tab.$TST) {
    console.log(
      "WARNING: Alerady destroyed tab has requested to be rendered! ",
      tab.id,
      new Error().stack
    );
    return false;
  ...
  containerElement =
    containerElement ||
    (tab.pinned ? win.pinnedContainerElement : win.containerElement);
  ...
  if (
    ...
    log(
      `render tab element for ${tab.id} (pinned=${tab.pinned}) before ${nextTab?.id}, tab, nextTab = `,
      tab,
      nextTab
    );
    nextElement =
      nextTab && nextTab.$TST.element.parentNode === containerElement
        ? nextTab.$TST.element
        : null;
  }

  if (
    tabElement.parentNode === containerElement &&
  ...
  if (created) {
    ...
    tab.$TST.invalidateElement(
      TabInvalidationTarget.Twisty |
        TabInvalidationTarget.CloseBox |
        TabInvalidationTarget.Tooltip |
        TabInvalidationTarget.Overflow
    );
    tab.$TST.updateElement(
      TabUpdateTarget.Counter |
        TabUpdateTarget.Overflow |
        TabUpdateTarget.TabProperties
    );
...
function reserveToNotifyTabsRendered() {
  ...
  const hasExternalListener = TSTAPI.hasListenerForMessageType(
    TSTAPI.kNOTIFY_TABS_RENDERED
  );
  ...
  if (reserveToNotifyTabsRendered.invoked) return;
  ...
  window.requestAnimationFrame(() => {
    ...
    if (hasInternalListener) onTabsRendered.dispatch(tabs);
...
export function unrenderTab(tab) {
  if (!tab || !tab.$TST || !tab.$TST.element) return false;
  ...
  const hasExternalListener = TSTAPI.hasListenerForMessageType(
    TSTAPI.kNOTIFY_TABS_UNRENDERED
  );
  if (hasInternalListener || hasExternalListener) {
    if (!unrenderTab.invoked) {
      ...
      window.requestAnimationFrame(() => {
        ...
        if (hasInternalListener) onTabsUnrendered.dispatch(tabs);
  ...
  if (!tabElement || !tabElement.parentNode) return false;
  ...
  if (mClearPoolTimer) clearTimeout(mClearPoolTimer);
...
Window.onInitialized.addListener((win) => {
  ...
  let innerPinnedContainer = document.getElementById(
    `window-${windowId}-pinned`
  );
  ...
  if (!innerNormalContainer) {
    innerNormalContainer = document
      .querySelector("#normal-tabs-container .virtual-scroll-container")
    ...
    case "renderHiddenTabs":
      {
        ...
        for (const tab of Tab.getHiddenTabs(TabsStore.getCurrentWindowId(), {
          iterator: true,
        })) {
          TabsStore.updateVirtualScrollRenderabilityIndexForTab(tab);
          if (!tab.pinned) hasNormalTab = true;
        }
        if (hasNormalTab) onNormalTabsChanged.dispatch();
      }
      break;
  }
});
...
export async function waitUntilNewTabIsMoved(tabId) {
  if (mAlreadyMovedNewTabs.has(tabId)) return true;
  if (mPromsiedMovedNewTabs.has(tabId)) return mPromsiedMovedNewTabs.get(tabId);
  const timer = setTimeout(
    () => {
      if (mMovedNewTabResolvers.has(tabId)) mMovedNewTabResolvers.get(tabId)();
    },
...
function setupPendingUpdate(update) {
  const pendingUpdate = mPendingUpdates.get(update.tabId) || {
    tabId: update.tabId,
  };
  ...
  if (update.removedAttributes.size > 0) {
    pendingUpdate.removedAttributes = new Set([
      ...(pendingUpdate.removedAttributes || []),
  ...
  if (update.removedStates.size > 0) {
    pendingUpdate.removedStates = new Set([
      ...(pendingUpdate.removedStates || []),
      ...update.removedStates,
    ]);
  ...
  if (update.addedStates.size > 0) {
    pendingUpdate.addedStates = new Set([
      ...(pendingUpdate.addedStates || []),
      ...update.addedStates,
    ]);
  ...
  pendingUpdate.soundStateChanged =
    pendingUpdate.soundStateChanged || update.soundStateChanged;
...
function tryApplyUpdate(update) {
  ...
  const highlightedChanged =
    update.updatedProperties && "highlighted" in update.updatedProperties;

  if (update.updatedProperties) {
    for (const [key, value] of Object.entries(update.updatedProperties)) {
      if (Tab.UNSYNCHRONIZABLE_PROPERTIES.has(key)) continue;
      tab[key] = value;
    }
  }
  ...
  tab.$TST.invalidateElement(
    TabInvalidationTarget.SoundButton | TabInvalidationTarget.Tooltip
  );
  ...
  if (highlightedChanged) {
    ...
    if (mReservedUpdateActiveTab) clearTimeout(mReservedUpdateActiveTab);
    mReservedUpdateActiveTab = setTimeout(() => {
      mReservedUpdateActiveTab = null;
      const activeTab = Tab.getActiveTab(tab.windowId);
      activeTab.$TST.invalidateElement(
        TabInvalidationTarget.SoundButton | TabInvalidationTarget.CloseBox
      );
    }, 50);
  }
}
...
async function activateRealActiveTab(windowId) {
  ...
  if (!tab)
    throw new Error(
      `FATAL ERROR: Active tab ${id} in the window ${windowId} is not tracked`
    );
...
function reserveToUpdateTabsIndex() {
  if (reserveToUpdateTabsIndex.invoked) return;
...
BackgroundConnection.onMessage.addListener(async (message) => {
  switch (message.type) {
    ...
    case Constants.kCOMMAND_BROADCAST_TAB_STATE:
      {
        if (!message.tabIds.length) break;
        ...
        for (const id of message.tabIds) {
          ...
          if (
            modified.has(Constants.kTAB_STATE_AUDIBLE) ||
        ...
        if (
          stickyStateChanged ||
          [...Tab.autoStickyStates.values()].some(
            (states) =>
              new Set([...states, ...modified]).size <
              states.size + modified.size
          )
        )
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_CREATING:
      {
        ...
        if (mRemovedTabIdsNotifiedBeforeTracked.has(nativeTab.id)) {
          log(
            `ignore kCOMMAND_NOTIFY_TAB_CREATING for already closed tab: ${nativeTab.id}`
          );
          return;
        }
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_CREATED:
      {
        ...
        if (!tab) {
          log(
            `ignore kCOMMAND_NOTIFY_TAB_CREATED for already closed tab: ${message.tabId}`
          );
          return;
        }
        tab.$TST.removeState(Constants.kTAB_STATE_ANIMATION_READY);
        tab.$TST.resolveOpened();
        if (message.maybeMoved) await waitUntilNewTabIsMoved(message.tabId);
        if (tab.pinned) {
          renderTab(tab);
        ...
        if (tab.active) {
          if (shouldApplyAnimation()) {
            ...
            if (
              !message.collapsed /* the new tab may be really collapsed not just for animation, and we should not expand */ &&
          ...
          const lastMessage = BackgroundConnection.fetchBufferedMessage(
            Constants.kCOMMAND_NOTIFY_TAB_ACTIVATED,
            `${BUFFER_KEY_PREFIX}window-${message.windowId}`
          );
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_ACTIVATED:
      {
        ...
        if (!tab) return;
        const lastActive = TabsStore.activeTabInWindow.get(
          lastMessage.windowId
        );
        if (lastActive)
          getTabContainerElement(lastActive).removeAttribute(
            "aria-activedescendant"
          );
        TabsStore.activeTabInWindow.set(lastMessage.windowId, tab);
        TabsInternalOperation.setTabActive(tab);
        getTabContainerElement(tab).setAttribute(
          "aria-activedescendant",
          getTabElementId(tab)
        );
      }
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_MOVED:
      {
        ...
        const tab = Tab.get(message.tabId);
        if (!tab || tab.index === message.toIndex) return;
        if (mPromisedInitialized) await mPromisedInitialized;
        if (tab.$TST.parent)
    ...
    case Constants.kCOMMAND_UPDATE_LOADING_STATE:
      {
        if (
          BackgroundConnection.handleBufferedMessage(
            message,
            `${BUFFER_KEY_PREFIX}${message.tabId}`
          )
        )
          return;
        await Tab.waitUntilTracked(message.tabId);
          ...
          if (lastMessage.status === "loading") {
            ...
            if (lastMessage.reallyChanged) {
              ...
              tab.$TST.delayedBurstEnd = setTimeout(() => {
                ...
                if (!tab.active)
                  tab.$TST.addState(
                    Constants.kTAB_STATE_NOT_ACTIVATED_SINCE_LOAD
                  );
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_REMOVING:
      {
        ...
        if (!tab) {
          log(
            `ignore kCOMMAND_NOTIFY_TAB_REMOVING for already closed tab: ${message.tabId}`
          );
        ...
        if (!tab.$TST.collapsed && shouldApplyAnimation()) {
          ...
          if (tab.pinned) onPinnedTabsChanged.dispatch(tab);
          else onNormalTabsChanged.dispatch(tab);
        }
      }
      break;
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_REMOVED:
      {
        ...
        if (!tab) {
          log(
            `ignore kCOMMAND_NOTIFY_TAB_REMOVED for already closed tab: ${message.tabId}`
          );
          return;
        }
        if (tab.active) {
        ...
        if (shouldApplyAnimation()) await wait(configs.collapseDuration);
        TabsStore.windows.get(message.windowId).detachTab(message.tabId);
        tab.$TST.destroy();
        unrenderTab(tab);
        if (tab.pinned) onPinnedTabsChanged.dispatch(tab);
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_LABEL_UPDATED:
      {
        ...
        if (!tab || !lastMessage) return;
        tab.title = lastMessage.title;
        tab.$TST.label = lastMessage.label;
        if (tab.$TST.element) tab.$TST.element.label = lastMessage.label;
        tab.$TST.invalidateCache();
      }
      break;
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_SOUND_STATE_UPDATED:
      {
        ...
        tab.$TST.toggleState(
          ...
          lastMessage.hasSoundPlayingMember
        );
        tab.$TST.toggleState(
          Constants.kTAB_STATE_HAS_MUTED_MEMBER,
          lastMessage.hasMutedMember
        );
        tab.$TST.toggleState(
          Constants.kTAB_STATE_HAS_AUTOPLAY_BLOCKED_MEMBER,
          lastMessage.hasAutoplayBlockedMember
        );
        tab.$TST.invalidateElement(TabInvalidationTarget.SoundButton);
      }
      break;
    ...
    case Constants.kCOMMAND_NOTIFY_HIGHLIGHTED_TABS_CHANGED:
      {
        ...
        const win = TabsStore.windows.get(message.windowId);
        if (!win || !win.containerElement) return;
        document.documentElement.classList.toggle(
          Constants.kTABBAR_STATE_MULTIPLE_HIGHLIGHTED,
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_SHOWN:
      {
        ...
        if (lastMessage.message.type === Constants.kCOMMAND_NOTIFY_TAB_HIDDEN) {
          ...
          tab.hidden = false;
          if (!tab.$TST.collapsed) TabsStore.addVisibleTab(tab);
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_COLLAPSED_STATE_CHANGED:
      {
        ...
        if (!tab || !lastMessage || lastMessage.collapsed) return;
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_ATTACHED_TO_WINDOW:
      {
        ...
        if (!tab) return;
        tab.$TST.invalidateCache();
        if (tab.active) TabsInternalOperation.setTabActive(tab); // to clear "active" state of other tabs
      }
      break;
    ...
    case Constants.kCOMMAND_NOTIFY_CHILDREN_CHANGED:
      {
        if (mPromisedInitialized) return;
        ...
        // cases.
        const relatedTabIds = [message.tabId].concat(
          message.addedChildIds,
          message.removedChildIds
        );
        ...
        if (message.addedChildIds.length > 0) {
          ...
          const childLevel =
            parseInt(tab.$TST.getAttribute(Constants.kLEVEL) || 0) + 1;
          for (const childId of message.addedChildIds) {
            const child = Tab.get(childId);
            if (!child || child.$TST.hasChild) continue;
