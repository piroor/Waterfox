...
function getTabHeight() {
  return configs.faviconizePinnedTabs
    ? Size.getRenderedFavIconizedTabHeight()
...
export function reposition(options = {}) {
  ...
  const width = faviconized
    ? Size.getRenderedFavIconizedTabWidth()
    : maxWidth + Size.getFavIconizedTabXOffset();
  const height = getTabHeight();
  const maxCol = faviconized
    ? Math.max(
        1,
  ...
  const pinnedTabsAreaRatio =
    Math.min(Math.max(0, configs.maxPinnedTabsRowsAreaPercentage), 100) / 100;
  const allTabsAreaHeight =
    Size.getAllTabsAreaSize() + GapCanceller.getOffset();
  mMaxVisibleRows = Math.max(
    1,
    Math.floor((allTabsAreaHeight * pinnedTabsAreaRatio) / height)
  );
  const contentsHeight =
    height * maxRow +
    (faviconized ? Size.getFavIconizedTabYOffset() : Size.getTabYOffset());
  mAreaHeight = Math.min(contentsHeight, mMaxVisibleRows * height);
  ...
  for (const tab of pinnedTabs) {
    ...
    if (row === maxRow - 1) mMaxColLastRow = col;
  ...
  log(
    "overflow: contentsHeight > mAreaHeight : ",
    contentsHeight > mAreaHeight
  );
  SidebarTabs.pinnedContainer.classList.toggle(
    "overflow",
    contentsHeight > mAreaHeight
  );
}
...
export function reserveToReposition(options = {}) {
  if (reserveToReposition.waiting) clearTimeout(reserveToReposition.waiting);
    ...
    case Constants.kCOMMAND_GET_ABOVE_TAB:
      {
        try {
          ...
          return Promise.resolve(mTabsMatrix.get(`${col}:${nextRow}`) || null);
        } catch (_error) {
          return Promise.resolve(mTabsMatrix.get(`0:${mMaxRow}`) || null);
        }
      }
      break;
    ...
    case Constants.kCOMMAND_GET_LEFT_TAB:
      {
        ...
        return Promise.resolve(mTabsMatrix.get(`${nextCol}:${row}`) || null);
      }
      break;
...
BackgroundConnection.onMessage.addListener(async (message) => {
  switch (message.type) {
    case Constants.kCOMMAND_NOTIFY_TAB_CREATED:
      {
        ...
        if (!tab) return;
        if (tab.pinned) reserveToReposition();
      }
      break;
    ...
    case Constants.kCOMMAND_NOTIFY_TAB_DETACHED_FROM_WINDOW:
      if (message.wasPinned) reserveToReposition();
      break;
