...
export function update(options = {}) {
  ...
  if (options.cache?.definition) {
    ...
    for (let i = 0; i <= mLastMaxLevel; i++) {
      generateIndentAndSelectorsForMaxLevel(
        i,
        indentToSelectors,
        defaultIndentToSelectors,
        indentUnitDefinitions
      );
    }
    ...
    for (const indentSet of [defaultIndentToSelectors, indentToSelectors]) {
      ...
      for (const indent of indents) {
        definitions.push(
          `${indentSet[indent].join(",\n")} { --tab-indent: ${indent}; }`
        );
      }
    }
    mIndentDefinition.textContent = indentUnitDefinitions
      .concat(definitions)
      .join("\n");
  }
}
function generateIndentAndSelectorsForMaxLevel(
  maxLevel,
  indentToSelectors,
  defaultIndentToSelectors,
  indentUnitDefinitions
  ...
  if (configuredMaxLevel < 0) configuredMaxLevel = Number.MAX_SAFE_INTEGER;
  ...
  defaultIndentToSelectors[defaultIndent].push(
    `${base}:not([${Constants.kLEVEL}="0"])`
  );
  ...
  for (let level = 1; level <= maxLevel; level++) {
    ...
    if (!indentToSelectors[indent]) indentToSelectors[indent] = [];
...
function calculateIndentUnit(maxLevel) {
  ...
  return Math.min(
    configs.baseIndent,
    Math.max(Math.floor(mLastMaxIndent / maxLevel), minIndent)
  );
}
...
export function tryUpdateVisualMaxTreeLevel() {
  ...
  if (
    tryUpdateVisualMaxTreeLevel.calledCount <=
      configs.maxAllowedImmediateRefreshCount &&
    !animation
  ) {
    updateVisualMaxTreeLevel();
...
function getMaxTreeLevel(windowId, options = {}) {
  ...
  let maxLevel = Math.max(
    ...tabs.map((tab) => parseInt(tab.$TST.attributes[Constants.kLEVEL] || 0))
  );
...
async function reserveToUpdateIndent() {
  ...
  const delay = shouldApplyAnimation()
...
CollapseExpand.onUpdated.addListener((tab, _options) => {
  ...
  if (
    (configs.indentAutoShrink && configs.indentAutoShrinkOnlyForVisible) ||
    // On no-animation mode, we should update max indent level immediately
    // as possible as we can without delay, to reduce visual flicking which
    // can trigger an epileptic seizure.
    // But we also have to reduce needless function calls for better performance.
    ...
    // See also: https://github.com/piroor/treestyletab/issues/3383
    (isFinishBatch && restVisibilityChangedTabIds.size === 0)
  )
