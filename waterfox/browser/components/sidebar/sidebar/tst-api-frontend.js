...
import { kTAB_ELEMENT_NAME } from "./components/TabElement.js";
...
const AUTO_REINSERT_PLACES = new Set(["tab-above", "tab-below"]);
...
const mNewTabButtonExtraItemsContainerRoots = Array.from(
  document.querySelectorAll(
    `.${Constants.kNEWTAB_BUTTON} .${Constants.kEXTRA_ITEMS_CONTAINER}`
  ),
  (container) => {
...
const mTabbarTopExtraItemsContainerRoot = (() => {
  const container = document.querySelector(
    `#tabbar-top > .${Constants.kEXTRA_ITEMS_CONTAINER}`
  );
  const root = container.attachShadow({ mode: "open" });
  root.itemById = new Map();
  return root;
...
const mTabbarBottomExtraItemsContainerRoot = (() => {
  const container = document.querySelector(
    `#tabbar-bottom > .${Constants.kEXTRA_ITEMS_CONTAINER}`
  );
  const root = container.attachShadow({ mode: "open" });
  root.itemById = new Map();
  return root;
...
TSTAPI.onRegistered.addListener((addon) => {
  ...
  if (addon.style) installStyle(addon.id, addon.style);
});
...
TSTAPI.onMessageExternal.addListener((message, sender) => {
  ...
  if (!message.windowId) message.windowId = message.window || mTargetWindow;

  if (message.windowId !== mTargetWindow) return;
...
const DANGEROUS_CONTENTS_SELECTOR = SAFE_CONTENTS.map(
  (selector) => `:not(${selector})`
).join("");

export function setExtraContentsTo(tab, id, params = {}) {
  if (!tab || !tab.$TST.element) return;
  if (typeof id !== "string") {
    // the addon id is optional
    params = id;
...
function setExtraContentsToContainer(container, id, params = {}) {
  ...
  if (AUTO_REINSERT_PLACES.has(place)) {
    const cacheHolderElement =
      container?.host?.closest(kTAB_ELEMENT_NAME) || container;
  ...
  if (!params.contents) {
    if (item) {
      if (item.styleElement) container.removeChild(item.styleElement);
      container.removeChild(item);
      container.itemById.delete(id);
    }
    if (cacheHolder) cacheHolder[cacheKey] = null;
    return;
  }
  ...
  if (cacheHolder) cacheHolder[cacheKey] = contentsSource;
  ...
  const dangerousContents = contents.querySelectorAll(
    DANGEROUS_CONTENTS_SELECTOR
  );
  ...
  if (dangerousContents.length > 0)
    console.log(
      `Could not include some elements as extra contents. provider=${id}, container:`,
      container,
      dangerousContents
    );
  ...
  for (const node of contents.querySelectorAll(
    ...
    for (const attribute of node.attributes) {
      ...
      if (
        ...
        node.setAttribute(
          "part",
          `${node.getAttribute("part") || ""} sanitized`
        );
      }
    }
  }
  ...
  if ("style" in params)
    item.styleElement.textContent = (params.style || "").replace(
      /%EXTRA_CONTENTS_PART%/gi,
      `${extraContentsPartName}`
    );

  DOMUpdater.update(item, contents);

  if (item.styleElement && !item.styleElement.parentNode)
...
function setExtraTabContentsToElement(tabElement, id, params = {}) {
  ...
  if (container) return setExtraContentsToContainer(container, id, params);
}
...
function onExtraContentsAboveChanged(id, params) {
  ...
  if (onExtraContentsAboveChanged.invoked) return;
...
function onExtraContentsBelowChanged(id, params) {
  ...
  if (onExtraContentsBelowChanged.invoked) return;
...
function throttledUpdateSize() {
  if (throttledUpdateSize.invoked) return;
...
export function clearAllExtraTabContents(id) {
  ...
  if (!mAddonsWithExtraContents.has(id)) return;
...
export function clearExtraNewTabButtonContents(id) {
  if (!id) setExtraNewTabButtonContents({});
  setExtraNewTabButtonContents(id, {});
}
...
function collectExtraContentsRoots({ tabs, place }) {
  switch (String(place).toLowerCase()) {
    case "indent": // for backward compatibility
    case "tab-indent":
      return (tabs || Tab.getAllTabs(mTargetWindow)).map(
        (tab) => tab.$TST.element.extraItemsContainerIndentRoot
      );

    case "behind": // for backward compatibility
    case "tab-behind":
      return (tabs || Tab.getAllTabs(mTargetWindow)).map(
        (tab) => tab.$TST.element.extraItemsContainerBehindRoot
      );

    case "front": // for backward compatibility
    case "tab-front":
      return (tabs || Tab.getAllTabs(mTargetWindow)).map(
        (tab) => tab.$TST.element.extraItemsContainerFrontRoot
      );

    case "tab-above":
      return [
        ...(tabs || Tab.getAllTabs(mTargetWindow)).map(
          (tab) => tab.$TST.element.extraItemsContainerAboveRoot
        ),
    ...
    case "tab-below":
      return [
        ...(tabs || Tab.getAllTabs(mTargetWindow)).map(
          (tab) => tab.$TST.element.extraItemsContainerBelowRoot
        ),
...
function setExtraContentsProperties({ id, tabs, place, part, properties }) {
  if (!id || !part || !properties) return;

  const roots = collectExtraContentsRoots({ id, tabs, place });
  for (const root of roots) {
    const node = root.querySelector(
      `[part~="${getExtraContentsPartName(id)}"][part~="${part}"]`
    );
    if (!node) continue;
...
function installStyle(id, style) {
  ...
  if (!styleElement) {
    ...
    document.head.insertBefore(
      styleElement,
      document.querySelector("#addons-style-rules")
    );
    mAddonStyles.set(id, styleElement);
  }
  styleElement.textContent = (style || "").replace(
    /%EXTRA_CONTENTS_PART%/gi,
    getExtraContentsPartName(id)
  );
}
...
for (const container of document.querySelectorAll(
  ...
  container.addEventListener("dblclick", onExtraContentsDblClick, {
    capture: true,
  });
}
document.addEventListener("keydown", onExtraContentsKeyEvent, {
  capture: true,
});
...
document.addEventListener("compositionstart", onExtraContentsCompositionEvent, {
  capture: true,
});
document.addEventListener(
  "compositionupdate",
  onExtraContentsCompositionEvent,
  { capture: true }
);
document.addEventListener("compositionend", onExtraContentsCompositionEvent, {
  capture: true,
});
document.addEventListener("focus", onExtraContentsFocusEvent, {
  capture: true,
});
...
async function notifyExtraContentsEvent(event, eventType, details = {}) {
  ...
  await TSTAPI.tryOperationAllowed(eventType, eventInfo, options);
  if (eventInfo.tab) eventInfo.tab.clearCache();
}
...
async function onExtraContentsInput(event) {
  await notifyExtraContentsEvent(event, TSTAPI.kNOTIFY_EXTRA_CONTENTS_INPUT, {
    data: event.data,
...
async function onExtraContentsChange(event) {
  await notifyExtraContentsEvent(event, TSTAPI.kNOTIFY_EXTRA_CONTENTS_CHANGE);
}
...
async function onExtraContentsFocusEvent(event) {
  ...
  const relatedTargetNode =
    event.relatedTarget && EventUtils.getElementTarget(event.relatedTarget);
  const relatedExtraContents = relatedTargetNode?.closest(`.extra-item`);
  if (
    relatedExtraContents &&
  ...
  try {
    ...
    if (extraContents) {
      ...
      return {
        ...
        targetPart: cleanupExtraContentsPartName(
          targetPart.getAttribute("part")
        ).trim(),
...
function cleanupExtraContentsPartName(string) {
  return string.replace(/ extra-contents-by-[-a-z0-9_]+\b/gi, "");
}

export async function tryMouseOperationAllowedWithExtraContents(
  eventType,
  rawEventType,
  mousedown,
  extraContentsInfo
  ...
  if (extraContentsInfo?.owners && extraContentsInfo.owners.size > 0) {
    const eventInfo = {
      ...mousedown.detail,
      originalTarget: extraContentsInfo.target,
      originalTargetPart: extraContentsInfo.targetPart,
