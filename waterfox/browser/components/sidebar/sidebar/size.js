...
export function updateTabs() {
  ...
  mFavIconizedTabXOffset =
    favIconizedMarginLeft > 0 && favIconizedMarginRight > 0
  ...
  mFavIconizedTabYOffset =
    favIconizedMarginTop > 0 && favIconizedMarginBottom > 0
  ...
  mTabXOffset =
    parseFloat(tabStyle.marginLeft) + parseFloat(tabStyle.marginRight);
  // simulating margin collapsing
  mTabMarginTop = parseFloat(tabStyle.marginTop);
  mTabMarginBottom = parseFloat(tabStyle.marginBottom);
  mTabYOffset =
    mTabMarginTop > 0 && mTabMarginBottom > 0
      ? Math.max(mTabMarginTop, mTabMarginBottom)
      : mTabMarginTop + mTabMarginBottom;

  const substanceRect = dummyTab
    .querySelector("tab-item-substance")
    .getBoundingClientRect();
  const uiRect = dummyTab
    .querySelector("tab-item-substance > .ui")
    .getBoundingClientRect();
  const captionRect = dummyTab
    .querySelector("tab-item-substance > .ui > .caption")
    .getBoundingClientRect();
  const favIconRect = dummyTab
    .querySelector("tab-favicon")
    .getBoundingClientRect();
  const labelRect = dummyTab.querySelector("tab-label").getBoundingClientRect();
  const closeBoxRect = dummyTab
    .querySelector("tab-closebox")
    .getBoundingClientRect();

  let shiftTabsForScrollbarDistance =
    configs.shiftTabsForScrollbarDistance.trim() || "0";
  if (
    !/^[0-9.]+(cm|mm|Q|in|pc|pt|px|em|ex|ch|rem|lh|vw|vh|vmin|vmax|%)$/.test(
      shiftTabsForScrollbarDistance
    )
  )
  ...
  if (modifiedAtPhase1) sizeDefinitionHolder.textContent = sizeDefinition;
  ...
  const finalSizeDefinition = sizeDefinition.replace(
    /--tab-size:[^;]+;/,
    `--tab-size: ${mTabHeight}px;`
  );
  const modifiedAtPhase2 =
  ...
  if (modifiedAtPhase1 || modifiedAtPhase2) onUpdated.dispatch();
}
...
export function updateContainers() {
  ...
  const pinnedContainerStyle = window.getComputedStyle(
    pinnedContainerBox,
    null
  );
  const newPinnedTabsContainerWidth =
    pinnedContainerBoxRect.width -
    parseFloat(pinnedContainerStyle.paddingLeft) -
    parseFloat(pinnedContainerStyle.borderLeftWidth) -
    parseFloat(pinnedContainerStyle.paddingRight) -
    parseFloat(pinnedContainerStyle.borderRightWidth);
  ...
  const normalTabsViewPortFollowingAreaSize =
    range.getBoundingClientRect().height;
  const newNormalTabsViewportSize =
    mTabBar.offsetHeight -
    normalTabsViewPortPrecedingAreaSize -
    normalTabsViewPortFollowingAreaSize;
  range.detach();
  if (newNormalTabsViewportSize !== mNormalTabsViewPortSize) modifiedCount++;
  mNormalTabsViewPortSize = newNormalTabsViewportSize;

  const newAllTabsAreaSize = mTabBar.parentNode.offsetHeight;
  if (newAllTabsAreaSize !== mAllTabsAreaSize) modifiedCount++;
  mAllTabsAreaSize = newAllTabsAreaSize;

  if (modifiedCount > 0) onUpdated.dispatch();
}
