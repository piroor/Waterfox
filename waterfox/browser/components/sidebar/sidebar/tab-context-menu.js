...
async function rebuild() {
  ...
  for (const [id, extraItems] of mExtraItems.entries()) {
    ...
    if (!TSTAPI.isSafeAtIncognito(id, incognitoParams) || !addon) continue;
    ...
    if (icon) addonItem.dataset.icon = icon;
    prepareAsSubmenu(addonItem);

    const toBeBuiltItems = [];
    for (const item of extraItems) {
      if (item.visible === false) continue;
      if (item.contexts && !item.contexts.includes("tab")) continue;
      if (
        item.documentUrlPatterns &&
        (!item.viewTypes ||
          !item.viewTypes.includes("sidebar") ||
          item.documentUrlPatterns.some(
            (pattern) => !/^moz-extension:/.test(pattern)
          ) ||
    ...
    if (topLevelItems.length === 1 && !topLevelItems[0].icons)
    ...
    if (id === browser.runtime.id) {
      for (const item of addonSubMenu.children) {
        if (!item.nextSibling)
          // except the last "Tree Style Tab" menu
          continue;
  ...
  if (!extraItemNodes.hasChildNodes()) return;
...
function chooseIconForAddon(params) {
  ...
  let sizes = Object.keys(icons)
    .map((aSize) => parseInt(aSize))
    .sort(compareAsNumber);
  const reducedSizes = sizes.filter((aSize) => aSize < 16);
  if (reducedSizes.length > 0) sizes = reducedSizes;
...
function prepareAsSubmenu(itemNode) {
  if (itemNode.querySelector("ul")) return itemNode;
...
function buildExtraItem(item, ownerAddonId) {
  ...
  if (item.type === "checkbox" || item.type === "radio") {
    if (item.checked) itemNode.classList.add("checked");
  }
  ...
  if (icon) itemNode.dataset.icon = icon;
  return itemNode;
}
...
const matchPattern =
  /^(?:(\*|http|https|file|ftp|app|moz-extension):\/\/([^/]+|)\/?(.*))$/i;
function matchPatternToRegExp(pattern) {
  if (pattern === "<all_urls>") return /^(?:https?|file|ftp|app):\/\//;
...
export async function open(options = {}) {
  ...
  options.canceller = () => {
    return (
      (typeof originalCanceller === "function" && originalCanceller()) ||
      mIsDirty
    );
  };
...
function applyContext() {
  if (mContextTab) {
    ...
    const states = [];
    if (mContextTab.active) states.push("active");
    if (mContextTab.pinned) states.push("pinned");
    if (mContextTab.audible) states.push("audible");
    if (mContextTab.$TST.muted) states.push("muted");
    if (mContextTab.discarded) states.push("discarded");
    if (mContextTab.incognito) states.push("incognito");
    if (mContextTab.$TST.multiselected) states.push("multiselected");
...
async function onCommand(item, event) {
  ...
  if (event.ctrlKey) {
    modifiers.push("Ctrl");
    if (isMacOS()) modifiers.push("MacCtrl");
  }
  ...
  if (owner === browser.runtime.id) {
    ...
    TSTAPI.isSafeAtIncognito(owner, {
      tab: contextTab,
  ...
  if (item.matches(".checkbox")) {
    ...
    for (const itemData of mExtraItems.get(item.dataset.itemOwnerId)) {
      if (itemData.id !== item.dataset.itemId) continue;
    ...
    for (const itemData of mExtraItems.get(item.dataset.itemOwnerId)) {
      ...
      if (itemData.id === item.dataset.itemId) radioItems = currentRadioItems;
    }
    if (radioItems) {
      for (const itemData of radioItems) {
        itemData.checked = itemData.id === item.dataset.itemId;
        const radioItem = document.getElementById(
          `${item.dataset.itemOwnerId}-${itemData.id}`
        );
        if (radioItem) radioItem.classList.toggle("checked", itemData.checked);
        browser.runtime
          .sendMessage({
            type: Constants.kCOMMAND_NOTIFY_CONTEXT_ITEM_CHECKED_STATUS_CHANGED,
            id: item.dataset.itemId,
  ...
  const result = Promise.all([
    browser.runtime
      .sendMessage({
        ...
        tab:
          message.tab &&
          (await TSTAPI.exportTab(message.tab, browser.runtime.id, { cache })),
      })
      .catch(ApiTabs.createErrorSuppressor()),
...
function onMessage(message, _sender) {
  ...
  switch (message.type) {
    case Constants.kCOMMAND_NOTIFY_TABS_CLOSING:
      ...
      if (message.windowId !== TabsStore.getCurrentWindowId()) return;
    ...
    case Constants.kCOMMAND_NOTIFY_CONTEXT_MENU_UPDATED:
      {
        ...
        mIsDirty = true;
        if (mUI.opened) open(mLastOpenOptions);
      }
      break;
  }
}
...
function onMessageExternal(message, sender) {
  switch (message.type) {
    ...
    case TSTAPI.kFAKE_CONTEXT_MENU_OPEN:
      log("TSTAPI.kCONTEXT_MENU_OPEN:", message, {
        id: sender.id,
        url: sender.url,
      });
      return (async () => {
    ...
    case TSTAPI.kOVERRIDE_CONTEXT:
      ...
      mReservedOverrideContext =
        message.context === "bookmark"
          ? { context: "bookmark", bookmarkId: message.bookmarkId }
          : message.context === "tab"
            ? { context: "tab", tabId: message.tabId }
            : null;
...
async function onContextMenu(event) {
        ...
        if (
          ...
          notify({
            ...
            message: browser.i18n.getMessage(
              `bookmarkContext_notification_notPermitted_message${isLinux() ? "_linux" : ""}`
            ),
  ...
  const originalTargetBookmarkElement =
    originalTarget?.closest("[data-bookmark-id]");
  ...
  const originalTargetTabElement = originalTarget?.closest("[data-tab-id]");
  const tab = originalTargetTabElement
    ? TabsStore.ensureLivingTab(
        Tab.get(parseInt(originalTargetTabElement.dataset.tabId))
      )
  ...
  if (event.target === document.body) {
    // when the application key is pressed
