...
((mod) => {
  ...
  CodeMirror.defineMode("css", (config, parserConfig) => {
    ...
    function wordAsValue(stream) {
      ...
      if (Object.hasOwn(valueKeywords, word)) override = "atom";
      else if (Object.hasOwn(colorKeywords, word)) override = "keyword";
    ...
    states.block = (type, stream, state) => {
      if (type === "word") {
        var word = stream.current().toLowerCase();
        if (Object.hasOwn(propertyKeywords, word)) {
          override = "property";
          return "maybeprop";
        } else if (Object.hasOwn(nonStandardPropertyKeywords, word)) {
    ...
    states.documentTypes = (type, stream, state) => {
      if (type === "word" && Object.hasOwn(documentTypes, stream.current())) {
        override = "tag";
        return state.context.type;
      } else {
    ...
    states.atBlock = (type, stream, state) => {
      ...
      if (type === "word") {
        ...
        if (
          ...
          override = "keyword";
        else if (Object.hasOwn(mediaTypes, word)) override = "attribute";
        else if (Object.hasOwn(mediaFeatures, word)) override = "property";
        else if (Object.hasOwn(mediaValueKeywords, word)) override = "keyword";
        else if (Object.hasOwn(propertyKeywords, word)) override = "property";
        else if (Object.hasOwn(nonStandardPropertyKeywords, word))
          override = highlightNonStandardPropertyKeywords
            ? "string-2"
            : "property";
        else if (Object.hasOwn(valueKeywords, word)) override = "atom";
        else if (Object.hasOwn(colorKeywords, word)) override = "keyword";
        else override = "error";
      }
      return state.context.type;
      ...
      if (type === "word") {
        if (
          (state.stateArg === "@font-face" &&
            !Object.hasOwn(fontProperties, stream.current().toLowerCase())) ||
          (state.stateArg === "@counter-style" &&
            !Object.hasOwn(counterDescriptors, stream.current().toLowerCase()))
        )
          override = "error";
