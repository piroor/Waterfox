...
export default class EventListenerManager {
  ...
  dispatch(...args) {
    ...
    if (results instanceof Promise) return results.then(this.normalizeResults);
    else return this.normalizeResults(results);
  }
          ...
          results[index++] = result
            ...
            .then((result) => {
              ...
              return {
                ...
                listener:
                  EventListenerManager.debug &&
                  this._sourceOfListeners.get(listener),
              };
            });
        if (timer) clearTimeout(timer);
        results[index++] = {
          ...
          async: false,
          listener:
            EventListenerManager.debug && this._sourceOfListeners.get(listener),
        };
      } catch (e) {
        console.log(e);
        if (timer) clearTimeout(timer);
