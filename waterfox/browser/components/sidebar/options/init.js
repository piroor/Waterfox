...
const options = new Options(configs, {
  ...
  onImporting(values) {
    for (const key of DEVICE_SPECIFIC_CONFIG_KEYS) {
      if (
        JSON.stringify(configs[key]) !== JSON.stringify(configs.$default[key])
      )
...
if (
  (location.hash && /^#!?$/.test(location.hash)) ||
...
function onConfigChanged(key) {
  ...
  switch (key) {
    ...
    case "parentTabOperationBehaviorMode":
      {
        const nodes = document.querySelectorAll(
          "#parentTabOperationBehaviorModeGroup > ul > li > :not(label)"
        );
        for (const node of nodes) {
          node.style.display = node.parentNode.querySelector(
            'input[type="radio"]'
          ).checked
            ? ""
            : "none";
          const chosen = node.querySelector(
            `[type="radio"][data-config-key="closeParentBehavior"][value="${configs.closeParentBehavior}"]`
          );
          if (chosen) {
            chosen.checked = true;
            continue;
          }
          const chooser = node.querySelector(
            '[data-config-key="closeParentBehavior"]'
          );
          if (chooser) chooser.value = configs.closeParentBehavior;
        }
      }
      break;
    ...
    case "autoAttachOnAnyOtherTrigger":
      {
        const nodes = document.querySelectorAll(
          ".sub.autoAttachOnAnyOtherTrigger label, .sub.autoAttachOnAnyOtherTrigger select"
        );
        const disabled =
    ...
    case "showExpertOptions":
      {
        ...
        for (const item of document.querySelectorAll(
          "#parentTabOperationBehaviorModeGroup li li"
        )) {
    ...
    case "syncDeviceInfo":
      {
        ...
        const iconRadio = document.querySelector(
          `#syncDeviceInfoIcon input[type="radio"][value=${JSON.stringify(sanitizeForHTMLText(icon))}]`
        );
        if (iconRadio && !iconRadio.checked) iconRadio.checked = true;
      }
      break;
    ...
    default:
      if (
        key.startsWith("chunkedUserStyleRules") &&
...
function onChangeChildCheckbox(event) {
  getParentCheckboxFromChild(event.currentTarget).checked =
    isAllChildrenChecked(event.currentTarget);
  saveLogForConfig();
}
...
async function onChangeBookmarkPermissionRequiredCheckboxState(event) {
  ...
  if (permissionCheckbox.checked) return;
...
function updateCtrlTabSubItems(enabled) {
  const elements = document.querySelectorAll(
    "#ctrlTabSubItemsContainer label, #ctrlTabSubItemsContainer input, #ctrlTabSubItemsContainer select"
  );
  if (enabled) {
...
function applyUserStyleRulesFieldTheme() {
  ...
  if (
    ...
    !document.querySelector(
      `link[href$="/extlib/codemirror-theme/${theme}.css"]`
    )
  ) {
    const range = document.createRange();
...
function saveLogForConfig() {
  ...
  for (const checkbox of document.querySelectorAll(
    'p input[type="checkbox"][id^="logFor-"]'
  )) {
...
async function updateBookmarksUI(enabled) {
  const elements = document.querySelectorAll(
    ".with-bookmarks-permission, .with-bookmarks-permission label, .with-bookmarks-permission input, .with-bookmarks-permission button"
  );
  if (enabled) {
    for (const element of elements) {
      element.removeAttribute("disabled");
    ...
    const defaultBookmarkParentChooser = document.getElementById(
    ...
    Bookmark.initFolderChooser(defaultBookmarkParentChooser, {
      ...
      onCommand: (item, _event) => {
        if (item.dataset.id) configs.defaultBookmarkParentId = item.dataset.id;
      },
      rootItems: (
        await browser.bookmarks.getTree().catch(ApiTabs.createErrorHandler())
      )[0].children,
  ...
  const triboolChecks = document.querySelectorAll(
    'input[type="checkbox"].require-bookmarks-permission, .require-bookmarks-permission input[type="checkbox"]'
  );
  if (enabled) {
    ...
    for (const checkbox of triboolChecks) {
      ...
      const message = checkbox.dataset.requestPermissionMessage;
      if (message) checkbox.parentNode.setAttribute("title", message);
    }
  }
}
...
async function initOtherDevices() {
  ...
  for (const device of devices) {
    const icon = device.icon
      ? `<img src="/resources/icons/${sanitizeForHTMLText(device.icon)}.svg">`
      : "";
...
function initUserStyleImportExportButtons() {
  ...
  exportButton.addEventListener("keydown", (event) => {
    if (event.key === "Enter" || event.key === " ") exportUserStyleToFile();
  });
  exportButton.addEventListener("click", (event) => {
    if (event.button === 0) exportUserStyleToFile();
  });
...
function initFileDragAndDropHandlers() {
  mUserStyleRulesField.addEventListener(
    "dragenter",
    (event) => {
  ...
  mUserStyleRulesField.addEventListener(
    ...
    (event) => {
      ...
      const hasFile = Array.from(dt.items, (item) => item.kind).some(
        (kind) => kind === "file"
      );
  ...
  mUserStyleRulesField.addEventListener(
    ...
    (event) => {
      ...
      if (files && files.length > 0) importFilesToUserStyleRulesField(files);
    },
    { capture: true }
  );
}
  ...
  if (files.some((file) => file.type.startsWith("image/"))) {
    ...
    try {
      result = await RichConfirm.showInPopup({
        modal: true,
        type: "common-dialog",
        url: (await Permissions.isGranted(Permissions.ALL_URLS))
          ? null
...
function updateThemeInformation(theme) {
  ...
  range.detach();
  document.getElementById("browserThemeCustomRulesBlock").style.display = rules
    ? "block"
    : "none";
}
...
function autoDetectDuplicatedTabDetectionDelay() {
  browser.runtime.sendMessage({
    type: Constants.kCOMMAND_AUTODETECT_DUPLICATED_TAB_DETECTION_DELAY,
  });
}

async function testDuplicatedTabDetection() {
  const successRate = await browser.runtime.sendMessage({
    type: Constants.kCOMMAND_TEST_DUPLICATED_TAB_DETECTION,
  });
  document.querySelector(
    "#delayForDuplicatedTabDetection_testResult"
  ...
  async () => {
    try {
      document.documentElement.classList.toggle(
        "successor-tab-support",
        typeof browser.tabs.moveInSuccession === "function"
      );
      document.documentElement.classList.toggle(
        "expose-unblock-autoplay-features",
        configs.exposeUnblockAutoplayFeatures
      );
    ...
    mShowExpertOptionsTemporarily = !!(
      ...
      !/^#!?$/.test(location.hash) &&
      document.querySelector(
        `.expert #${location.hash.replace(/^#!?/, "")}, .expert#${location.hash.replace(/^#!?/, "")}`
      )
    );
    ...
    try {
      ...
      if (focusedItem) focusedItem.scrollIntoView({ block: "start" });
    } catch (error) {
      console.error(error);
    }
...
function initAccesskeys() {
  for (const label of document.querySelectorAll(".contextConfigs label")) {
    for (const child of label.childNodes) {
      if (child.nodeType === Node.TEXT_NODE) removeAccesskeyMark(child);
    }
  }
}
...
function initDuplicatedTabDetection() {
  const autoDetectDuplicatedTabDetectionDelayButton = document.getElementById(
    "delayForDuplicatedTabDetection_autoDetectButton"
  );
  autoDetectDuplicatedTabDetectionDelayButton.addEventListener(
    "click",
    (event) => {
      if (event.button !== 0) return;
  ...
  const testDuplicatedTabDetectionButton = document.getElementById(
    "delayForDuplicatedTabDetection_testButton"
  );
...
function initLinks() {
  document
    .getElementById("link-optionsPage-top")
    .setAttribute("href", `${location.href.split("#")[0]}#!`);
  document
    .getElementById("link-optionsPage")
    .setAttribute("href", `${location.href.split("#")[0]}#!`);
  document
    .getElementById("link-startupPage")
    .setAttribute("href", Constants.kSHORTHAND_URIS.startup);
  document
    .getElementById("link-groupPage")
    .setAttribute("href", Constants.kSHORTHAND_URIS.group);
  document
    .getElementById("link-tabbarPage")
    .setAttribute("href", Constants.kSHORTHAND_URIS.tabbar);
  ...
  document
    .getElementById("link-runBenchmark")
    .setAttribute(
      "href",
      `${Constants.kSHORTHAND_URIS.testRunner}?benchmark=true`
    );
}
...
function initTheme() {
  if (browser.theme?.getCurrent) {
    ...
    browser.theme.onUpdated.addListener((updateInfo) =>
      updateThemeInformation(updateInfo.theme)
    );
  }
}
...
function initFocusedItem() {
  ...
  for (const fieldset of document.querySelectorAll("fieldset.collapsible")) {
    if (
      configs.optionsExpandedGroups.includes(fieldset.id) ||
    ...
    const onChangeCollapsed = () => {
      if (!fieldset.id) return;
      const otherExpandedSections = configs.optionsExpandedGroups.filter(
        (id) => id !== fieldset.id
      );
      if (fieldset.classList.contains("collapsed"))
        configs.optionsExpandedGroups = otherExpandedSections;
      else
        configs.optionsExpandedGroups = otherExpandedSections.concat([
          fieldset.id,
        ]);
    };
...
function initCollapsibleSections({ focusedItem }) {
  for (const heading of document.querySelectorAll("body > section > h1")) {
    ...
    if (
      !configs.optionsExpandedSections.includes(section.id) &&
      (!focusedItem || !section.contains(focusedItem))
    )
      section.classList.add("collapsed");
    heading.addEventListener("click", () => {
      section.classList.toggle("collapsed");
      const otherExpandedSections = configs.optionsExpandedSections.filter(
        (id) => id !== section.id
      );
      if (section.classList.contains("collapsed"))
        configs.optionsExpandedSections = otherExpandedSections;
      else
        configs.optionsExpandedSections = otherExpandedSections.concat([
          section.id,
        ]);
    });
  }
...
function initPermissionOptions() {
  ...
  Permissions.isGranted(Permissions.BOOKMARKS).then((granted) =>
    updateBookmarksUI(granted)
  );
  Permissions.isGranted(Permissions.ALL_URLS).then((granted) =>
  ...
  for (const checkbox of bookmarksPermissionCheckboxes) {
    Permissions.bindToCheckbox(Permissions.BOOKMARKS, checkbox, {
  ...
  Permissions.bindToCheckbox(
    ...
    document.querySelector(
      "#clipboardReadPermissionGranted_middleClickPasteURLOnNewTabButton"
    ),
    {
      ...
      onChanged: async (granted) => {
        if (granted) {
          ...
          const onUpdated = async (tabId, changeInfo, tab) => {
            if (tabId !== tab.id || !("hidden" in changeInfo)) return;
            await wait(60 * 1000);
            if (aborted) return;
  ...
  for (const checkbox of document.querySelectorAll(
    'input[type="checkbox"].require-bookmarks-permission'
  )) {
    checkbox.addEventListener(
      "change",
      onChangeBookmarkPermissionRequiredCheckboxState
    );
  }

  for (const node of document.querySelectorAll(".with-bookmarks-permission")) {
    Permissions.bindToClickable(Permissions.BOOKMARKS, node, {
      onChanged: (granted) => updateBookmarksUI(granted),
    });
  }
}
...
function initLogCheckboxes() {
  ...
  for (const checkbox of document.querySelectorAll(
    'legend input[type="checkbox"][id^="logFor-"]'
  )) {
...
function initPreviews() {
  for (const previewImage of document.querySelectorAll(
...
async function initExternalAddons() {
  ...
  const description = document.getElementById(
    "externalAddonPermissionsGroupDescription"
  );
  const range = document.createRange();
  range.selectNodeContents(description);
  description.appendChild(
    range.createContextualFragment(
      browser.i18n.getMessage("config_externaladdonpermissions_description")
    )
  );
  range.detach();
  ...
  for (const addon of addons) {
    if (addon.id === browser.runtime.id) continue;
    ...
    const incognitoLabel = incognitoCell.appendChild(
      document.createElement("label")
    );
    const incognitoCheckbox = incognitoLabel.appendChild(
    ...
    incognitoCheckbox.checked = configs.incognitoAllowedExternalAddons.includes(
      addon.id
    );
    incognitoCheckbox.addEventListener("change", () => {
      const updatedValue = new Set(configs.incognitoAllowedExternalAddons);
      if (incognitoCheckbox.checked) updatedValue.add(addon.id);
      else updatedValue.delete(addon.id);
    ...
    if (addon.permissions.length > 0) {
      const permissionsLabel = permissionsCell.appendChild(
        document.createElement("label")
      );
      const permissionsCheckbox = permissionsLabel.appendChild(
        document.createElement("input")
      );
      ...
      permissionsCheckbox.addEventListener("change", () => {
        browser.runtime.sendMessage({
          ...
          permissions: permissionsCheckbox.checked
            ? addon.permissions
            : addon.permissions.map((permission) => `!  ${permission}`),
        });
      });
      const permissionNames = addon.permissions
        .map((permission) => {
          try {
            return (
...
function initSync() {
  ...
  deviceInfoNameField.addEventListener("input", () => {
    ...
    deviceInfoNameField.$throttling = setTimeout(async () => {
      ...
      configs.syncDeviceInfo = JSON.parse(
        JSON.stringify({
  ...
  const deviceInfoIconRadiogroup = document.querySelector(
    "#syncDeviceInfoIcon"
  );
  deviceInfoIconRadiogroup.addEventListener("change", (_event) => {
    ...
    deviceInfoIconRadiogroup.$throttling = setTimeout(async () => {
      ...
      const checkedRadio = deviceInfoIconRadiogroup.querySelector(
...
import("/extlib/codemirror.js").then(async () => {
  ...
  mUserStyleRulesFieldEditor = CodeMirror(mUserStyleRulesField, {
